  ...
  if (keylist) {
    ...
    for (i = 0; NULL != (skey = index234(ssh2keys, i)); i++) {
      ...
      if (len < sizeof(listentry) - 2) {
        listentry[len] = '\t';
        strncpy(
            listentry + len + 1, skey->comment, sizeof(listentry) - len - 1);
      }
      SendDlgItemMessage(keylist, 100, LB_ADDSTRING, 0, (LPARAM)listentry);
    }
...
static void add_keyfile(Filename filename)
{
   ...
   */
  {
    ...
    if (keylist) {
      if (keylistlen < 4) {
        MessageBox(
            NULL, "Received broken key list?!", APPNAME, MB_OK | MB_ICONERROR);
        return;
      }
    ...
    if (needs_pass) {
      ...
      if (pp) {
        ...
        dlgret = DialogBoxParam(
            instance, MAKEINTRESOURCE(210), NULL, PassphraseProc, (LPARAM)&pps);
        passphrase_box = NULL;
        if (!dlgret) {
          if (comment)
            sfree(comment);
                                ...
                                LPARAM lParam)
{
  ...
  case WM_HELP:
    if (help_path) {
      ...
      switch (id) {
      case 100:
        cmd = "JI(`',`pageant.keylist')";
        break;
      case 101:
        cmd = "JI(`',`pageant.addkey')";
        break;
      case 102:
        cmd = "JI(`',`pageant.remkey')";
        break;
      }
      if (cmd) {
                                ...
                                LPARAM lParam)
{
  ...
  case WM_SYSCOMMAND:
    ...
    case IDM_ABOUT:
      if (!aboutbox) {
        aboutbox =
            CreateDialog(instance, MAKEINTRESOURCE(213), NULL, AboutProc);
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
  ...
  if (agent_exists())
    ...
    main_hwnd = CreateWindow(APPNAME,
                             APPNAME,
