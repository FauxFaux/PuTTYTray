                         ...
                         pageant_logfn_t logfn)
{
     ...
     */
    {
      ...
      if (logfn) { /* skip this loop if not logging */
        ...
        for (i = 0; NULL != (skey = pageant_nth_ssh2_key(i)); i++) {
          char *fingerprint = ssh2_fingerprint(skey->alg, skey->data);
          plog(
              logctx, logfn, "returned key: %s %s", fingerprint, skey->comment);
          sfree(fingerprint);
        }
      }
  ...
  case SSH2_AGENTC_SIGN_REQUEST:
     ...
     */
    {
      ...
      signature =
          key->alg->sign(key->data, (const char *)data, datalen, &siglen);
      len = 5 + 4 + siglen;
      PUT_32BIT(ret, len - 4);
  ...
  case SSH2_AGENTC_ADD_IDENTITY:
     ...
     */
    {
      ...
      if (logfn) {
        ...
        plog(logctx, logfn, "submitted key: %s %s", fingerprint, key->comment);
        sfree(fingerprint);
      }
...
static void pageant_conn_closing(Plug plug,
...
static void pageant_conn_receive(Plug plug, int urgent, char *data, int len)
{
  ...
  while (len > 0) {
...

    {
      ...
      if (pc->real_packet) {
        reply = pageant_handle_msg(pc->pktbuf,
                                   pc->len,
                                   &replylen,
                                   pc,
                                   pc->logfn ? pageant_conn_log : NULL);
      } else {
        plog(pc->logctx, pc->logfn, "%p: overlong message (%u)", pc, pc->len);
        plog(pc->logctx,
             pc->logfn,
             "%p: reply: SSH_AGENT_FAILURE "
             "(message too long)",
             pc);
...
static void pageant_listen_closing(Plug plug,
                                   const char *error_msg,
                                   int error_code,
                                   int calling_back)
{
