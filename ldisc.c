...
void *ldisc_create(
    Conf *conf, Terminal *term, Backend *back, void *backhandle, void *frontend)
{
  ...
  if (EDITING) {
    while (len--) {
      ...
      case CTRL('D'): /* logout or send */
         ...
         */
      case CTRL('J'):
        if (ldisc->protocol == PROT_RAW && ldisc->buflen > 0 &&
