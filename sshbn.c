 ...
 */
static void internal_mul(unsigned short *a,
                         unsigned short *b,
                         unsigned short *c,
                         int len)
{
...
static void internal_add_shifted(unsigned short *number, unsigned n, int shift)
{
  int word = 1 + (shift / 16);
  int bshift = shift % 16;
 ...
 */
static void internal_mod(unsigned short *a,
                         int alen,
                         unsigned short *m,
                         int mlen,
                         unsigned short *quot,
                         int qshift)
{
  ...
  for (i = 0; i <= alen - mlen; i++) {
    ...
    if (t > ((unsigned long)r << 16) + ai1) {
      ...
      if (r >= (unsigned long)m0 && t > ((unsigned long)r << 16) + ai1)
        q--;
    }
...
Bignum bigmuladd(Bignum a, Bignum b, Bignum addend)
{
  ...
  internal_mul(
      workspace + 0 * mlen, workspace + 1 * mlen, workspace + 2 * mlen, mlen);
