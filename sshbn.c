 ...
 * off the top.
 */
static BignumInt internal_add(const BignumInt *a,
                              const BignumInt *b,
                              BignumInt *c,
                              int len)
{
  ...
  for (i = 0; i <= alen - mlen; i++) {
    ...
    if (quot)
      internal_add_shifted(
          quot, q, qshift + BIGNUM_INT_BITS * (alen - mlen - i));
  }
}
...
int bignum_bitcount(Bignum bn)
{
  ...
  while (bitcount >= 0 && (bn[bitcount / BIGNUM_INT_BITS + 1] >>
...
int bignum_byte(Bignum bn, int i)
{
  if (i < 0 || i >= (int)(BIGNUM_INT_BYTES * bn[0]))
    ...
    return (bn[i / BIGNUM_INT_BYTES + 1] >> ((i % BIGNUM_INT_BYTES) * 8)) &
           0xFF;
}
...
/*
 * gcc -Wall -g -O0 -DTESTBN -o testbn sshbn.c misc.c conf.c tree234.c
 * unix/uxmisc.c -I. -I unix -I charset
 *
