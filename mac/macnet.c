...
static struct macnet_stack ot = {ot_namelookup,
                                 ot_nonamelookup,
                                 ot_getaddr,
 ...
 */
SockAddr sk_namelookup(char const *host,
                       char **canonicalname,
                       int address_family)
{
              ...
              Plug plug)
{
  ...
  if (stack != NULL)
    return stack->new (
        addr, port, privport, oobinline, nodelay, keepalive, plug);
  return NULL;
}
    ...
    char *srcaddr, int port, Plug plug, int local_host_only, int address_family)
{
  ...
  if (stack != NULL)
    return stack->newlistener(
        srcaddr, port, plug, local_host_only, address_family);
  return NULL;
}
