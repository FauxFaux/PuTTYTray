...
#define PUT_PERMISSIONS(attrs, perms)                                          \
  ((perms) >= 0 ? ((attrs).flags |= SSH_FILEXFER_ATTR_PERMISSIONS,             \
                   (attrs).permissions = (perms))                              \
                : ((attrs).flags &= ~SSH_FILEXFER_ATTR_PERMISSIONS))
#define GET_PERMISSIONS(attrs)                                                 \
  ((attrs).flags & SSH_FILEXFER_ATTR_PERMISSIONS ? (attrs).permissions : -1)
 ...
 */
struct sftp_request *fxp_open_send(char *path,
                                   int type,
                                   struct fxp_attrs *attrs);
