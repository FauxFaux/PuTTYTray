...
#define fix_cpos                                                               \
  do {                                                                         \
    cpos = scrtop + curs_y * (cols + 1) + curs_x;                              \
  } while (0)
...
static void scroll(int topline, int botline, int lines, int sb)
{
  ...
  if (lines > 0) {
    ...
    if (selend > scroll_top && selend < scroll_top + size + scroll_size) {
      selend -= size;
...
void term_out(void)
{
  ...
  for (inbuf_reap = 0; inbuf_reap < inbuf_head; inbuf_reap++) {
     ...
     * of i18n.
     */
    if (((c & 0x60) == 0 || c == '\177') && termstate < DO_CTRLS &&
      ...
      case SEEN_CSI:
        ...
        if (isdigit(c)) {
          ...
          case 'm': /* set graphics rendition */
          {
            ...
            if (use_bce)
              erase_char = (' ' | (curr_attr &
                                   (ATTR_FGMASK | ATTR_BGMASK | ATTR_BLINK)));
          } break;
          case 's': /* save cursor */
            save_cursor(TRUE);
            break;
...
static void do_paint(Context ctx, int may_optimise)
{
  ...
  for (i = 0; i < rows; i++) {
    ...
    for (j = 0; j <= cols; j++, idx++) {
      ...
      wanttext[idx] =
          lattr |
          ((*d ^ rv ^ (selstart <= d && d < selend ? ATTR_REVERSE : 0)) |
  ...
  for (i = 0; i < rows; i++) {
    ...
    for (j = 0; j <= cols; j++, idx++) {
      ...
      int keep_going = (start != -1 && needs_update &&
                        (t & ATTR_MASK) == attr && j - start < sizeof(ch));
...
void term_scroll(int rel, int where)
{
  ...
  disptop = (rel < 0 ? scrtop : rel > 0 ? sbtop : disptop) + n;
  if (disptop < sbtop)
    disptop = sbtop;
  if (disptop > scrtop)
    disptop = scrtop;
