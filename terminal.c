...
static void do_paint(Terminal *term, Context ctx, int may_optimise)
{
   ...
   *
   * TODO: We should find out somehow that the stuff on screen has changed since
   * last paint. How to do it?
   */
                ...
                int alt)
{
    ...
    if (term->ldisc) {
      ...
      if (wheel) {
        ...
        case MA_CLICK:
          ...
          // MORE HACKING (@unphased: allow sequences of mouse wheel up and
          // mouse wheel down to pass through)
          if (term->mouse_is_down == braw && braw != MBT_WHEEL_UP &&
              braw != MBT_WHEEL_DOWN) {
            unlineptr(ldata);
            return;
          }
  ...
  if (bcooked == MBT_SELECT && a == MA_CLICK) {
    ...
    if (term->selstate == ABOUT_TO &&
        poseq(term->selanchor, selpoint)) { // HACK: ADDED FOR HYPERLINK STUFF
      unlineptr(ldata);
      return;
    }
