...
static void child_launch_handler(union control *ctrl,
                                 void *dlg,
                                 void *data,
                                 int event)
{
...
static void help_handler(union control *ctrl, void *dlg, void *data, int event)
{
  HWND *hwndp = (HWND *)ctrl->generic.context.p;
                          ...
                          int protocol)
{
  ...
  c = ctrl_pushbutton(
      s, "About", NO_SHORTCUT, HELPCTX(no_help), about_handler, P(hwndp));
  ...
  if (has_help) {
    c = ctrl_pushbutton(
        s, "Help", 'h', HELPCTX(no_help), help_handler, P(hwndp));
    c->generic.column = col++;
  }
   ...
   */
  s = ctrl_getset(
      b, "Terminal/Keyboard", "features", "Enable extra keyboard features:");
  ctrl_checkbox(s,
  ...
  ctrl_checkbox(s,
                ...
                I(CONF_rightaltkey));
  ctrl_checkbox(s,
                "Set meta bit on alt (instead of escape)",
                'm',
                HELPCTX(no_help),
                conf_checkbox_handler,
                I(CONF_alt_metabit));
  ...
  s = ctrl_getset(b, "Terminal/Bell", "style", "Set the style of bell");
  {
    ...
    for (i = 0; i < s->ncontrols; i++) {
      c = s->ctrls[i];
      if (c->generic.type == CTRL_RADIO && c->generic.context.i == CONF_beep) {
  ...
  ctrl_checkbox(s,
                ...
                conf_checkbox_handler,
                I(CONF_system_colour));
  ctrl_editbox(s,
               "Window opacity (50-255)",
               't',
               30,
               HELPCTX(no_help),
               conf_editbox_handler,
  ...
  ctrl_radiobuttons(s,
                    ...
                    I(RESIZE_TERM),
                    "Change rows and columns only when maximised",
                    I(RESIZE_MAXTERM),
  ...
  s = ctrl_getset(
      b, "Connection", "reconnect", "Reconnect options (UNSUPPORTED)");
  ctrl_checkbox(s,
  ...
  s = ctrl_getset(
      b, "Window/Hyperlinks", "general", "General options for hyperlinks");
  ...
  ctrl_radiobuttons(s,
                    ...
                    "Always",
                    NO_SHORTCUT,
                    I(URLHACK_UNDERLINE_ALWAYS),
                    "When hovered upon",
                    NO_SHORTCUT,
                    I(URLHACK_UNDERLINE_HOVER),
                    "Never",
                    NO_SHORTCUT,
                    I(URLHACK_UNDERLINE_NEVER),
                    NULL);

  ctrl_checkbox(s,
                "Use ctrl+click to launch hyperlinks",
                'l',
                HELPCTX(no_help),
                conf_checkbox_handler,
  ...
  ctrl_filesel(s,
               "or specify an application to open hyperlinks with:",
               's',
               "Application (*.exe)\0*.exe\0All files (*.*)\0*.*\0\0",
               TRUE,
               "Select executable to open hyperlinks with",
               HELPCTX(no_help),
               conf_filesel_handler,
               I(CONF_url_browser));
  ...
  ctrl_radiobuttons(s,
                    ...
                    conf_radiobutton_handler,
                    I(CONF_url_defregex),
                    "Select sensible URLs",
                    NO_SHORTCUT,
                    I(URLHACK_REGEX_CLASSIC),
                    "Select nearly any URL",
                    NO_SHORTCUT,
                    I(URLHACK_REGEX_LIBERAL),
                    "Custom",
                    NO_SHORTCUT,
                    I(URLHACK_REGEX_CUSTOM),
                    NULL);
