                       ...
                       int address_family)
{
  ...
  ret->family = (address_family == ADDRTYPE_IPV4
                     ? AF_INET
                     :
#ifndef NO_IPV6
  ...
  if ((a = p_inet_addr(host)) == (unsigned long)INADDR_NONE) {
    ...
    if (ret_family == AF_UNSPEC) {
      ...
      if (ret->ai) {
        ...
        if (p_getnameinfo) {
          if (p_getnameinfo((struct sockaddr *)ret->ai->ai_addr,
...
void sk_getaddr(SockAddr addr, char *buf, int buflen)
{
  ...
  if (addr->ai) {
    ...
    if (dllWS2) {
      fAddrToStr = (FADDRTOSTR)GetProcAddress(dllWS2, "WSAAddressToStringA");
      if (fAddrToStr) {
        fAddrToStr(addr->ai->ai_addr, addr->ai->ai_addrlen, NULL, buf, &buflen);
      } else
...
static DWORD try_connect(Actual_Socket sock)
{
  ...
  while (1) {
    ...
    sockcode =
        p_bind(s,
               (sock->addr->family == AF_INET6 ? (struct sockaddr *)&a6
                                               : (struct sockaddr *)&a),
  ...
  if (sock->addr->ai) {
    if (family == AF_INET6) {
      ...
      a.sin_family = AF_INET;
      a.sin_addr = ((struct sockaddr_in *)sock->addr->ai->ai_addr)->sin_addr;
...
int select_result(WPARAM wParam, LPARAM lParam)
{
  ...
  if ((err = WSAGETSELECTERROR(lParam)) != 0) {
     ...
     */
    if (s->addr) {
      plug_log(s->plug, 1, s->addr, s->port, winsock_error_string(err), err);
