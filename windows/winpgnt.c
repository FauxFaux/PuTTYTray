...
BOOL starts_at_startup()
{
  ...
  HKEY run;
  run = run_key();
  RegQueryValueEx(run, APPNAME, NULL, NULL, (LPBYTE)them, &len);
  RegCloseKey(run);
...
BOOL reg_keys(HKEY *hkey)
{
  return ERROR_SUCCESS ==
         RegOpenKeyEx(HKEY_CURRENT_USER, PAGEANT_REG_KEYS, 0, KEY_READ, hkey);
}
 ...
 */
static int CALLBACK PassphraseProc(HWND hwnd,
                                   UINT msg,
                                   WPARAM wParam,
                                   LPARAM lParam)
{
...
{
  return ERROR_SUCCESS ==
         RegCreateKey(HKEY_CURRENT_USER, PAGEANT_REG_KEYS, hkey);
}
...
static char *add_keyfile_ret(Filename *filename)
{
  ...
  if (type != SSH_KEYTYPE_SSH1 && type != SSH_KEYTYPE_SSH2 &&
    ...
    char *msg = dupprintf("Couldn't load this key (%s)", key_type_to_str(type));
    return msg;
  }
  ...
  if (type == SSH_KEYTYPE_SSH1) {
    if (already_running) {
      ...
      reqlen = 4 + 1 + /* length, message type */
               ...
               ssh1_bignum_length(rkey->iqmp) + ssh1_bignum_length(rkey->p) +
    ...
    if (already_running) {
      ...
      reqlen += alglen;
      reqlen +=
          skey->alg->openssh_fmtkey(skey->data, request + reqlen, keybloblen);
      PUT_32BIT(request + reqlen, clen);
...
static void add_keyfile(Filename *filename)
{
  ...
  if (msg) {
    ...
    sfree(msg);
    message_box(
        extmsg, APPNAME, MB_OK | MB_ICONERROR, HELPCTXID(errors_cantloadkey));
    sfree(extmsg);
  }
}
...
static void load_registry_keys()
{
  ...
  if (ERROR_SUCCESS ==
    ...
    char name[MAX_PATH];
    while (ERROR_SUCCESS ==
           RegEnumValue(hkey, i++, name, &namelen, NULL, NULL, NULL, NULL)) {
      ...
      extmsg = dupprintf("Couldn't load '%s': %s\n\n"
                         "Would you like to remove it from the list of "
                         "automatically loaded keys?",
                         filename_to_str(filename),
                         msg);
      sfree(msg);
      switch (message_box(extmsg,
                          APPNAME,
...
static void answer_msg(void *msg)
{
  ...
  case SSH2_AGENTC_SIGN_REQUEST:
     ...
     */
    {
      ...
      // Presumably this is in response to user action, so SYSTEMMODAL
      // (toppmost) seems reasonable
      if (!confirm_mode ||
        ...
        // errors cause the client to abort the connection, this seems like a
        // better fallback
        signature = _strdup("");
        siglen = 0;
      }
 ...
 */
static int CALLBACK KeyListProc(HWND hwnd,
                                UINT msg,
                                WPARAM wParam,
                                LPARAM lParam)
{
  ...
  case WM_COMMAND:
    ...
    case 108: /* copy fingerprints */
      if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED) {
         ...
         */
        for (i = sCount - 1; (itemNum >= 0 || numSelected == 0) && (i >= 0);
             i--) {
    ...
    case 107: /* add ~/.ssh/id_rsa */
    {
      ...
      if (!file_exists(path->path) &&
          IDYES ==
              MessageBox(
                  hwnd,
                  "~/.ssh/id_rsa doesn't exist, would you like to create it?",
                  APPNAME,
                  MB_YESNO)) {
                                ...
                                LPARAM lParam)
{
  ...
  case WM_SYSTRAY_LEFT_CLICK:
    if (!menuinprogress) {
      ...
      update_sessions();
      SetForegroundWindow(hwnd);
      ret = TrackPopupMenu(message == WM_SYSTRAY_LEFT_CLICK ? session_menu
                                                            : systray_menu,
  ...
  case WM_SYSCOMMAND:
    ...
    case IDM_PUTTY:
      if ((int)ShellExecute(
              hwnd, NULL, our_path, _T("--as-putty"), _T(""), SW_SHOW) <= 32) {
        MessageBox(
    ...
    case IDM_CLOSE:
      ...
      // GD: For some reasons, just sending a close message does not exit
      // cleanly
      //     This patch is copied from the main exit function at the bottom of
      //     this file
    ...
    case IDM_CONFIRM_KEY_USAGE:
      ...
      CheckMenuItem(systray_menu,
                    IDM_CONFIRM_KEY_USAGE,
    ...
    default: {
      if (wParam >= IDM_SESSIONS_BASE && wParam <= IDM_SESSIONS_MAX) {
        ...
        if ((int)ShellExecute(hwnd, NULL, our_path, param, _T(""), SW_SHOW) <=
            32) {
          MessageBox(
              NULL, "Unable to execute PuTTY!", "Error", MB_OK | MB_ICONERROR);
        }
...
{
  if (ShellExecute(NULL, _T("open"), cmdline, args, NULL, show) <=
      (HINSTANCE)32) {
    char *msg;
    msg = dupprintf(
        "Failed to run \"%.100s\", Error: %d", cmdline, (int)GetLastError());
...
int pageant_main(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
  ...
  AppendMenu(systray_menu,
             MF_ENABLED | (confirm_mode ? MF_CHECKED : MF_UNCHECKED),
             IDM_CONFIRM_KEY_USAGE,
             "&Confirm key usage");
  AppendMenu(systray_menu,
             MF_ENABLED | (starts_at_startup() ? MF_CHECKED : MF_UNCHECKED),
             IDM_START_AT_STARTUP,
             "&Start with Windows");
  AppendMenu(systray_menu,
             MF_ENABLED | (saves_keys() ? MF_CHECKED : MF_UNCHECKED),
             IDM_SAVE_KEYS,
             "&Persist keys");
