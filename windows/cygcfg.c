...
extern void config_protocolbuttons_handler(union control *,
                                           void *,
                                           void *,
                                           int);
...
static int is64Bits()
{
  return (NULL != GetProcAddress(GetModuleHandle("kernel32"), "IsWow64Process"))
             ? 1
             : 0;
}
...
void cygterm_setup_config_box(struct controlbox *b, int midsession)
{
  ...
  if (!midsession) {
    ctrl_settitle(
        b, "Connection/Cygterm", "Options controlling Cygterm sessions");
    s = ctrl_getset(
        b, "Connection/Cygterm", "cygterm", "Configure Cygwin paths");
    ctrl_checkbox(s,
                  "Autodetect Cygwin installation",
                  'd',
                  HELPCTX(no_help),
