...
static enum { BOLD_NONE, BOLD_SHADOW, BOLD_FONT } bold_font_mode;
...
static char *stpcpy_max(char *dst, const char *src, size_t n)
{
  while (n-- && (*dst = *src++))
    dst++;
  return dst;
...
int putty_main(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
   ...
   */
  {
    ...
    if (!setprocessacl(error)) {
      char *message = dupprintf("Could not restrict process ACL: %s", error);
    ...
    if (*p == '@') {
      ...
      for (i = 0; i < argc; i++) {
        ...
        ret = cmdline_process_param(
            p, i + 1 < argc ? argv[i + 1] : NULL, 1, conf);
        if (ret == -2) {
  ...
  if (!prev) {
    ...
    if (win_icon && filename_to_str(win_icon)[0]) {
      ...
      wndclass.hIcon = LoadImage(inst,
                                 MAKEINTRESOURCE(IDI_MAINICON),
                                 IMAGE_ICON,
                                 GetSystemMetrics(SM_CXICON),
                                 GetSystemMetrics(SM_CYICON),
                                 LR_DEFAULTCOLOR | LR_SHARED);
      wndclass.hIconSm = LoadImage(inst,
                                   MAKEINTRESOURCE(IDI_MAINICON),
                                   IMAGE_ICON,
...

  {
    ...
    hwnd = CreateWindowExW(exwinmode,
                           uappname,
                           uappname,
                           winmode,
                           CW_USEDEFAULT,
  ...
  if (conf_get_int(conf, CONF_transparency) >= 50 &&
      conf_get_int(conf, CONF_transparency) < 255) {
  ...
  while (1) {
    ...
    n = MsgWaitForMultipleObjects(
        nhandles, handles, FALSE, timeout, QS_ALLINPUT);
...
static void init_fonts(int pick_width, int pick_height)
{
  ...
  bold_font_mode =
      conf_get_int(conf, CONF_bold_style) & 1 ? BOLD_FONT : BOLD_NONE;
...
void request_resize(void *frontend, int w, int h)
{
  ...
  if (IsZoomed(hwnd)) {
    if (conf_get_int(conf, CONF_resize_action) == RESIZE_MAXTERM ||
        conf_get_int(conf, CONF_resize_action) == RESIZE_TERM)
      return;
  }
...
static void reset_window(int reinit)
{
  ...
  if (IsZoomed(hwnd)) {
    ...
    if (resize_action != RESIZE_MAXTERM && resize_action != RESIZE_TERM &&
...
static Mouse_Button translate_button(Mouse_Button button)
{
  ...
  if (button == MBT_MIDDLE)
    return conf_get_int(conf, CONF_mouse_is_xterm) == 1 ||
                   conf_get_int(conf, CONF_mouse_is_xterm) == 3
               ? MBT_PASTE
               : MBT_EXTEND;
  if (button == MBT_RIGHT)
    return conf_get_int(conf, CONF_mouse_is_xterm) == 1 ||
  ...
  case WM_SYSCOMMAND:
    ...
    case IDM_SAVEDSESS: {
      ...
      if (wParam == IDM_DUPSESS) {
        ...
        filemap = CreateFileMapping(
            INVALID_HANDLE_VALUE, &sa, PAGE_READWRITE, 0, size, NULL);
    ...
    case IDM_RECONF: {
      ...
      resize_action = conf_get_int(conf, CONF_resize_action);
      {
        ...
        int i;
        for (i = 0; i < lenof(popup_menus); i++)
          EnableMenuItem(popup_menus[i].menu,
                         IDM_FULLSCREEN,
      ...
      /* Enable or disable the scroll bar, etc */
      {
        ...
        if (conf_get_int(conf, CONF_alwaysontop) !=
          ...
          if (conf_get_int(conf, CONF_alwaysontop)) {
            ...
            // HACK: PuttyTray / Always on top:
            CheckMenuItem(
                GetSystemMenu(hwnd, FALSE), IDM_VISIBLE, MF_UNCHECKED);
          }
        }
        if (conf_get_int(conf, CONF_sunken_edge))
        ...
        nflg = flag;
        if (conf_get_int(conf,
                         is_full_screen() ? CONF_scrollbar_in_fullscreen
                                          : CONF_scrollbar))
          nflg |= WS_VSCROLL;
        else
          nflg &= ~WS_VSCROLL;
        ...
        if (resize_action == RESIZE_DISABLED ||
            resize_action == RESIZE_MAXTERM || is_full_screen())
          nflg &= ~WS_THICKFRAME;
        else
          nflg |= WS_THICKFRAME;
...

      {
        ...
        FontSpec *prev_font = conf_get_fontspec(prev_conf, CONF_font);
      ...
      while (abs(wheel_accumulator) >= WHEEL_DELTA) {
        ...
        if (send_raw_mouse &&
          ...
          if (control_pressed) {
            ...
            int scrollLines = scrolllines == -1
                                  ? term->rows / 2
                                  : scrolllines == -2
                                        ? term->rows
                                        : scrolllines < -2 ? 3 : scrolllines;
            term_scroll(
  ...
  for (remaining = len; remaining > 0;
    ...
    if (maxlen == 1) {
      ...
      if (remaining - len >= 1 && IS_LOW_VARSEL(text[len]))
        len++;
      else if (remaining - len >= 2 && IS_HIGH_VARSEL(text[len], text[len + 1]))
        len += 2;
    }
...
#define SHOW_TOASCII_RESULT
  ...
  if ((HIWORD(lParam) & KF_ALTDOWN) && (conf_get_int(conf, CONF_rightaltkey) ||
                                        (keystate[VK_RMENU] & 0x80) == 0))
    left_alt = 1;
  ...
  if (left_alt ||
      (term->app_keypad_keys && !no_applic_k && funky_type != FUNKY_XTERM) ||
  ...
  if (key_down &&
      (conf_get_int(conf, CONF_rightaltkey) ||
       (keystate[VK_RMENU] & 0x80) == 0) &&
      !compose_state) {
    ...
    if (left_alt && wParam == VK_SPACE && conf_get_int(conf, CONF_alt_space)) {
    ...
    if (!left_alt) {
      ...
      if (funky_type == FUNKY_VT400 ||
          (funky_type <= FUNKY_LINUX && term->app_keypad_keys && !no_applic_k))
        switch (wParam) {
        case VK_EXECUTE:
          xkey = 'P';
          break;
    ...
    if (wParam == VK_BACK &&
        shift_state != 0) { /* Shift-Backspace, Ctrl-Backspace */
    ...
    if (wParam == VK_RETURN &&
        shift_state != 0) { /* Shift-Return, Ctrl-Return */
      /* send LINEFEED */
      *p++ = 012;
      return p - output;
    }
    ...
    if ((term->vt52_mode || funky_type == FUNKY_VT100P) && code >= 11 &&
        code <= 24) {
      int offt = 0;
      if (code > 15)
        offt++;
    ...
    if ((code == 1 || code == 4) && conf_get_int(conf, CONF_rxvt_homeend)) {
   ...
   * the boring stuff */
  {
    ...
    if (osVersion.dwPlatformId == VER_PLATFORM_WIN32_NT && p_ToUnicodeEx) {
      ...
      if (r > 0) {
        ...
        MultiByteToWideChar(
            CP_ACP, 0, (LPCSTR)keysb, r, keys_unicode, lenof(keys_unicode));
      }
    }
      ...
      for (i = 0; i < r; i++) {
        ...
        if (!key_down) {
          ...
          if (capsOn && wch < 0x80) {
            ...
            cbuf[0] = '\033';
            cbuf[1] =
                wch | ((left_alt & conf_get_int(conf, CONF_alt_metabit)) << 7);
            term_seen_key_event(term);
            if (ldisc)
              luni_send(
                  ldisc,
 ...
 * The new version with FlashWindowEx is nice but where do I trigger the icon
 * flash if I use it?
 */
static void flash_window(int mode)
{
...
void move_window(void *frontend, int x, int y)
{
  ...
  if (resize_action == RESIZE_DISABLED || resize_action == RESIZE_FONT ||
      IsZoomed(hwnd))
    return;
...
BOOL MakeWindowTransparent(HWND hWnd, int factor)
{
  // First, see if we can get the API call we need. If we've tried once, we
  ...
  if (factor < 255) {
    // Windows need to be layered to be made transparent. This is done by
    // modifying the extended style bits to contain WS_EX_LAYARED.
    SetLastError(0);
    SetWindowLong(
        hWnd, GWL_EXSTYLE, GetWindowLong(hWnd, GWL_EXSTYLE) | WS_EX_LAYERED);
    if (GetLastError()) {
      return FALSE;
    }

    // Now, we need to set the 'layered window attributes'. This is where the
    // alpha values get set.
    return pSetLayeredWindowAttributes(
        hWnd, RGB(255, 255, 255), factor, LWA_ALPHA);
