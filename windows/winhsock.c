...
static int handle_gotdata(struct handle *h, void *data, int len)
{
  ...
  if (len < 0) {
    return plug_closing(ps->plug, "Read error from handle", 0, 0);
  } else if (len == 0) {
...
static char *sk_handle_peer_info(Socket s)
{
  ...
  DECL_WINDOWS_FUNCTION(
      static, BOOL, GetNamedPipeClientProcessId, (HANDLE, PULONG));
  ...
  if (!kernel32_module) {
     ...
     * files to type-check */
    GET_WINDOWS_FUNCTION_NO_TYPECHECK(kernel32_module,
    ...
    HANDLE send_H, HANDLE recv_H, HANDLE stderr_H, Plug plug, int overlapped)
{
  ...
  if (ret->stderr_H)
    ret->stderr_h = handle_input_new(ret->stderr_H, handle_stderr, ret, flags);
