...
static DWORD WINAPI handle_input_threadfunc(void *param)
{
  ...
  while (1) {
    ...
    ctx->readret = ReadFile(ctx->h, ctx->buffer, readlen, &ctx->len, povl);
    ...
    if (ctx->readret && ctx->len == 0 && (ctx->flags & HANDLE_FLAG_IGNOREEOF))
      continue;
...
static DWORD WINAPI handle_output_threadfunc(void *param)
{
  ...
  while (1) {
    ...
    if (povl)
      memset(povl, 0, sizeof(OVERLAPPED));
    ctx->writeret =
        WriteFile(ctx->h, ctx->buffer, ctx->len, &ctx->lenwritten, povl);
    if (povl && !ctx->writeret && GetLastError() == ERROR_IO_PENDING)
      ctx->writeret = GetOverlappedResult(ctx->h, povl, &ctx->lenwritten, TRUE);
