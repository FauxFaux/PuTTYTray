...
DWORD WINAPI agent_query_thread(LPVOID param)
{
  ...
  id = SendMessage(data->hwnd, WM_COPYDATA, (WPARAM)NULL, (LPARAM)&data->cds);
  ret = NULL;
  if (id > 0) {
...
DECL_WINDOWS_FUNCTION(static, BOOL, OpenProcessToken, (HANDLE, DWORD, PHANDLE));
...
int init_advapi(void)
{
  ...
  return advapi && GET_WINDOWS_FUNCTION(advapi, GetSecurityInfo) &&
                ...
                void *callback_ctx)
{
  ...
  *outlen = 0;
  ...
  filemap = CreateFileMapping(
      INVALID_HANDLE_VALUE, psa, PAGE_READWRITE, 0, AGENT_MAX_MSGLEN, mapname);
