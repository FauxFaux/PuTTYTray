...
char *reg_read_setting_s(void *handle, const char *key)
{
  ...
  if (RegQueryValueEx((HKEY)handle, key, 0, &type, ret, &size) !=
          ERROR_SUCCESS ||
      type != REG_SZ) {
    sfree(ret);
    return NULL;
...
int reg_verify_host_key(const char *hostname,
                        int port,
                        const char *keytype,
                        const char *key)
{
  ...
  if (ret != ERROR_SUCCESS && ret != ERROR_MORE_DATA &&
    ...
    if (ret == ERROR_SUCCESS && type == REG_SZ) {
      ...
      if (!strcmp(otherstr, key))
        RegSetValueEx(rkey, regname, 0, REG_SZ, otherstr, strlen(otherstr) + 1);
    }
...
void reg_store_host_key(const char *hostname,
                        int port,
                        const char *keytype,
                        const char *key)
{
...
static DWORD errorShow(const char *pcErrText, const char *pcErrParam)
{
  ...
  char pcBuf[16];
  char *pcHlaska = snewn(
      (pcErrParam ? strlen(pcErrParam) : 0) + strlen(pcErrText) + 31, char);
  ...
  if (hwRodic != NULL) {
    hwRodic = GetLastActivePopup(hwRodic);
  }
  if (MessageBox(hwRodic,
                 pcHlaska,
                 "Error",
...
char *joinPath(char *pcDest, char *pcMain, char *pcSuf)
{
  ...
  if (0 == ExpandEnvironmentStrings(pcSuf, pcBuf, MAX_PATH)) {
    /* JK: failure -> revert back - but it ussualy won't work, so report error
     * to user! */
...
void *file_enum_settings_start(void)
{
  ...
  if (RegOpenKey(HKEY_CURRENT_USER, puttystr, &key) != ERROR_SUCCESS) {
     ...
     * file_enum_settings_next will solve this by starting scanning dir sesspath
     */
  }
  ret->key = key;
  ret->fromFile = 0;
...
char *file_enum_settings_next(void *handle, char *buffer, int buflen)
{
  ...
  if (!e->fromFile) {
    ...
    /* JK: skip directories (extra check for "." and ".." too, seems to bug on
     * some machines) */
    while ((FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ||
           FindFileData.cFileName[0] ==
               '.') { // HACK: PUTTY TRAY / PUTTY FILE: Fixed directory check
...
int file_verify_host_key(const char *hostname,
                         int port,
                         const char *keytype,
                         const char *key)
{
  ...
  if (ret == ERROR_MORE_DATA ||
    ...
    userMB = MessageBox(NULL,
                        "The host key is cached in the Windows registry. "
...
void file_store_host_key(const char *hostname,
                         int port,
                         const char *keytype,
                         const char *key)
{
...
static HANDLE access_random_seed(int action)
{
  ...
  if (RegOpenKey(HKEY_CURRENT_USER, PUTTY_REG_POS, &rkey) == ERROR_SUCCESS) {
    int ret = RegQueryValueEx(rkey, "RandSeedFile", 0, &type, seedpath, &size);
    if (ret != ERROR_SUCCESS || type != REG_SZ)
      seedpath[0] = '\0';
    RegCloseKey(rkey);
                                       ...
                                       char **out)
{
  ...
  ret = RegQueryValueEx(
      pjumplist_key, reg_jumplist_value, NULL, &type, old_value, &value_length);
...
STORAGE_TYPE_SWITCHER(char *,
                      read_setting_s,
                      CONCAT(void *handle, const char *key),
                      CONCAT(handle, key))

STORAGE_TYPE_SWITCHER(int,
                      read_setting_i,
...
STORAGE_TYPE_SWITCHER(void *, enum_settings_start, CONCAT(), CONCAT())