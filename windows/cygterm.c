...
static void cygterm_log(Plug p,
                        int type,
                        SockAddr addr,
                        int port,
...
static int cygterm_closing(Plug plug,
                           const char *error_msg,
                           int error_code,
                           int calling_back)
{
...
static int cygterm_receive(Plug plug, int urgent, char *data, int len)
{
  Local local = (Local)plug;
  int backlog;
...
static void cygterm_sent(Plug plug, int bufsize)
{
  Local local = (Local)plug;
  local->bufsize = bufsize;
}

static void cygterm_size(void *handle, int width, int height);

static int cygterm_accepting(Plug plug,
                             ...
                             accept_ctx_t ctx)
{
  ...
  /* Reset terminal size */
  cygterm_size(local,
               conf_get_int(local->conf, CONF_width),
               conf_get_int(local->conf, CONF_height));
  cygterm_debug("OK");
  return 0;
}
...
static const char *spawnChild(char *cmd,
                                ...
                                int keepalive)
{
   ...
   */
  static const struct plug_function_table fn_table = {cygterm_log,
                                                      cygterm_closing,
                                                      cygterm_receive,
                                                      cygterm_sent,
                                                      cygterm_accepting};
  ...
  *backend_handle = local;
  ...
  if (conf_get_int(conf, CONF_cygterm64)) {
    cmdlinelen = sprintf(cmdline,
                         CTHELPER64 " %u %s ",
                         cport,
                         conf_get_str(local->conf, CONF_termtype));
  } else {
    cmdlinelen = sprintf(cmdline,
                         CTHELPER " %u %s ",
                         cport,
...
static void cygterm_free(void *handle)
{
  Local local = handle;
  cygterm_debug("top");
  sfree(local);
...
static void cygterm_reconfig(void *handle, Conf *conf)
{
  Local local = handle;
  cygterm_debug("top");
  local->conf = conf;
...
static int cygterm_send(void *handle, char *buf, int len)
{
  Local local = handle;
...
static int cygterm_sendbuffer(void *handle)
{
  Local local = handle;
  cygterm_debug("top");
  return local->bufsize;
...
static int cygterm_connected(void *handle)
{
  Local local = handle;
  cygterm_debug("top");
  return local->s != NULL;
...
static int cygterm_exitcode(void *handle)
{
  Local local = handle;
  cygterm_debug("top");
...
static int cygterm_sendok(void *handle)
{
  cygterm_debug("top");
  return 1;
}

static void cygterm_unthrottle(void *handle, int backlog)
{
  Local local = handle;
  cygterm_debug("top");
  sk_set_frozen(local->s, backlog > CYGTERM_MAX_BACKLOG);
...
static int cygterm_ldisc(void *handle, int option)
{
  Local local = handle;
...
static void cygterm_provide_ldisc(void *handle, void *ldisc)
{
  cygterm_debug("top");
}

static void cygterm_provide_logctx(void *handle, void *logctx)
{
  cygterm_debug("top");
}

static int cygterm_cfg_info(void *handle)
{
  return 0;
}

Backend cygterm_backend = {cygterm_init,
                           cygterm_free,
                           cygterm_reconfig,
                           cygterm_send,
                           cygterm_sendbuffer,
...
static char *strecpy(char *d, const char *s)
{
  while ((*d = *s++))
    d++;
  return d;
...
static BOOL getRegistry(char *valueData,
                        LPDWORD psize,
                        HKEY key,
                        const char *subKey,
                        ...
                        int use64)
{
  ...
  if (ERROR_SUCCESS != (ret = RegOpenKeyEx(key,
                                           subKey,
                                           0,
                                           KEY_READ | (use64 ? KEY_WOW64_64KEY
                                                             : KEY_WOW64_32KEY),
                                           &k)))
    return ret;

  if (ERROR_SUCCESS ==
      (ret = RegQueryInfoKey(k, 0, 0, 0, 0, 0, 0, 0, 0, psize, 0, 0)))
...
#define CYGWIN_U_SETUP_ROOTDIR                                                 \
  HKEY_CURRENT_USER, "Software\\Cygwin\\setup", "rootdir"
...
static const char *spawnChild(char *cmd, LPPROCESS_INFORMATION ppi, PHANDLE pin)
{
  ...
  if (!CreateProcess(NULL,
                     cmd, /* command line */
                     NULL,
                     NULL, /* no process or thread security attributes */
