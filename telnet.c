...
static const struct Opt *const opts[] = {&o_naws,
                                         &o_tspeed,
                                         &o_ttype,
                                         &o_oenv,
                                         &o_nenv,
...
static void process_subneg(Telnet telnet)
{
  ...
  case TELOPT_TTYPE:
    if (telnet->sb_len == 1 && telnet->sb_buf[0] == TELQUAL_SEND) {
      ...
      for (n = 0; termtype[n]; n++)
        b[n + 4] =
            (termtype[n] >= 'a' && termtype[n] <= 'z' ? termtype[n] + 'A' - 'a'
                                                      : termtype[n]);
      b[n + 4] = IAC;
      b[n + 5] = SE;
  ...
  case TELOPT_NEW_ENVIRON:
    ...
    if (p < q && *p == TELQUAL_SEND) {
      ...
      bsize = 20;
      for (eval = conf_get_str_strs(telnet->conf, CONF_environmt, NULL, &ekey);
           eval != NULL;
           eval = conf_get_str_strs(telnet->conf, CONF_environmt, ekey, &ekey))
      ...
      for (eval = conf_get_str_strs(telnet->conf, CONF_environmt, NULL, &ekey);
           eval != NULL;
           eval =
               conf_get_str_strs(telnet->conf, CONF_environmt, ekey, &ekey)) {
        b[n++] = var;
        for (e = ekey; *e; e++)
          b[n++] = *e;
        b[n++] = value;
      ...
      if (n == 6) {
        logbuf =
            dupprintf("client:\tSB %s IS <nothing>", telopt(telnet->sb_opt));
        logevent(telnet->frontend, logbuf);
        sfree(logbuf);
      } else {
        logbuf = dupprintf("client:\tSB %s IS:", telopt(telnet->sb_opt));
        logevent(telnet->frontend, logbuf);
        sfree(logbuf);
        for (eval =
                 conf_get_str_strs(telnet->conf, CONF_environmt, NULL, &ekey);
             eval != NULL;
             eval =
                 conf_get_str_strs(telnet->conf, CONF_environmt, ekey, &ekey)) {
    ...
    case SUBNEGOT:
      ...
      subneg_addchar:
        if (telnet->sb_len >= telnet->sb_size) {
          ...
          telnet->sb_buf =
              sresize(telnet->sb_buf, telnet->sb_size, unsigned char);
        }
                               ...
                               int keepalive)
{
  static const struct plug_function_table fn_table = {
      telnet_log, telnet_closing, telnet_receive, telnet_sent, NULL};
  SockAddr addr;
  const char *err;
  Telnet telnet;
  char *loghost;
...
static int telnet_send(void *handle, char *buf, int len)
{
  ...
  while (p < end) {
    ...
    while (p < end && !iswritable(*p)) {
      telnet->bufsize = sk_write(telnet->s, (char *)(*p == IAC ? iac : cr), 2);
      p++;
    }
  }
...
static void telnet_size(void *handle, int width, int height)
{
  ...
  telnet->bufsize = sk_write(telnet->s, (char *)b, n);
  logbuf = dupprintf(
      "client:\tSB NAWS %d,%d", telnet->term_width, telnet->term_height);
  logevent(telnet->frontend, logbuf);
  sfree(logbuf);
}
...
{
  static const struct telnet_special specials[] = {{"Are You There", TS_AYT},
                                                   {"Break", TS_BRK},
                                                   {"Synch", TS_SYNCH},
