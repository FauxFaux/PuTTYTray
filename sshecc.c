...
static void initialise_wcurve(struct ec_curve *curve,
                              int bits,
...
static void initialise_mcurve(struct ec_curve *curve,
                              int bits,
                              const unsigned char *p,
                              const unsigned char *a,
                              const unsigned char *b,
...
static void initialise_ecurve(struct ec_curve *curve,
                              int bits,
                              const unsigned char *p,
                              const unsigned char *l,
                              const unsigned char *d,
                              const unsigned char *Bx,
...
static Bignum ecf_add(const Bignum a,
                      const Bignum b,
...
static int ec_point_verify(const struct ec_point *a)
{
  if (a->infinity) {
    ...
    Bignum lhs = NULL, x3 = NULL, ax = NULL, x3ax = NULL, x3axm = NULL,
           x3axb = NULL, rhs = NULL;
...
static struct ec_point *ecp_mul_(const struct ec_point *a,
                                 const Bignum b,
                                 int aminus3)
{
...
static struct ec_point *ecp_summul(const Bignum a,
                                   const Bignum b,
...
static void _ecdsa_sign(const Bignum privateKey,
                        const struct ec_curve *curve,
                        const unsigned char *data,
                        const int dataLen,
                        Bignum *r,
                        Bignum *s)
{
  ...
  *s = NULL;
  ...
  k = dss_gen_k("ECDSA deterministic k generator",
                curve->w.n,
                privateKey,
                digest,
                sizeof(digest));
                          ...
                          int len)
{
  ...
  if (curve->type == EC_WEIERSTRASS) {
    ...
    if (!p)
      return NULL;
    if (!match_ssh_id(slen, p, curve->name))
      return NULL;
  }
                             ...
                             int priv_len)
{
  ...
  if (!publicKey || bignum_cmp(publicKey->x, ec->publicKey.x) ||
...
static int ecdsa_openssh_fmtkey(void *key, unsigned char *blob, int len)
{
  ...
  pointlen = (bignum_bitcount(ec->publicKey.curve->p) + 7) / 8;
  namelen = strlen(ec->publicKey.curve->name);
  bloblen = 4 + namelen              /* <LEN> nistpXXX */
    ...
    void *key, const char *sig, int siglen, const char *data, int datalen)
{
  ...
  if (ec->publicKey.curve->type == EC_EDWARDS) {
    ...
    s = bignum_from_bytes_le((unsigned char *)p +
...
static unsigned char *ecdsa_sign(void *key,
                                 const char *data,
                                 int datalen,
                                 int *siglen)
{
...
static const unsigned char nistp384_oid[] = {0x2b, 0x81, 0x04, 0x00, 0x22};
...
static const unsigned char nistp521_oid[] = {0x2b, 0x81, 0x04, 0x00, 0x23};
...
const struct ssh_kexes ssh_ecdh_kex = {
    sizeof(ec_kex_list) / sizeof(*ec_kex_list), ec_kex_list};
