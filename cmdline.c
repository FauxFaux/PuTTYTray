...
#define SAVEABLE(pri)                                                          \
  do {                                                                         \
    if (need_save) {                                                           \
      cmdline_save_param(p, value, pri);                                       \
      return ret;                                                              \
    }                                                                          \
  } while (0)
...
#define UNAVAILABLE_IN(flag)                                                   \
  do {                                                                         \
    if (cmdline_check_unavailable(flag, p))                                    \
      return ret;                                                              \
  } while (0)
...
int cmdline_process_param(char *p, char *value, int need_save, Conf *conf)
{
  ...
  if (!strcmp(p, "-loadfile") || !strcmp(p, "-file") ||
      !strcmp(p, "-fileload")) {
    RETURN(2);
  ...
  if (!strcmp(p, "-log")) {
    ...
    if (conf_get_int(conf, CONF_logtype) ==
        LGTYP_NONE) // Force out type only if set to none
  ...
  if (!strcmp(p, "-sercfg")) {
    ...
    while (nextitem[0] != '\0') {
      ...
      if (length == 1) {
        ...
        default:
          cmdline_error("Unrecognised suboption \"-sercfg %c\"", *nextitem);
        }
        ...
        if (serspeed != 0) {
          ...
          cmdline_error("Unrecognised suboption \"-sercfg %s\"", nextitem);
        }
      }
  ...
  for (pri = 0; pri < NPRIORITIES; pri++)
    for (i = 0; i < saves[pri].nsaved; i++)
      cmdline_process_param(
          saves[pri].params[i].p, saves[pri].params[i].value, 0, conf);
}
