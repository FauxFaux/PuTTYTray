...
void conf_radiobutton_handler(union control *ctrl,
                              void *dlg,
                              void *data,
                              int event)
{
  ...
  if (event == EVENT_REFRESH) {
    ...
    conf_set_int(conf, ctrl->radio.context.i, ctrl->radio.buttondata[button].i);
  }
}

#define CHECKBOX_INVERT (1 << 30)
void conf_checkbox_handler(union control *ctrl,
                           void *dlg,
                           void *data,
                           int event)
{
...
void conf_editbox_handler(union control *ctrl, void *dlg, void *data, int event)
{
  /*
...
void conf_filesel_handler(union control *ctrl, void *dlg, void *data, int event)
{
...
void conf_fontsel_handler(union control *ctrl, void *dlg, void *data, int event)
{
...
static void config_host_handler(union control *ctrl,
                                void *dlg,
                                void *data,
                                int event)
{
...
static void config_port_handler(union control *ctrl,
                                void *dlg,
                                void *data,
                                int event)
{
 ...
 */
void config_protocolbuttons_handler(union control *ctrl,
                                    void *dlg,
                                    void *data,
                                    int event)
{
...
static void loggingbuttons_handler(union control *ctrl,
                                   void *dlg,
                                   void *data,
                                   int event)
{
...
static void numeric_keypad_handler(union control *ctrl,
                                   void *dlg,
                                   void *data,
                                   int event)
{
...
static void cipherlist_handler(union control *ctrl,
                               void *dlg,
                               void *data,
                               int event)
{
  ...
  if (event == EVENT_REFRESH) {
    ...
    static const struct {
      const char *s;
      int c;
    } ciphers[] = {{"ChaCha20 (SSH-2 only)", CIPHER_CHACHA20},
                   {"3DES", CIPHER_3DES},
    ...
    for (i = 0; i < CIPHER_MAX; i++)
      conf_set_int_int(
          conf, CONF_ssh_cipherlist, i, dlg_listbox_getid(ctrl, dlg, i));
  }
}

#ifndef NO_GSSAPI
static void gsslist_handler(union control *ctrl,
                            void *dlg,
                            void *data,
                            int event)
{
  ...
  if (event == EVENT_REFRESH) {
    ...
    for (i = 0; i < ngsslibs; i++)
      conf_set_int_int(
          conf, CONF_ssh_gsslist, i, dlg_listbox_getid(ctrl, dlg, i));
  }
}
#endif

static void kexlist_handler(union control *ctrl,
                            void *dlg,
                            void *data,
                            int event)
{
  ...
  if (event == EVENT_REFRESH) {
    ...
    static const struct {
      const char *s;
      int k;
    } kexes[] = {{"Diffie-Hellman group 1", KEX_DHGROUP1},
    ...
    for (i = 0; i < KEX_MAX; i++)
      conf_set_int_int(
          conf, CONF_ssh_kexlist, i, dlg_listbox_getid(ctrl, dlg, i));
  }
}

static void hklist_handler(union control *ctrl,
                           void *dlg,
                           void *data,
                           int event)
{
  ...
  if (event == EVENT_REFRESH) {
    ...
    for (i = 0; i < HK_MAX; i++)
      conf_set_int_int(
          conf, CONF_ssh_hklist, i, dlg_listbox_getid(ctrl, dlg, i));
  }
}

static void printerbox_handler(union control *ctrl,
                               void *dlg,
                               void *data,
                               int event)
{
...
static void codepage_handler(union control *ctrl,
                             void *dlg,
                             void *data,
                             int event)
{
  ...
  if (event == EVENT_REFRESH) {
    ...
    dlg_update_start(ctrl, dlg);
    thiscp = cp_name(decode_codepage(conf_get_str(conf, CONF_line_codepage)));
    dlg_listbox_clear(ctrl, dlg);
...
static void sshbug_handler(union control *ctrl,
                           void *dlg,
                           void *data,
                           int event)
{
...
static void sessionsaver_handler(union control *ctrl,
                                 void *dlg,
                                 void *data,
                                 int event)
{
  ...
  if (event == EVENT_REFRESH) {
    ...
    } else if (!ssd->midsession && ssd->delbutton && ctrl == ssd->delbutton) {
      int i = dlg_listbox_index(ssd->listbox, dlg);
      if (i <= 0) {
        dlg_beep(dlg);
      } else {
...
static void charclass_handler(union control *ctrl,
                              void *dlg,
                              void *data,
                              int event)
{
  Conf *conf = (Conf *)data;
  struct charclass_data *ccd = (struct charclass_data *)ctrl->generic.context.p;
...
static void colour_handler(union control *ctrl,
                           void *dlg,
                           void *data,
                           int event)
{
  Conf *conf = (Conf *)data;
  struct colour_data *cd = (struct colour_data *)ctrl->generic.context.p;
...
{
  Conf *conf = (Conf *)data;
  struct ttymodes_data *td = (struct ttymodes_data *)ctrl->generic.context.p;
  ...
  if (event == EVENT_REFRESH) {
    if (ctrl == td->listbox) {
      ...
      for (val = conf_get_str_strs(conf, CONF_ttymodes, NULL, &key);
        ...
        char *disp =
            dupprintf("%s\t%s", key, (val[0] == 'A') ? "(auto)" : val + 1);
...
static void environ_handler(union control *ctrl,
                            void *dlg,
                            void *data,
                            int event)
{
  Conf *conf = (Conf *)data;
  struct environ_data *ed = (struct environ_data *)ctrl->generic.context.p;
...
static void portfwd_handler(union control *ctrl,
                            void *dlg,
                            void *data,
                            int event)
{
  Conf *conf = (Conf *)data;
  struct portfwd_data *pfd = (struct portfwd_data *)ctrl->generic.context.p;
                      ...
                      int protcfginfo)
{
  ...
  ssd = (struct sessionsaver_data *)ctrl_alloc_with_free(
  ...
  ssd->cancelbutton = ctrl_pushbutton(
      s, "Cancel", 'c', HELPCTX(no_help), sessionsaver_handler, P(ssd));
  ...
  if (!midsession) {
    struct hostport *hp =
        (struct hostport *)ctrl_alloc(b, sizeof(struct hostport));
  ...
  if (!midsession) {
    ssd->loadbutton = ctrl_pushbutton(
        s, "Load", 'l', HELPCTX(session_saved), sessionsaver_handler, P(ssd));
  ...
  ssd->savebutton = ctrl_pushbutton(
      s, "Save", 'v', HELPCTX(session_saved), sessionsaver_handler, P(ssd));
  ssd->savebutton->generic.column = 1;
  if (!midsession) {
    ssd->delbutton = ctrl_pushbutton(
        s, "Delete", 'd', HELPCTX(session_saved), sessionsaver_handler, P(ssd));
   ...
   */
  ctrl_settitle(
      b, "Terminal/Keyboard", "Options controlling the effects of keys");

  s = ctrl_getset(
      b, "Terminal/Keyboard", "mappings", "Change the sequences sent by:");
  ctrl_radiobuttons(s,
                    "The Backspace key",
                    'b',
                    2,
  ...
  s = ctrl_getset(
      b, "Terminal/Keyboard", "appkeypad", "Application keypad settings:");
  ctrl_radiobuttons(s,
   ...
   */
  ctrl_settitle(b, "Terminal/Bell", "Options controlling the terminal bell");
  ...
  s = ctrl_getset(
      b, "Terminal/Bell", "overload", "Control the bell overload behaviour");
  ctrl_checkbox(s,
  ...
  s = ctrl_getset(
      b, "Window/Appearance", "cursor", "Adjust the use of the cursor");
  ctrl_radiobuttons(s,
                    "Cursor appearance:",
                    NO_SHORTCUT,
                    3,
  ...
  s = ctrl_getset(
      b, "Window/Appearance", "mouse", "Adjust the use of the mouse pointer");
  ctrl_checkbox(s,
  ...
  s = ctrl_getset(b, "Window/Appearance", "border", "Adjust the window border");
  ctrl_editbox(s,
   ...
   */
  ctrl_settitle(
      b, "Window/Translation", "Options controlling character set translation");

  s = ctrl_getset(
      b, "Window/Translation", "trans", "Character set translation");
  ctrl_combobox(s,
  ...
  s = ctrl_getset(b, "Window/Selection", "mouse", "Control use of mouse");
  ctrl_checkbox(s,
  ...
  ccd = (struct charclass_data *)ctrl_alloc(b, sizeof(struct charclass_data));
  ccd->listbox = ctrl_listbox(s,
                              "Character classes:",
                              'e',
  ...
  ccd->button = ctrl_pushbutton(
      s, "Set", 's', HELPCTX(selection_charclasses), charclass_handler, P(ccd));
  ...
  s = ctrl_getset(
      b, "Window/Colours", "general", "General options for colour usage");
  ctrl_checkbox(s,
  ...
  cd->button = ctrl_pushbutton(
      s, "Modify", 'm', HELPCTX(colours_config), colour_handler, P(cd));
  ...
  if (protocol >= 0) {
    ...
    if (!midsession) {
      s = ctrl_getset(
          b, "Connection", "tcp", "Low-level TCP connection options");
      ctrl_checkbox(s,
...
#ifndef NO_IPV6
      s = ctrl_getset(
          b, "Connection", "ipversion", "Internet protocol version");
      ctrl_radiobuttons(s,
                        NULL,
                        NO_SHORTCUT,
                        3,
...

      {
        ...
        s = ctrl_getset(
            b, "Connection", "identity", "Logical name of remote host");
        ctrl_editbox(s,
                     label,
                     'm',
                     100,
    ...
    if (!midsession) {
      ...
      s = ctrl_getset(b, "Connection/Data", "login", "Login details");
      ctrl_editbox(s,
                   "Auto-login username",
                   'u',
                   50,
                   ...
                   I(1));
      {
        ...
        char *userlabel =
            dupprintf("Use system username (%s)", user ? user : "");
        sfree(user);
        ctrl_radiobuttons(s,
      ...
      s = ctrl_getset(b, "Connection/Data", "term", "Terminal details");
      ctrl_editbox(s,
      ...
      s = ctrl_getset(b, "Connection/Data", "env", "Environment variables");
      ctrl_columns(s, 2, 80, 20);
      ed = (struct environ_data *)ctrl_alloc(b, sizeof(struct environ_data));
      ed->varbox = ctrl_editbox(s,
                                "Variable",
                                'v',
                                60,
      ...
      ed->addbutton = ctrl_pushbutton(
          s, "Add", 'd', HELPCTX(telnet_environ), environ_handler, P(ed));
      ed->addbutton->generic.column = 1;
      ed->rembutton = ctrl_pushbutton(
          s, "Remove", 'r', HELPCTX(telnet_environ), environ_handler, P(ed));
      ed->rembutton->generic.column = 1;
      ctrl_columns(s, 1, 100);
      ed->listbox = ctrl_listbox(s,
                                 NULL,
                                 NO_SHORTCUT,
                                 HELPCTX(telnet_environ),
                                 environ_handler,
  ...
  if (!midsession) {
     ...
     */
    ctrl_settitle(b, "Connection/Proxy", "Options controlling proxy usage");
  ...
  if (!midsession || protocol == PROT_TELNET) {
     ...
     */
    ctrl_settitle(
        b, "Connection/Telnet", "Options controlling Telnet connections");

    s = ctrl_getset(
        b, "Connection/Telnet", "protocol", "Telnet protocol adjustments");
  ...
  if (!midsession) {
     ...
     */
    ctrl_settitle(
        b, "Connection/Rlogin", "Options controlling Rlogin connections");
  ...
  if (backend_from_proto(PROT_SSH) && (!midsession || protocol == PROT_SSH)) {
    ...
    if (!midsession) {
      ...
      s = ctrl_getset(
          b, "Connection/SSH", "data", "Data to send to the server");
      ctrl_editbox(s,
                   "Remote command:",
                   'r',
                   100,
    ...
    if (protcfginfo != 1 && protcfginfo != -1) {
      ...
      s = ctrl_getset(
          b, "Connection/SSH/Kex", "main", "Key exchange algorithm options");
      c = ctrl_draglist(s,
    ...
    if (protcfginfo != 1 && protcfginfo != -1) {
      ctrl_settitle(
          b, "Connection/SSH/Host keys", "Options controlling SSH host keys");
    ...
    if (!midsession) {
      ...
      s = ctrl_getset(
          b, "Connection/SSH/Auth", "methods", "Authentication methods");
      ctrl_checkbox(s,
      ...
      s = ctrl_getset(
          b, "Connection/SSH/Auth", "params", "Authentication parameters");
      ctrl_checkbox(s,
    ...
    if (!midsession) {
      ...
      s = ctrl_getset(b, "Connection/SSH/TTY", "ttymodes", "Terminal modes");
      td = (struct ttymodes_data *)ctrl_alloc(b, sizeof(struct ttymodes_data));
      ctrl_columns(s, 2, 75, 25);
      c = ctrl_text(s, "Terminal modes to send:", HELPCTX(ssh_ttymodes));
      c->generic.column = 0;
      td->rembutton = ctrl_pushbutton(
          s, "Remove", 'r', HELPCTX(ssh_ttymodes), ttymodes_handler, P(td));
      td->rembutton->generic.column = 1;
      td->rembutton->generic.tabdelay = 1;
      ctrl_columns(s, 1, 100);
      td->listbox = ctrl_listbox(
          s, NULL, NO_SHORTCUT, HELPCTX(ssh_ttymodes), ttymodes_handler, P(td));
      ...
      ctrl_columns(s, 2, 75, 25);
      td->modelist = ctrl_droplist(
          s, "Mode:", 'm', 67, HELPCTX(ssh_ttymodes), ttymodes_handler, P(td));
      td->modelist->generic.column = 0;
      td->addbutton = ctrl_pushbutton(
          s, "Add", 'd', HELPCTX(ssh_ttymodes), ttymodes_handler, P(td));
      ...
      c->generic.column = 0;
      td->valradio = ctrl_radiobuttons(s,
                                       NULL,
                                       NO_SHORTCUT,
                                       2,
    ...
    if (!midsession) {
       ...
       */
      ctrl_settitle(
          b, "Connection/SSH/X11", "Options controlling SSH X11 forwarding");
     ...
     */
    ctrl_settitle(
        b, "Connection/SSH/Tunnels", "Options controlling SSH port forwarding");

    s = ctrl_getset(b, "Connection/SSH/Tunnels", "portfwd", "Port forwarding");
    ctrl_checkbox(s,
    ...
    pfd->rembutton = ctrl_pushbutton(s,
                                     "Remove",
                                     'r',
    ...
    pfd->addbutton = ctrl_pushbutton(
        s, "Add", 'd', HELPCTX(ssh_tunnels_portfwd), portfwd_handler, P(pfd));
    pfd->addbutton->generic.column = 2;
    pfd->addbutton->generic.tabdelay = 1;
    pfd->sourcebox = ctrl_editbox(s,
                                  "Source port",
                                  's',
                                  40,
                                  HELPCTX(ssh_tunnels_portfwd),
    ...
    pfd->destbox = ctrl_editbox(s,
                                ...
                                P(pfd),
                                P(NULL));
    pfd->direction = ctrl_radiobuttons(s,
                                       NULL,
                                       NO_SHORTCUT,
                                       3,
                                       HELPCTX(ssh_tunnels_portfwd),
    ...
    if (!midsession) {
      ...
      ctrl_droplist(s,
                    ...
                    'j',
                    20,
                    HELPCTX(ssh_bugs_winadj),
                    sshbug_handler,
                    I(CONF_sshbug_winadj));
      ctrl_droplist(s,
