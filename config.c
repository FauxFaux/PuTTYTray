                             ...
                             int event)
{
  ...
  if (event == EVENT_REFRESH) {
    ...
    if (ctrl == td->listbox) {
      ...
      val = conf_get_str_str(conf,
                             CONF_ttymodes,
                             conf_get_str_nthstrkey(conf, CONF_ttymodes, ind));
      assert(val != NULL);
      /* Do this first to defuse side-effects on radio buttons: */
      dlg_editbox_set(td->valbox, dlg, val + 1);
      dlg_radiobutton_set(
          td->valradio, dlg, val[0] == 'A' ? 0 : (val[0] == 'N' ? 1 : 2));
    }
  } else if (event == EVENT_VALCHANGE) {
    if (ctrl == td->valbox) {
    ...
    if (ctrl == td->setbutton) {
      ...
      if (ind >= 0) {
        ...
        str = (type == 'V' ? dlg_editbox_get(td->valbox, dlg) : dupstr(""));
                      ...
                      int protcfginfo)
{
  ...
  if (backend_from_proto(PROT_SSH) && (!midsession || protocol == PROT_SSH)) {
    ...
    if (!midsession) {
      ...
      c->generic.column = 0;
      td->setbutton = ctrl_pushbutton(
          s, "Set", 's', HELPCTX(ssh_ttymodes), ttymodes_handler, P(td));
