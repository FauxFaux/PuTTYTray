...
static int adb_closing(Plug plug,
                       const char *error_msg,
                       int error_code,
                       int calling_back)
{
 ...
 * freed by the caller.
 */
static const char *adb_init(void *frontend_handle,
                            void **backend_handle,
                            Conf *conf,
                            char *host,
                            int port,
                            ...
                            int keepalive)
{
  static const struct plug_function_table fn_table = {
      adb_log, adb_closing, adb_receive, adb_sent, NULL};
  SockAddr addr;
  const char *err;
  Adb adb;
  ...
  *backend_handle = adb;
   ...
   */
  {
    ...
    buf = dupprintf(
        "Looking up host \"%s\"%s",
        "localhost",
        (conf_get_int(conf, CONF_addressfamily) == ADDRTYPE_IPV4
             ? " (IPv4)"
             : (conf_get_int(conf, CONF_addressfamily) == ADDRTYPE_IPV6
                    ? " (IPv6)"
                    : "")));
    logevent(adb->frontend, buf);
    sfree(buf);
  }
  addr = name_lookup("localhost",
                     port,
                     realhost,
                     conf,
   ...
   * Open socket.
   */
  adb->s = new_connection(
      addr, *realhost, port, 0, 1, nodelay, keepalive, (Plug)adb, conf);
...
#define ADB_SHELL_HOST_MAX_LEN                                                 \
  (sizeof(sendbuf) - 4 - ADB_SHELL_SERIAL_PREFIX_LEN)
      if (len > ADB_SHELL_HOST_MAX_LEN)
        len = ADB_SHELL_HOST_MAX_LEN;
      sprintf(sendbuf,
              "%04lx" ADB_SHELL_SERIAL_PREFIX,
