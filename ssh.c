...
static void construct_packet(Ssh ssh, int pkttype, va_list ap1, va_list ap2)
{
  ...
  while ((argtype = va_arg(ap2, int)) != PKT_END) {
    ...
    if (len && (ssh->pktout_logmode != PKTLOG_EMIT)) {
      ssh->pktout_nblanks++;
      ssh->pktout_blanks =
          sresize(ssh->pktout_blanks, ssh->pktout_nblanks, struct logblank_t);
...
static void ssh2_pkt_adddata(Ssh ssh, void *data, int len)
{
  if (ssh->pktout_logmode != PKTLOG_EMIT) {
    ...
    ssh->pktout_blanks =
        sresize(ssh->pktout_blanks, ssh->pktout_nblanks, struct logblank_t);
    ssh->pktout_blanks[ssh->pktout_nblanks - 1].offset = ssh->pktout.length - 6;
    ssh->pktout_blanks[ssh->pktout_nblanks - 1].len = len;
    ssh->pktout_blanks[ssh->pktout_nblanks - 1].type = ssh->pktout_logmode;
  }
...
static int ssh1_pkt_getrsakey(Ssh ssh,
                              struct RSAKey *key,
      ...
      if (sport && dport) {
        ...
        if (type == 'L') {
          ...
          if (err) {
            ...
            logeventf(
                ssh, "Local port %s forwarding to %s", sportdesc, dportdesc);
          }
          sfree(dportdesc);
        } else if (type == 'D') {
          const char *err = pfd_addforward(
      ...
      if (ssh->pktin.type == SSH1_SMSG_STDOUT_DATA ||
        ...
        if (find234(ssh->rportfwds, &pf, NULL) == NULL) {
          ...
          sprintf(buf,
                  ...
                  port);
          logevent(buf);
          e = pfd_newconnect(&c->u.pfd.s, pf.dhost, port, c, &ssh->cfg);
          if (e != NULL) {
            char buf[256];
