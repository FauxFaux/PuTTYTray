...
static const struct {
  ...
  enum
  {
    ...
    {"INTR", 1, TTY_OP_CHAR},     {"QUIT", 2, TTY_OP_CHAR},
    {"ERASE", 3, TTY_OP_CHAR},    {"KILL", 4, TTY_OP_CHAR},
    {"EOF", 5, TTY_OP_CHAR},      {"EOL", 6, TTY_OP_CHAR},
    {"EOL2", 7, TTY_OP_CHAR},     {"START", 8, TTY_OP_CHAR},
    {"STOP", 9, TTY_OP_CHAR},     {"SUSP", 10, TTY_OP_CHAR},
    {"DSUSP", 11, TTY_OP_CHAR},   {"REPRINT", 12, TTY_OP_CHAR},
    {"WERASE", 13, TTY_OP_CHAR},  {"LNEXT", 14, TTY_OP_CHAR},
    {"FLUSH", 15, TTY_OP_CHAR},   {"SWTCH", 16, TTY_OP_CHAR},
    {"STATUS", 17, TTY_OP_CHAR},  {"DISCARD", 18, TTY_OP_CHAR},
    {"IGNPAR", 30, TTY_OP_BOOL},  {"PARMRK", 31, TTY_OP_BOOL},
    {"INPCK", 32, TTY_OP_BOOL},   {"ISTRIP", 33, TTY_OP_BOOL},
    {"INLCR", 34, TTY_OP_BOOL},   {"IGNCR", 35, TTY_OP_BOOL},
    {"ICRNL", 36, TTY_OP_BOOL},   {"IUCLC", 37, TTY_OP_BOOL},
    {"IXON", 38, TTY_OP_BOOL},    {"IXANY", 39, TTY_OP_BOOL},
    {"IXOFF", 40, TTY_OP_BOOL},   {"IMAXBEL", 41, TTY_OP_BOOL},
    {"ISIG", 50, TTY_OP_BOOL},    {"ICANON", 51, TTY_OP_BOOL},
    {"XCASE", 52, TTY_OP_BOOL},   {"ECHO", 53, TTY_OP_BOOL},
    {"ECHOE", 54, TTY_OP_BOOL},   {"ECHOK", 55, TTY_OP_BOOL},
    {"ECHONL", 56, TTY_OP_BOOL},  {"NOFLSH", 57, TTY_OP_BOOL},
    {"TOSTOP", 58, TTY_OP_BOOL},  {"IEXTEN", 59, TTY_OP_BOOL},
    {"ECHOCTL", 60, TTY_OP_BOOL}, {"ECHOKE", 61, TTY_OP_BOOL},
    {"PENDIN", 62, TTY_OP_BOOL}, /* XXX is this a real mode? */
    {"OPOST", 70, TTY_OP_BOOL},   {"OLCUC", 71, TTY_OP_BOOL},
    {"ONLCR", 72, TTY_OP_BOOL},   {"OCRNL", 73, TTY_OP_BOOL},
    {"ONOCR", 74, TTY_OP_BOOL},   {"ONLRET", 75, TTY_OP_BOOL},
    {"CS7", 90, TTY_OP_BOOL},     {"CS8", 91, TTY_OP_BOOL},
...
{
  if (_stricmp(s, "yes") == 0 || _stricmp(s, "on") == 0 ||
      _stricmp(s, "true") == 0 || _stricmp(s, "+") == 0)
    return 1; /* true */
  else if (_stricmp(s, "no") == 0 || _stricmp(s, "off") == 0 ||
           _stricmp(s, "false") == 0 || _stricmp(s, "-") == 0)
    return 0; /* false */
  else
    return (atoi(s) != 0);
}
...
static void ssh_disconnect(
    Ssh ssh, char *client_reason, char *wire_reason, int code, int clean_exit)
{
  char *error;
  if (!client_reason)
  ...
  if (!filename_is_null(s->keyfile)) {
    ...
    if (keytype == SSH_KEYTYPE_SSH1) {
      ...
      if (rsakey_pubblob(s->keyfile,
                         ...
                         &s->publickey_comment,
                         &error)) {
        s->publickey_encrypted = rsakey_encrypted(s->keyfile, NULL);
      } else {
        char *msgbuf;
...
void sshfwd_unclean_close(struct ssh_channel *c, const char *err)
{
  ...
  case CHAN_X11:
    x11_close(c->u.x11.xconn);
    logeventf(ssh,
              "Forwarded X11 connection terminated due to local "
              "error: %s",
              err);
    break;
  case CHAN_SOCKDATA:
    ...
    case CHAN_AGENT:
      ...
      while (len > 0) {
        ...
        if (c->u.a.lensofar >= 4 && len > 0) {
          unsigned int l =
              min(c->u.a.totallen - c->u.a.lensofar, (unsigned)len);
...
static void ssh1_send_ttymode(void *data, char *mode, char *val)
{
  ...
  int i = 0;
  unsigned int arg = 0;
  while (strcmp(mode, ssh_ttymodes[i].mode) != 0)
    i++;
  if (i == lenof(ssh_ttymodes))
    return;
...
static void ssh2_mkkey(
    Ssh ssh, Bignum K, unsigned char *H, char chr, unsigned char *keyspace)
{
...
begin_key_exchange:
  ...
  if (!s->hkey || !ssh->hostkey->verifysig(s->hkey,
                                           s->sigdata,
                                           s->siglen,
...
static struct ssh_channel *ssh2_channel_msg(Ssh ssh, struct Packet *pktin)
{
  ...
  if (!c || (c->type != CHAN_SHARING && c->halfopen &&
...
static void ssh2_msg_channel_data(Ssh ssh, struct Packet *pktin)
{
  ...
  if (data) {
    ...
    case CHAN_AGENT:
      while (length > 0) {
        ...
        if (c->u.a.lensofar >= 4 && length > 0) {
          unsigned int l =
              min(c->u.a.totallen - c->u.a.lensofar, (unsigned)length);
     ...
     */
    ssh2_set_window(
        c, bufsize < c->v.v2.locmaxwin ? c->v.v2.locmaxwin - bufsize : 0);
    /*
                             ...
                             struct Packet *pktin)
{
  ...
  if (!s->we_are_in) {
    ...
    if (!filename_is_null(s->keyfile)) {
      ...
      if (keytype == SSH_KEYTYPE_SSH2) {
        ...
        s->publickey_blob = ssh2_userkey_loadpub(s->keyfile,
                                                 ...
                                                 &error);
        if (s->publickey_blob) {
          s->publickey_encrypted = ssh2_userkey_encrypted(s->keyfile, NULL);
        } else {
          char *msgbuf;
          logeventf(ssh, "Unable to load private key (%s)", error);
          msgbuf = dupprintf("Unable to load private key file "
    ...
    if (conf_get_int(ssh->conf, CONF_tryagent) && agent_exists()) {
      ...
      if (!agent_query(s->agent_request,
                       5,
                       &r,
                       &s->agent_responselen,
                       ssh_agent_callback,
...
static const struct telnet_special *ssh_get_specials(void *handle)
{
  ...
  static struct telnet_special
      ssh_specials[lenof(ssh2_ignore_special) + lenof(ssh2_rekey_special) +
