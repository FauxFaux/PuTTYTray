...
static struct Packet *ssh2_rdpkt(Ssh ssh, unsigned char **data, int *datalen)
{
  ...
  if (ssh->logctx) {
    ...
    log_packet(ssh->logctx,
               PKT_INCOMING,
               st->pktin->type,
               ssh2_pkt_type(ssh->pkt_kctx, ssh->pkt_actx, st->pktin->type),
               st->pktin->data + 6,
               st->pktin->length - 6,
               nblanks,
               &blank,
...
static void ssh_pkt_adddata(struct Packet *pkt, const void *data, int len)
{
  if (pkt->logmode != PKTLOG_EMIT) {
    ...
    assert(pkt->body);
    pkt->blanks[pkt->nblanks - 1].offset =
        pkt->length - (pkt->body - pkt->data);
...
static void ssh_gotdata(Ssh ssh, unsigned char *data, int datalen)
{
  ...
  if (ssh->logctx)
    log_packet(
        ssh->logctx, PKT_INCOMING, -1, NULL, data, datalen, 0, NULL, NULL);
...
static void ssh1_smsg_x11_open(Ssh ssh, struct Packet *pktin)
{
  ...
  if (!ssh->X11_fwd_enabled) {
    ...
    if (x11_init(&c->u.x11.s, ssh->x11disp, c, NULL, -1, ssh->conf) != NULL) {
...
static void ssh1_msg_port_open(Ssh ssh, struct Packet *pktin)
{
  ...
  if (pfp == NULL) {
    ...
    e = pfd_newconnect(
        &c->u.pfd.s, pf.dhost, port, c, ssh->conf, pfp->pfrec->addressfamily);
    if (e != NULL) {
  ...
  if (conf_get_int(ssh->conf, CONF_x11_forward) &&
    ...
    if (ssh->v1_local_protoflags & SSH1_PROTOFLAG_SCREEN_NUMBER) {
      send_packet(ssh,
                  ...
                  PKT_STR,
                  ssh->x11disp->remoteauthprotoname,
                  PKTT_PASSWORD,
                  PKT_STR,
                  ssh->x11disp->remoteauthdatastring,
                  PKTT_OTHER,
                  PKT_INT,
...
begin_key_exchange:
   ...
   */
  {
    ...
    if (pktin->length > 5)
      hash_string(
          ssh->kex->hash, ssh->exhash, pktin->data + 5, pktin->length - 5);
  ...
  ssh_set_frozen(ssh, 0);
  if (s->dlgret == 0) {
    ssh_disconnect(ssh, "User aborted at host key verification", NULL, 0, TRUE);
    crStopV;
  }
...
static struct ssh_channel *ssh2_channel_msg(Ssh ssh, struct Packet *pktin)
{
  ...
  if (!c || (c->halfopen && pktin->type != SSH2_MSG_CHANNEL_OPEN_CONFIRMATION &&
...
static void ssh2_msg_channel_open(Ssh ssh, struct Packet *pktin)
{
  ...
  if (typelen == 3 && !memcmp(type, "x11", 3)) {
    ...
    else if ((x11err = x11_init(&c->u.x11.s,
                                ssh->x11disp,
                                c,
                                addrstr,
                                peerport,
