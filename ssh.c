...
static unsigned int ssh_tty_parse_boolean(char *s)
{
  if (stricmp(s, "yes") == 0 || stricmp(s, "on") == 0 ||
      stricmp(s, "true") == 0 || stricmp(s, "+") == 0)
    return 1; /* true */
  else if (stricmp(s, "no") == 0 || stricmp(s, "off") == 0 ||
           stricmp(s, "false") == 0 || stricmp(s, "-") == 0)
    return 0; /* false */
  else
    return (atoi(s) != 0);
}
...
#define translate(x)                                                           \
  if (type == x)                                                               \
  return #x
#define translatek(x, ctx)                                                     \
  if (type == x && (pkt_kctx == ctx))                                          \
  return #x
#define translatea(x, ctx)                                                     \
  if (type == x && (pkt_actx == ctx))                                          \
  return #x
...
static const char *ssh2_pkt_type(Pkt_KCtx pkt_kctx, Pkt_ACtx pkt_actx, int type)
{
...
const static struct ssh_compress *const compressions[] = {&ssh_zlib,
                                                          &ssh_comp_none};
...
static void ssh1_protocol(Ssh ssh,
                          const void *vin,
                          int inlen,
                          struct Packet *pktin);
static void ssh2_protocol(Ssh ssh,
                          const void *vin,
                          int inlen,
                          struct Packet *pktin);
static void ssh2_bare_connection_protocol(Ssh ssh,
                                          const void *vin,
                                          int inlen,
                                          struct Packet *pktin);
...
static void do_ssh2_transport(Ssh ssh,
                              const void *vin,
                              int inlen,
                              struct Packet *pktin);
...
struct ssh_tag {
  ...
  void (*protocol)(Ssh ssh, const void *vin, int inlen, struct Packet *pkt);
  struct Packet *(*s_rdpkt)(Ssh ssh, const unsigned char **data, int *datalen);
...
static void ssh1_log_incoming_packet(Ssh ssh, struct Packet *pkt)
{
  ...
  if (ssh->logomitdata && (pkt->type == SSH1_SMSG_STDOUT_DATA ||
...
static void ssh1_log_outgoing_packet(Ssh ssh, struct Packet *pkt)
{
  ...
  if (ssh->logomitdata && (pkt->type == SSH1_CMSG_STDIN_DATA ||
                           pkt->type == SSH1_MSG_CHANNEL_DATA)) {
    /* "Session data" packets - omit the data string. */
    if (pkt->type == SSH1_MSG_CHANNEL_DATA)
      ssh_pkt_getuint32(pkt); /* skip channel id */
 ...
 */
static struct Packet *ssh1_rdpkt(Ssh ssh,
...
static void ssh2_log_incoming_packet(Ssh ssh, struct Packet *pkt)
{
  ...
  if (ssh->logomitdata && (pkt->type == SSH2_MSG_CHANNEL_DATA ||
  ...
  if (pkt->type == SSH2_MSG_USERAUTH_REQUEST &&
    ...
    if (slen == 8 && !memcmp(str, "password", 8)) {
      ...
      if (str) {
        ...
        ssh_pkt_getstring(pkt, &str, &slen);
        if (str)
          blanks[nblanks - 1].len = pkt->savedpos - blanks[nblanks].offset;
      }
    }
...
static struct Packet *ssh2_rdpkt(Ssh ssh,
                                 const unsigned char **data,
                                 int *datalen)
{
  ...
  if (ssh->sccipher && (ssh->sccipher->flags & SSH_CIPHER_IS_CBC) &&
    ...
    st->pktin->data =
        snewn(OUR_V2_PACKETLIMIT + st->maclen + APIEXTRA, unsigned char);
    ...
    for (;;) { /* Once around this loop per cipher block. */
      ...
      ssh->sccipher->decrypt(
          ssh->sc_cipher_ctx, st->pktin->data + st->packetlen, st->cipherblk);
      /* Feed that block to the MAC. */
      ssh->scmac->bytes(
          ssh->sc_mac_ctx, st->pktin->data + st->packetlen, st->cipherblk);
    ...
    if (ssh->sccipher && (ssh->sccipher->flags & SSH_CIPHER_SEPARATE_LENGTH)) {
      ...
      ssh->sccipher->decrypt_length(
          ssh->sc_cipher_ctx, len, 4, st->incoming_sequence);
    ...
    if (ssh->sccipher)
      ssh->sccipher->decrypt(
          ssh->sc_cipher_ctx, st->pktin->data + 4, st->packetlen - 4);
  } else {
    ...
    if (ssh->sccipher)
      ssh->sccipher->decrypt(
          ssh->sc_cipher_ctx, st->pktin->data, st->cipherblk);
...
static int s_wrpkt_prepare(Ssh ssh, struct Packet *pkt, int *offset_p)
{
  ...
  if (ssh->cipher)
    ssh->cipher->encrypt(ssh->v1_cipher_ctx, pkt->data + pktoffs + 4, biglen);
...
static int s_write(Ssh ssh, void *data, int len)
{
  if (ssh->logctx)
    log_packet(
        ssh->logctx, PKT_OUTGOING, -1, NULL, data, len, 0, NULL, NULL, 0, NULL);
  if (!ssh->s)
    return 0;
...
static void s_wrpkt_defer(Ssh ssh, struct Packet *pkt)
{
  ...
  memcpy(ssh->deferred_send_data + ssh->deferred_len, pkt->data + offset, len);
...
static void ssh_pkt_addstring_data(struct Packet *pkt,
                                   const char *data,
                                   int len)
{
...
#define ssh2_pkt_addstring_data(pkt, data, len)                                \
  ssh_pkt_addstring_data(pkt, data, len)
...
static int ssh2_pkt_construct(Ssh ssh, struct Packet *pkt)
{
  ...
  if (ssh->cscipher && (ssh->cscipher->flags & SSH_CIPHER_SEPARATE_LENGTH)) {
    ssh->cscipher->encrypt_length(
        ssh->cs_cipher_ctx, pkt->data, 4, ssh->v2_outgoing_sequence);
  }
  ...
  if (ssh->csmac && ssh->csmac_etm) {
    ...
    if (ssh->cscipher)
      ssh->cscipher->encrypt(
          ssh->cs_cipher_ctx, pkt->data + 4, pkt->length + padding - 4);
...
static void ssh2_pkt_send_noqueue(Ssh ssh, struct Packet *pkt)
{
  ...
  if (!ssh->kex_in_progress && !ssh->bare_connection &&
      ssh->max_data_size != 0 && ssh->outgoing_data_size > ssh->max_data_size)
 ...
 */
static void ssh2_pkt_send_with_padding(Ssh ssh, struct Packet *pkt, int padsize)
{
#if 0
    if (0) {
	/*
...
static void ssh_detect_bugs(Ssh ssh, char *vstring)
{
  ...
  if (conf_get_int(ssh->conf, CONF_sshbug_rekey2) == FORCE_ON ||
      (conf_get_int(ssh->conf, CONF_sshbug_rekey2) == AUTO &&
       (wc_match("DigiSSH_2.0", imp) || wc_match("OpenSSH_2.[0-4]*", imp) ||
  ...
  if (conf_get_int(ssh->conf, CONF_sshbug_chanreq) == FORCE_ON ||
      (conf_get_int(ssh->conf, CONF_sshbug_chanreq) == AUTO &&
       (wc_match("OpenSSH_[2-5].*", imp) || wc_match("OpenSSH_6.[0-6]*", imp) ||
...
static void ssh_send_verstring(Ssh ssh, const char *protoname, char *svers)
{
  ...
  if (ssh->version == 2) {
    ...
    verstring = dupprintf("SSH-%s-%s\012",
                          (ssh_versioncmp(svers, "1.5") <= 0 ? svers : "1.5"),
                          sshver);
  }
  ...
  logeventf(
      ssh, "We claim version: %.*s", strcspn(verstring, "\015\012"), verstring);
...
static int do_ssh_connection_init(Ssh ssh, unsigned char c)
{
   ...
   * extension space.
   */
  static const char protoname[] = "SSHCONNECTION@putty.projects.tartarus.org-";
  ...
  if (ssh_versioncmp(s->version, "2.0") < 0) {
    bombout(("Server announces compatibility with SSH-1 in bare ssh-connection "
             "protocol"));
    crStop(0);
  }
  if (conf_get_int(ssh->conf, CONF_sshprot) == 0) {
...
static void ssh_socket_log(Plug plug,
                           int type,
                           SockAddr addr,
                           int port,
                           ...
                           int error_code)
{
  ...
  if (!ssh->attempting_connshare)
    backend_socket_log(ssh->frontend,
                       type,
                       addr,
                       port,
                       error_msg,
                       ...
                       const char *us_err)
{
  if (event == SHARE_NONE) {
    ...
    if (logtext) {
      ...
      if (ds_err)
        logeventf(ssh,
                  "Could not set up connection sharing"
                  " as downstream: %s",
                  ds_err);
      if (us_err)
        logeventf(ssh,
...
static void ssh_hostport_setup(const char *host,
                               int port,
                               Conf *conf,
                               char **savedhost,
                               int *savedport,
 ...
 * freed by the caller.
 */
static const char *connect_to_host(Ssh ssh,
                                   const char *host,
                                   int port,
                                   char **realhost,
                                   int nodelay,
                                   int keepalive)
{
  static const struct plug_function_table fn_table = {
      ssh_socket_log, ssh_closing, ssh_receive, ssh_sent, NULL};
  ...
  ssh_hostport_setup(
      host, port, ssh->conf, &ssh->savedhost, &ssh->savedport, &loghost);
  ...
  ssh->s = ssh_connection_sharing_init(
      ssh->savedhost, ssh->savedport, ssh->conf, ssh, &ssh->connshare);
  ...
  if (ssh->s != NULL) {
    ...
    *realhost = dupstr(host); /* best we can do */
    ...
    ssh->s = new_connection(
        addr, *realhost, port, 0, 1, nodelay, keepalive, (Plug)ssh, ssh->conf);
...
static void ssh_agentf_try_forward(struct ssh_channel *c)
{
   ...
   */
  if (c->ssh->throttled_all || (c->ssh->version == 2 && c->v.v2.remwindow == 0))
    return;
 ...
 * => log `wire_reason'.
 */
static void ssh_disconnect(Ssh ssh,
                           ...
                           int clean_exit)
{
  ...
  if (wire_reason) {
    if (ssh->version == 1) {
      send_packet(ssh, SSH1_MSG_DISCONNECT, PKT_STR, wire_reason, PKT_END);
...
int verify_ssh_manual_host_key(Ssh ssh,
                               ...
                               void *ssh2keydata)
{
  ...
  if (fingerprint) {
    ...
    if (conf_get_str_str_opt(ssh->conf, CONF_ssh_manual_hostkeys, fingerprint))
      return 1; /* success */
  }
  ...
  if (ssh2keydata) {
    ...
    sfree(binblob);
    if (conf_get_str_str_opt(ssh->conf, CONF_ssh_manual_hostkeys, base64blob)) {
      sfree(base64blob);
      return 1; /* success */
    }
    sfree(base64blob);
                         ...
                         struct Packet *pktin)
{
   ...
   */
  {
    ...
    rsa_fingerprint(
        logmsg + strlen(logmsg), sizeof(logmsg) - strlen(logmsg), &s->hostkey);
    logevent(logmsg);
  }
  ...
  s->len = (s->hostkey.bytes > s->servkey.bytes ? s->hostkey.bytes
                                                : s->servkey.bytes);
   ...
   */
  {
    ...
    if (s->dlgret == 0) { /* did not match */
      ...
      if (s->dlgret == 0) {
        ssh_disconnect(
            ssh, "User aborted at host key verification", NULL, 0, TRUE);
        crStop(0);
      }
    } else {
      sfree(keystr);
...

  {
    ...
    for (i = 0; !cipher_chosen && i < CIPHER_MAX; i++) {
      int next_cipher = conf_get_int_int(ssh->conf, CONF_ssh_cipherlist, i);
    ...
    if (warn) {
      ...
      ssh_set_frozen(ssh, 0);
      if (s->dlgret == 0) {
        ssh_disconnect(ssh, "User aborted at cipher warning", NULL, 0, TRUE);
        crStop(0);
      }
    }
  }
  ...
  if (!filename_is_null(s->keyfile)) {
    int keytype;
    logeventf(ssh, "Reading key file \"%.150s\"", filename_to_str(s->keyfile));
    ...
    if (keytype == SSH_KEYTYPE_SSH1 || keytype == SSH_KEYTYPE_SSH1_PUBLIC) {
      ...
      if (rsakey_pubblob(s->keyfile,
        ...
        s->privatekey_encrypted = rsakey_encrypted(s->keyfile, NULL);
      } else {
        char *msgbuf;
  ...
  while (pktin->type == SSH1_SMSG_FAILURE) {
    ...
    if (conf_get_int(ssh->conf, CONF_tryagent) && agent_exists() &&
      ...
      if (s->response && s->responselen >= 5 &&
        ...
        if (s->nkeys < 0) {
          logeventf(ssh, "Pageant reported negative key count %d", s->nkeys);
          s->nkeys = 0;
        }
        s->p += 4;
        ...
        for (s->keyi = 0; s->keyi < s->nkeys; s->keyi++) {
          ...
          s->p += 4;
          {
            ...
            do { /* do while (0) to make breaking easy */
              ...
              s->p += 4;
              if (s->commentlen < 0 ||
                  toint(s->responselen - (s->p - s->response)) < s->commentlen)
                break;
              s->commentp = (char *)s->p;
              s->p += s->commentlen;
              ok = TRUE;
          ...
          if (s->publickey_blob) {
            if (!memcmp(pkblob, s->publickey_blob, s->publickey_bloblen)) {
              logeventf(ssh,
...

          {
            ...
            *q++ = SSH1_AGENTC_RSA_CHALLENGE;
            ...
            ssh->auth_agent_query = agent_query(
                agentreq, len + 4, &vret, &retlen, ssh_agent_callback, ssh);
            if (ssh->auth_agent_query) {
              sfree(agentreq);
              do {
                crReturn(0);
    ...
    if (s->publickey_blob && s->privatekey_available && !s->tried_publickey) {
      /*
      ...
      s->keyfile = conf_get_filename(ssh->conf, CONF_keyfile);
      logeventf(ssh, "Trying public key \"%s\"", filename_to_str(s->keyfile));
      s->tried_publickey = 1;
      ...
      while (!got_passphrase) {
         ...
         */
        s->keyfile = conf_get_filename(ssh->conf, CONF_keyfile);
        ret = loadrsakey(s->keyfile, &s->key, passphrase, &error);
        if (passphrase) {
    ...
    if (conf_get_int(ssh->conf, CONF_try_tis_auth) &&
      ...
      if (pktin->type != SSH1_SMSG_AUTH_TIS_CHALLENGE) {
        ...
        s->cur_prompt->instruction = dupprintf("Using TIS authentication.%s%s",
                                               (*instr_suf) ? "\n" : "",
                                               instr_suf);
    ...
    if (s->pwpkt_type == SSH1_CMSG_AUTH_PASSWORD) {
      ...
      add_prompt(s->cur_prompt,
                 dupprintf("%s@%s's password: ", ssh->username, ssh->savedhost),
                 FALSE);
    }
    ...
    if (s->pwpkt_type == SSH1_CMSG_AUTH_PASSWORD) {
      ...
      if (!(ssh->remote_bugs & BUG_CHOKES_ON_SSH1_IGNORE) &&
        ...
        send_packet(
            ssh, s->pwpkt_type, PKT_INT, len, PKT_DATA, ss, len, PKT_END);
      } else {
        /*
...
static void ssh_sharing_global_request_response(Ssh ssh,
                                                struct Packet *pktin,
                                                void *ctx)
{
  share_got_pkt_from_server(ctx, pktin->type, pktin->body, pktin->length);
}
...
static void ssh1_msg_port_open(Ssh ssh, struct Packet *pktin)
{
  ...
  if (pfp == NULL) {
    ...
    err = pfd_connect(
        &c->u.pfd.pf, pf.dhost, port, c, ssh->conf, pfp->pfrec->addressfamily);
    if (err != NULL) {
 ...
 */
static int ssh_agent_channel_data(struct ssh_channel *c, char *data, int length)
{
...
static int ssh_channel_data(struct ssh_channel *c,
                            int is_stderr,
                            char *data,
                            int length)
{
                               ...
                               struct Packet *pktin)
{
  ...
  if (conf_get_int(ssh->conf, CONF_x11_forward)) {
    ssh->x11disp =
        x11_setup_display(conf_get_str(ssh->conf, CONF_x11_display), ssh->conf);
    if (!ssh->x11disp) {
      ...
      if (ssh->v1_local_protoflags & SSH1_PROTOFLAG_SCREEN_NUMBER) {
        send_packet(ssh,
                    ...
                    PKT_STR,
                    ssh->x11auth->protoname,
                    PKT_STR,
                    ssh->x11auth->datastring,
                    PKT_INT,
                    ssh->x11disp->screennum,
                    PKT_END);
      } else {
        send_packet(ssh,
                    SSH1_CMSG_X11_REQUEST_FORWARDING,
  ...
  if (!conf_get_int(ssh->conf, CONF_nopty)) {
    ...
    sscanf(conf_get_str(ssh->conf, CONF_termspeed),
           "%d,%d",
           &ssh->ospeed,
           &ssh->ispeed);
 ...
 */
static int first_in_commasep_string(char const *needle,
                                    char const *haystack,
                                    int haylen)
{
  int needlen;
...
static int in_commasep_string(char const *needle,
                              char const *haystack,
                              int haylen)
{
  char *p;
 ...
 */
static unsigned char *ssh2_mkkey(
    Ssh ssh, Bignum K, unsigned char *H, char chr, int keylen)
{
                              ...
                              struct Packet *pktin)
{
  ...
  const char *kexlist_descr[NKEXLIST] = {"key exchange algorithm",
                                         "host key algorithm",
...
begin_key_exchange:
  ssh->pkt_kctx = SSH2_PKTCTX_NOKEX;
  {
    ...
    for (i = 0; i < KEX_MAX; i++) {
      ...
      case KEX_RSA:
        ...
        break;
      case KEX_ECDH:
        s->preferred_kex[s->n_preferred_kex++] = &ssh_ecdh_kex;
        break;
      case KEX_WARN:
    ...
    for (i = 0; i < s->n_preferred_kex; i++) {
      ...
      if (!k)
        ...
        for (j = 0; j < k->nkexes; j++) {
          alg = ssh2_kexinit_addalg(s->kexlists[KEXLIST_KEX], k->list[j]->name);
    ...
    if (!s->got_session_id) {
      ...
      for (i = 0; i < s->n_preferred_hk; i++) {
        ...
        for (j = 0; j < lenof(hostkey_algs); j++) {
          if (hostkey_algs[j].id != s->preferred_hk[i])
            continue;
          if (have_ssh_host_key(ssh->savedhost,
                                ssh->savedport,
      ...
      assert(ssh->kex);
      alg =
          ssh2_kexinit_addalg(s->kexlists[KEXLIST_HOSTKEY], ssh->hostkey->name);
      ...
      for (i = 0; i < s->n_preferred_ciphers; i++) {
        ...
        if (!c)
          ...
          for (j = 0; j < c->nciphers; j++) {
            alg = ssh2_kexinit_addalg(s->kexlists[k], c->list[j]->name);
    ...
    for (j = KEXLIST_CSMAC; j <= KEXLIST_SCMAC; j++) {
      ...
      for (i = 0; i < s->nmacs; i++)
        ...
        if (s->maclist[i]->etm_name) {
          alg = ssh2_kexinit_addalg(s->kexlists[j], s->maclist[i]->etm_name);
          alg->u.mac.mac = s->maclist[i];
          alg->u.mac.etm = TRUE;
        }
    }
   ...
   */
  {
    ...
    for (i = 0; i < NKEXLIST; i++) {
      ...
      for (j = 0; j < MAXKEXLIST; j++) {
        ...
        if (in_commasep_string(alg->name, str, len)) {
          ...
          if (i == KEXLIST_KEX || i == KEXLIST_HOSTKEY) {
            ...
            if (j != 0 || !first_in_commasep_string(alg->name, str, len))
              s->guessok = FALSE;
          }
          if (i == KEXLIST_KEX) {
      ...
      bombout((
          "Couldn't agree a %s (available: %.*s)", kexlist_descr[i], len, str));
      crStopV;
    matched:;
      ...
      if (i == KEXLIST_HOSTKEY) {
        ...
        for (j = 0; j < lenof(hostkey_algs); j++) {
          if (hostkey_algs[j].alg != ssh->hostkey &&
              in_commasep_string(hostkey_algs[j].alg->name, str, len) &&
              !have_ssh_host_key(ssh->savedhost,
                                 ssh->savedport,
                                 hostkey_algs[j].alg->keytype)) {
    ...
    hash_string(
        ssh->kex->hash, ssh->exhash, pktin->body - 1, pktin->length + 1);
    ...
    if (s->warn_kex) {
      ...
      ssh_set_frozen(ssh, 0);
      if (s->dlgret == 0) {
        ssh_disconnect(ssh, "User aborted at kex warning", NULL, 0, TRUE);
        crStopV;
      }
    }
    ...
    if (s->warn_hk) {
      ...
      for (j = 0; j < ssh->n_uncert_hostkeys; j++) {
        ...
        if (better) {
          if (betteralgs) {
            ...
            betteralgs =
                dupcat(betteralgs, ",", hktype->alg->name, (const char *)NULL);
            sfree(old_ba);
          } else {
      ...
      ssh_set_frozen(ssh, 0);
      if (s->dlgret == 0) {
        ssh_disconnect(ssh, "User aborted at host key warning", NULL, 0, TRUE);
        crStopV;
      }
    }
  ...
  if (ssh->kex->main_type == KEXTYPE_DH) {
    ...
    s->hkey = ssh->hostkey->newkey(ssh->hostkey, s->hostkeydata, s->hostkeylen);
...

    {
      ...
      hash_string(ssh->kex->hash, ssh->exhash, publicPoint, publicPointLength);
      sfree(publicPoint);
    }
    ...
    logeventf(
        ssh, "Doing RSA key exchange with hash %s", ssh->kex->hash->text_name);
     ...
     */
    {
      ...
      ssh_rsakex_encrypt(
          ssh->kex->hash, kstr2, kstr2len, outstr, outstrlen, s->rsakey);
  ...
  if (!ssh->hostkey->verifysig(s->hkey,
                               s->sigdata,
                               s->siglen,
  ...
  if (!s->got_session_id) {
     ...
     */
    {
      ...
      for (i = 0; i < lenof(hostkey_algs); i++) {
        ...
        if (j < ssh->n_uncert_hostkeys) {
          char *newlist;
          if (list)
            newlist = dupprintf("%s/%s", list, hostkey_algs[i].alg->name);
          else
    ...
    /* First check against manually configured host keys. */
    s->dlgret =
        verify_ssh_manual_host_key(ssh, s->fingerprint, ssh->hostkey, s->hkey);
    if (s->dlgret == 0) { /* did not match */
      bombout(("Host key did not appear in manually configured list"));
      crStopV;
    } else if (s->dlgret < 0) { /* none configured; use standard handling */
      ...
      ssh_set_frozen(ssh, 0);
      if (s->dlgret == 0) {
        ssh_disconnect(ssh, "Aborted at host key verification", NULL, 0, TRUE);
        crStopV;
      }
    }
    sfree(s->fingerprint);
    ...
    sfree(s->fingerprint);
    store_host_key(
        ssh->savedhost, ssh->savedport, ssh->hostkey->keytype, s->keystr);
  ...
  if (ssh->cscipher) {
    ...
    key = ssh2_mkkey(
        ssh, s->K, s->exchange_hash, 'C', ssh->cscipher->padded_keybytes);
    ssh->cscipher->setkey(ssh->cs_cipher_ctx, key);
    smemclr(key, ssh->cscipher->padded_keybytes);
    sfree(key);

    key = ssh2_mkkey(ssh, s->K, s->exchange_hash, 'A', ssh->cscipher->blksize);
  ...
  if (ssh->csmac) {
    ...
    key = ssh2_mkkey(ssh, s->K, s->exchange_hash, 'E', ssh->csmac->keylen);
  ...
  if (ssh->sccipher) {
    ...
    key = ssh2_mkkey(
        ssh, s->K, s->exchange_hash, 'D', ssh->sccipher->padded_keybytes);
    ssh->sccipher->setkey(ssh->sc_cipher_ctx, key);
    smemclr(key, ssh->sccipher->padded_keybytes);
    sfree(key);

    key = ssh2_mkkey(ssh, s->K, s->exchange_hash, 'B', ssh->sccipher->blksize);
  ...
  if (ssh->scmac) {
    ...
    key = ssh2_mkkey(ssh, s->K, s->exchange_hash, 'F', ssh->scmac->keylen);
  ...
  if (conf_get_int(ssh->conf, CONF_ssh_rekey_time) != 0)
    ssh->next_rekey = schedule_timer(
        conf_get_int(ssh->conf, CONF_ssh_rekey_time) * 60 * TICKSPERSEC,
        ssh2_timer,
        ssh);
   ...
   */
  while (
      !((pktin && pktin->type == SSH2_MSG_KEXINIT) || (!pktin && inlen < 0))) {
  wait_for_rekey:
    if (!ssh->protocol_initial_phase_done) {
      ssh->protocol_initial_phase_done = TRUE;
      /*
...
static int ssh_is_simple(Ssh ssh)
{
   ...
   */
  return (conf_get_int(ssh->conf, CONF_ssh_simple) && !ssh->bare_connection &&
          !ssh->connshare);
}
 ...
 */
static void ssh2_handle_winadj_response(struct ssh_channel *,
                                        struct Packet *,
                                        void *);
static void ssh2_set_window(struct ssh_channel *c, int newwin)
{
...
static struct ssh_channel *ssh_channel_msg(Ssh ssh, struct Packet *pktin)
{
  ...
  if (c->type == CHAN_SHARING) {
    share_got_pkt_from_server(
        c->u.sharing.ctx, pktin->type, pktin->body, pktin->length);
    return NULL;
  }
  return c;
}
...
static void ssh2_msg_channel_data(Ssh ssh, struct Packet *pktin)
{
  ...
  if (data) {
    ...
    bufsize = ssh_channel_data(
        c, ext_type == SSH2_EXTENDED_DATA_STDERR, data, length);
    /*
...
{
  if (ssh->version == 2 && !conf_get_int(ssh->conf, CONF_ssh_no_shell) &&
...
static void ssh2_channel_check_close(struct ssh_channel *c)
{
  ...
  if ((!((CLOSES_SENT_EOF | CLOSES_RCVD_EOF) & ~c->closes) ||
       c->type == CHAN_ZOMBIE) &&
      !c->v.v2.chanreq_head && !(c->closes & CLOSES_SENT_CLOSE)) {
    /*
...
#define CHECK_HYPOTHESIS(offset, result)                                       \
  do {                                                                         \
    ...
    if (q >= 0 && q + 4 <= len) {                                              \
      ...
      if (q >= 0 && q + 4 <= len &&                                            \
          ((q = toint(q + 4 + GET_32BIT(p + q))) != 0) && q == len)            \
        result = TRUE;                                                         \
    }                                                                          \
  } while (0)
...
static void ssh2_msg_channel_open(Ssh ssh, struct Packet *pktin)
{
  ...
  if (typelen == 3 && !memcmp(type, "x11", 3)) {
    ...
    if (!ssh->X11_fwd_enabled && !ssh->connshare)
      ...
      c->u.x11.xconn = x11_init(ssh->x11authtree, c, addrstr, peerport);
      c->type = CHAN_X11;
    ...
    if (realpf == NULL) {
      ...
      if (realpf->share_ctx) {
         ...
         */
        share_got_pkt_from_server(
            realpf->share_ctx, pktin->type, pktin->body, pktin->length);
        sfree(c);
        return;
      }

      err = pfd_connect(&c->u.pfd.pf,
                        realpf->dhost,
                        realpf->dport,
                        c,
                        ssh->conf,
  ...
  } else if (typelen == 22 && !memcmp(type, "auth-agent@openssh.com", 22)) {
    if (!ssh->agentfwd_enabled)
                                 ...
                                 int initial_len)
{
  ...
  share_setup_x11_channel(share_cs,
                          share_chan,
                          c->localid,
                          c->remoteid,
                          c->v.v2.remwindow,
...
static void ssh2_setup_x11(struct ssh_channel *c,
                           struct Packet *pktin,
                           void *ctx)
{
  ...
  logevent("Requesting X11 forwarding");
  pktout = ssh2_chanreq_init(ssh->mainchan, "x11-req", ssh2_setup_x11, s);
...
static void ssh2_setup_agent(struct ssh_channel *c,
                             struct Packet *pktin,
                             void *ctx)
{
  ...
  pktout = ssh2_chanreq_init(
      ssh->mainchan, "auth-agent-req@openssh.com", ssh2_setup_agent, s);
  ssh2_pkt_send(ssh, pktout);
...
static void ssh2_setup_pty(struct ssh_channel *c,
                           struct Packet *pktin,
                           void *ctx)
{
  ...
  pktout = ssh2_chanreq_init(ssh->mainchan, "pty-req", ssh2_setup_pty, s);
...
static void ssh2_setup_env(struct ssh_channel *c,
                           struct Packet *pktin,
                           void *ctx)
{
...
static void ssh2_response_authconn(struct ssh_channel *c,
                                   struct Packet *pktin,
                                   void *ctx)
{
  if (pktin)
                             ...
                             struct Packet *pktin)
{
  ...
  /* ssh->packet_dispatch[SSH2_MSG_USERAUTH_PASSWD_CHANGEREQ] =
   * ssh2_msg_authconn; duplicate case value */
  /* ssh->packet_dispatch[SSH2_MSG_USERAUTH_INFO_REQUEST] = ssh2_msg_authconn;
  ...
  ssh->packet_dispatch[SSH2_MSG_USERAUTH_BANNER] = ssh2_msg_userauth_banner;
  ...
  if (!s->we_are_in) {
    ...
    if (!filename_is_null(s->keyfile)) {
      int keytype;
      logeventf(
          ssh, "Reading key file \"%.150s\"", filename_to_str(s->keyfile));
      keytype = key_type(s->keyfile);
      if (keytype == SSH_KEYTYPE_SSH2 ||
          keytype == SSH_KEYTYPE_SSH2_PUBLIC_RFC4716 ||
          keytype == SSH_KEYTYPE_SSH2_PUBLIC_OPENSSH) {
        const char *error;
        s->publickey_blob = ssh2_userkey_loadpub(s->keyfile,
    ...
    if (conf_get_int(ssh->conf, CONF_tryagent) && agent_exists()) {
      ...
      ssh->auth_agent_query = agent_query(s->agent_request,
                                          5,
                                          &r,
                                          &s->agent_responselen,
                                          ssh_agent_callback,
      ...
      if (s->agent_response && s->agent_responselen >= 5 &&
        ...
        if (s->nkeys < 0) {
          logeventf(ssh,
                    "Pageant response contained a negative"
                    " key count %d",
                    s->nkeys);
          s->nkeys = 0;
        ...
        if (s->publickey_blob) {
          ...
          for (keyi = 0; keyi < s->nkeys; keyi++) {
            ...
            if (s->pklen == s->publickey_bloblen &&
                !memcmp(p + 4, s->publickey_blob, s->publickey_bloblen)) {
              logeventf(ssh,
                        "Pageant key #%d matches "
                        "configured key file",
                        keyi);
    ...
    while (1) {
      ...
      if (s->can_pubkey && !s->done_agent && s->nkeys) {
        ...
        if (pktin->type != SSH2_MSG_USERAUTH_PK_OK) {
          ...
          s->siglen = s->pktout->length - 5 + 4 + ssh->v2_session_id_len;
          ...
          *s->q++ = SSH2_AGENTC_SIGN_REQUEST;
          ...
          PUT_32BIT(s->q, 0);
          ssh->auth_agent_query = agent_query(s->agentreq,
                                              s->len + 4,
                                              &vret,
                                              &s->retlen,
                                              ssh_agent_callback,
          ...
          sfree(s->agentreq);
          if (s->ret) {
            if (s->retlen >= 9 && s->ret[4] == SSH2_AGENT_SIGN_RESPONSE &&
        ...
        ssh2_pkt_addstring_start(s->pktout);
        ssh2_pkt_addstring_data(
            s->pktout, (char *)s->publickey_blob, s->publickey_bloblen);
        ssh2_pkt_send(ssh, s->pktout);
        ...
        if (key) {
           ...
           * outgoing packet.
           */
          sigdata_len = s->pktout->length - 5 + 4 + ssh->v2_session_id_len;
          if (ssh->remote_bugs & BUG_SSH2_PK_SESSIONID)
            sigdata_len -= 4;
         ...
         */
        {
          ...
          for (i = 0; i < ngsslibs; i++) {
            int want_id = conf_get_int_int(ssh->conf, CONF_ssh_gsslist, i);
        ...
        ssh_pkt_adddata(s->pktout, s->gss_buf.value, s->gss_buf.length);
        ssh2_pkt_send(ssh, s->pktout);
        crWaitUntilV(pktin);
        ...
        s->gss_stat = s->gsslib->import_name(
            s->gsslib, ssh->fullhostname, &s->gss_srv_name);
        ...
        do {
          ...
          if (s->gss_stat != SSH_GSS_S_COMPLETE &&
            ...
            if (s->gsslib->display_status(s->gsslib, s->gss_ctx, &s->gss_buf) ==
                SSH_GSS_OK) {
          ...
          if (s->gss_sndtok.length != 0) {
            ...
            ssh_pkt_addstring_data(
                s->pktout, s->gss_sndtok.value, s->gss_sndtok.length);
            ssh2_pkt_send(ssh, s->pktout);
        ...
        ssh_pkt_addstring_start(s->pktout);
        ssh_pkt_addstring_data(
            s->pktout, (char *)ssh->v2_session_id, ssh->v2_session_id_len);
        ...
        while (pktin->type == SSH2_MSG_USERAUTH_INFO_REQUEST) {
          ...
          for (i = 0; i < s->num_prompts; i++) {
            ...
            int prompt_len;
            int echo;
            static char noprompt[] = "<server failed to send prompt>: ";
            ...
            add_prompt(
                s->cur_prompt, dupprintf("%.*s", prompt_len, prompt), echo);
          }
          ...
          for (i = 0; i < s->num_prompts; i++) {
            ssh2_pkt_addstring(s->pktout, s->cur_prompt->prompts[i]->result);
          }
        ...
        while (pktin->type == SSH2_MSG_USERAUTH_PASSWD_CHANGEREQ) {
          ...
          add_prompt(
              ...
              FALSE);
          add_prompt(s->cur_prompt, dupstr("Enter new password: "), FALSE);
          add_prompt(s->cur_prompt, dupstr("Confirm new password: "), FALSE);
          ...
          while (!got_new) {
            ...
            got_new = (strcmp(s->cur_prompt->prompts[1]->result,
                              s->cur_prompt->prompts[2]->result) == 0);
            if (!got_new)
          ...
          ssh2_pkt_addbool(s->pktout, TRUE);
          ssh2_pkt_addstring(s->pktout, s->password);
          ssh2_pkt_addstring(s->pktout, s->cur_prompt->prompts[1]->result);
          free_prompts(s->cur_prompt);
          ssh2_pkt_send_with_padding(ssh, s->pktout, 256);
  ...
  if (ssh->mainchan && ssh_is_simple(ssh)) {
     ...
     */
    s->pktout = ssh2_chanreq_init(
        ssh->mainchan, "simple@putty.projects.tartarus.org", NULL, NULL);
    ssh2_pkt_send(ssh, s->pktout);
  }
  ...
  if (ssh->mainchan && !ssh->ncmode) {
    ...
    if (conf_get_int(ssh->conf, CONF_x11_forward)) {
      ssh->x11disp = x11_setup_display(
          conf_get_str(ssh->conf, CONF_x11_display), ssh->conf);
      if (!ssh->x11disp) {
        /* FIXME: return an error message from x11_setup_display */
        logevent("X11 forwarding not enabled: unable to"
                 " initialise X display");
    ...
    while (1) {
      ...
      if (subsys) {
        s->pktout = ssh2_chanreq_init(
            ssh->mainchan, "subsystem", ssh2_response_authconn, NULL);
        ssh2_pkt_addstring(s->pktout, cmd);
      } else if (*cmd) {
        s->pktout = ssh2_chanreq_init(
            ssh->mainchan, "exec", ssh2_response_authconn, NULL);
        ssh2_pkt_addstring(s->pktout, cmd);
      } else {
        s->pktout = ssh2_chanreq_init(
            ssh->mainchan, "shell", ssh2_response_authconn, NULL);
      }
      ssh2_pkt_send(ssh, s->pktout);
      ...
      if (pktin->type != SSH2_MSG_CHANNEL_SUCCESS) {
         ...
         */
        if (!ssh->fallback_cmd && *conf_get_str(ssh->conf, CONF_remote_cmd2)) {
...
static void ssh2_msg_unexpected(Ssh ssh, struct Packet *pktin)
{
  char *buf =
      dupprintf("Server protocol violation: unexpected %s packet",
                ssh2_pkt_type(ssh->pkt_kctx, ssh->pkt_actx, pktin->type));
  ssh_disconnect(ssh, NULL, buf, SSH2_DISCONNECT_PROTOCOL_ERROR, FALSE);
  sfree(buf);
}
...
static void ssh2_protocol_setup(Ssh ssh)
{
  ...
  /* ssh->packet_dispatch[SSH2_MSG_KEX_DH_GEX_REQUEST] = ssh2_msg_transport;
   * duplicate case value */
  /* ssh->packet_dispatch[SSH2_MSG_KEX_DH_GEX_GROUP] = ssh2_msg_transport;
   * duplicate case value */
  ...
  /* ssh->packet_dispatch[SSH2_MSG_USERAUTH_PASSWD_CHANGEREQ] =
   * ssh2_msg_unexpected; duplicate case value */
  /* ssh->packet_dispatch[SSH2_MSG_USERAUTH_INFO_REQUEST] = ssh2_msg_unexpected;
   * duplicate case value */
                            ...
                            int keepalive)
{
  ...
  *backend_handle = ssh;
  ...
  ssh->incoming_data_size = ssh->outgoing_data_size = ssh->deferred_data_size =
      0L;
  ssh->max_data_size =
      parse_blocksize(conf_get_str(ssh->conf, CONF_ssh_rekey_data));
  ssh->kex_in_progress = FALSE;
  ...
  case SSH_STATE_SESSION:
    if (!conf_get_int(ssh->conf, CONF_nopty)) {
      if (ssh->version == 1) {
        ...
        pktout = ssh2_chanreq_init(ssh->mainchan, "window-change", NULL, NULL);
        ssh2_pkt_adduint32(pktout, ssh->term_width);
        ssh2_pkt_adduint32(pktout, ssh->term_height);
...
static void ssh_special(void *handle, Telnet_Special code)
{
  ...
  if (code == TS_EOF) {
    ...
    if (!ssh->kex_in_progress && !ssh->bare_connection && ssh->version == 2) {
