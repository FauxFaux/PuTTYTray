...
static void rlogin_log(Plug plug,
                       int type,
                       SockAddr addr,
                       int port,
...
static void rlogin_startup(Rlogin rlogin, const char *ruser)
{
  ...
  sk_write(
      rlogin->s, rlogin->cfg.localusername, strlen(rlogin->cfg.localusername));
  sk_write(rlogin->s, &z, 1);
  sk_write(rlogin->s, ruser, strlen(ruser));
  sk_write(rlogin->s, &z, 1);
  sk_write(rlogin->s, rlogin->cfg.termtype, strlen(rlogin->cfg.termtype));
  sk_write(rlogin->s, "/", 1);
  for (p = rlogin->cfg.termspeed; isdigit((unsigned char)*p); p++)
    continue;
                               ...
                               int keepalive)
{
  static const struct plug_function_table fn_table = {
      rlogin_log, rlogin_closing, rlogin_receive, rlogin_sent};
  SockAddr addr;
  const char *err;
  Rlogin rlogin;
  ...
  *backend_handle = rlogin;
  ...
  if (get_remote_username(cfg, ruser, sizeof(ruser))) {
    ...
    add_prompt(rlogin->prompt,
               dupstr("rlogin username: "),
               TRUE,
               sizeof(cfg->username));
...
static int rlogin_send(void *handle, char *buf, int len)
{
  ...
  if (rlogin->prompt) {
     ...
     */
    int ret = get_userpass_input(rlogin->prompt, (unsigned char *)buf, len);
    if (ret >= 0) {
      rlogin_startup(rlogin, rlogin->prompt->prompts[0]->result);
