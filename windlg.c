...
static void ctlposinit(struct ctlpos *cp,
                       HWND hwnd,
                       int leftborder,
                       int rightborder,
                       int topborder)
...
static void doctl(struct ctlpos *cp,
                  RECT r,
                  char *wclass,
                  int wstyle,
                  int exstyle,
 ...
 * panel).
 */
static void radioline(struct ctlpos *cp, char *text, int id, int nacross, ...)
{
  RECT r;
  va_list ap;
  int group;
 ...
 */
static void staticbtn(
    struct ctlpos *cp, char *stext, int sid, char *btext, int bid)
{
  const int height =
 ...
 */
static void ersatztab(
    struct ctlpos *cp, char *stext, int sid, int lid, int s2id)
{
 ...
 */
static void envsetter(struct ctlpos *cp,
                      char *stext,
                      int sid,
                      char *e1stext,
                      int e1sid,
 ...
 */
static void charclass(struct ctlpos *cp,
                      char *stext,
                      int sid,
                      int listid,
                      char *btext,
 ...
 */
static void colouredit(struct ctlpos *cp,
                       char *stext,
                       int sid,
                       int listid,
                       char *btext,
  ...
  CheckDlgButton(hwnd, IDC_PALETTE, cfg.try_palette);
  {
    ...
    for (i = 0; i < 22; i++)
      if (cfg.bold_colour || permcolour[i])
        SendDlgItemMessage(hwnd, IDC_LIST, LB_ADDSTRING, 0, (LPARAM)colours[i]);
  }
    ...
    HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, int dlgtype)
{
  ...
  case WM_INITDIALOG:
    ...
    /* The Telnet panel. Accelerators used: [acgo] svldrbf */
    {
      ...
      if (dlgtype == 0) {
        bartitle(
            &cp, "Options controlling Telnet connections", IDC_TITLE_TELNET);
        beginbox(&cp,
                 "Data to send to the server",
                 IDC_BOX_TELNET1,
                 IDC_BOXT_TELNET1);
    ...
    if (backends[2].backend != NULL) {
      ...
      if (dlgtype == 0) {
        ...
        beginbox(
            &cp, "Data to send to the server", IDC_BOX_SSH1, IDC_BOXT_SSH1);
        multiedit(
            &cp, "&Remote command:", IDC_CMDSTATIC, IDC_CMDEDIT, 100, NULL);
        endbox(&cp);
        beginbox(&cp, "Authentication options", IDC_BOX_SSH2, IDC_BOXT_SSH2);
        checkbox(&cp, "Atte&mpt TIS or CryptoCard authentication", IDC_AUTHTIS);
        ...
        editbutton(&cp,
                   ...
                   IDC_PKBUTTON);
        endbox(&cp);
        beginbox(&cp, "Protocol options", IDC_BOX_SSH3, IDC_BOXT_SSH3);
  ...
  case WM_COMMAND:
    ...
    if (GetWindowLong(hwnd, GWL_USERDATA) == 1)
      ...
      case IDC_TSEDIT:
        if (HIWORD(wParam) == EN_CHANGE)
          GetDlgItemText(
              hwnd, IDC_TSEDIT, cfg.termspeed, sizeof(cfg.termspeed) - 1);
        break;
      case IDC_LOGEDIT:
        if (HIWORD(wParam) == EN_CHANGE)
          GetDlgItemText(
        ...
        if (HIWORD(wParam) == BN_CLICKED ||
          ...
          if (cfg.bold_colour && n != 22) {
            for (i = 0; i < 22; i++)
              if (!permcolour[i])
                SendDlgItemMessage(
                    hwnd, IDC_LIST, LB_INSERTSTRING, i, (LPARAM)colours[i]);
      ...
      case IDC_88592CP852:
        cfg.xlat_enablekoiwin = IsDlgButtonChecked(hwnd, IDC_KOI8WIN1251);
        cfg.xlat_88592w1250 = IsDlgButtonChecked(hwnd, IDC_88592WIN1250);
        cfg.xlat_88592cp852 = IsDlgButtonChecked(hwnd, IDC_88592CP852);
        break;
      ...
      case IDC_VTPOORMAN:
        cfg.vtmode =
            (IsDlgButtonChecked(hwnd, IDC_VTXWINDOWS)
                 ? VT_XWINDOWS
                                ...
                                LPARAM lParam)
{
  ...
  if (msg == WM_COMMAND && LOWORD(wParam) == IDCX_ABOUT) {
    EnableWindow(hwnd, 0);
    DialogBox(hinst, MAKEINTRESOURCE(IDD_ABOUTBOX), GetParent(hwnd), AboutProc);
    EnableWindow(hwnd, 1);
    SetActiveWindow(hwnd);
  }
...
void showabout(HWND hwnd)
{
  if (!abtbox) {
    abtbox =
        CreateDialog(hinst, MAKEINTRESOURCE(IDD_ABOUTBOX), hwnd, AboutProc);
