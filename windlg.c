                                       ...
                                       LPARAM lParam)
{
  ...
  case WM_INITDIALOG:
     ...
     */
    {
      ...
      for (i = 0; (c = winctrl_findbyindex(&ctrls_panel, i)) != NULL; i++) {
        if (c->ctrl) {
                         ...
                         char *fingerprint)
{
  ...
  if (ret == 2) { /* key was different */
    ...
    mbret = MessageBox(NULL, message, title, MB_ICONWARNING | MB_YESNOCANCEL);
    sfree(message);
    sfree(title);
    if (mbret == IDYES)
...
void askcipher(void *frontend, char *ciphername, int cs)
{
  ...
  message = dupprintf(
      ...
      ciphername);
  title = dupprintf(mbtitle, appname);
  mbret = MessageBox(NULL, message, title, MB_ICONWARNING | MB_YESNO);
  sfree(message);
  sfree(title);
  if (mbret == IDYES)
    return;
