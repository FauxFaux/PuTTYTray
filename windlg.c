...
static int CALLBACK LogProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_INITDIALOG: {
    ...
    SendDlgItemMessage(hwnd, IDN_LIST, LB_SETTABSTOPS, 2, (LPARAM)tabs);
  }
    for (i = 0; i < nevents; i++)
  ...
  case WM_COMMAND:
    ...
    case IDN_COPY:
      if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED) {
        int selcount;
        int *selitems;
        selcount = SendDlgItemMessage(hwnd, IDN_LIST, LB_GETSELCOUNT, 0, 0);
        ...
        if (selitems) {
          int count = SendDlgItemMessage(
              hwnd, IDN_LIST, LB_GETSELITEMS, selcount, (LPARAM)selitems);
          int i;
          int size;
          char *clipdata;
...
static void init_dlg_ctrls(HWND hwnd, int keepsess)
{
  ...
  if (!keepsess) {
    ...
    for (i = 0; i < nsessions; i++)
      SendDlgItemMessage(
          hwnd, IDC_SESSLIST, LB_ADDSTRING, 0, (LPARAM)(sessions[i]));
  }
  ...
  CheckRadioButton(hwnd,
                   ...
                   cfg.protocol == PROT_SSH
                       ...
                       : cfg.protocol == PROT_TELNET
                             ? IDC_PROTTELNET
                             : cfg.protocol == PROT_RLOGIN ? IDC_PROTRLOGIN
                                                           : IDC_PROTRAW);
                                                  ...
                                                  : IDC_LSTATXAPN);
  {
    ...
    while (*p) {
      SendDlgItemMessage(hwnd, IDC_ENVLIST, LB_ADDSTRING, 0, (LPARAM)p);
      p += strlen(p) + 1;
    }
    p = cfg.portfwd;
    while (*p) {
      SendDlgItemMessage(hwnd, IDC_PFWDLIST, LB_ADDSTRING, 0, (LPARAM)p);
      p += strlen(p) + 1;
    }
  }
  CheckRadioButton(
      hwnd, IDC_EMBSD, IDC_EMRFC, cfg.rfc_environ ? IDC_EMRFC : IDC_EMBSD);
  CheckRadioButton(hwnd,
                   IDC_TPASSIVE,
                   IDC_TACTIVE,
...

  {
    ...
    for (i = 0; i < CIPHER_MAX; i++) {
      ...
      pos = SendDlgItemMessage(
          hwnd, IDC_CIPHERLIST, LB_ADDSTRING, 0, (LPARAM)cstr);
      SendDlgItemMessage(hwnd, IDC_CIPHERLIST, LB_SETITEMDATA, pos, (LPARAM)c);
    }
  }
  ...
  CheckDlgButton(hwnd, IDC_RTFPASTE, cfg.rtf_paste);
  {
    ...
    SendDlgItemMessage(hwnd, IDC_CCLIST, LB_SETTABSTOPS, 4, (LPARAM)tabs);
  }
  for (i = 0; i < 128; i++) {
    char str[100];
    sprintf(str,
            ...
            (i >= 0x21 && i != 0x7F) ? i : ' ',
            cfg.wordness[i]);
    SendDlgItemMessage(hwnd, IDC_CCLIST, LB_ADDSTRING, 0, (LPARAM)str);
  }
  ...
  CheckDlgButton(hwnd, IDC_PALETTE, cfg.try_palette);
  {
    ...
    for (i = n; i-- > 0;)
      SendDlgItemMessage(hwnd, IDC_COLOURLIST, LB_DELETESTRING, i, 0);
    for (i = 0; i < 22; i++)
      if (cfg.bold_colour || permcolour[i])
        SendDlgItemMessage(
            hwnd, IDC_COLOURLIST, LB_ADDSTRING, 0, (LPARAM)colours[i]);
  }
...

  {
    ...
    for (i = 0; (cp = cp_enumerate(i)) != NULL; i++) {
      SendDlgItemMessage(hwnd, IDC_CODEPAGE, CB_ADDSTRING, 0, (LPARAM)cp);
    }
...

  {
    ...
    SendDlgItemMessage(
        hwnd, IDC_PRINTER, CB_ADDSTRING, 0, (LPARAM)PRINTER_DISABLED_STRING);
    for (i = 0; i < nprinters; i++) {
      char *printer_name = printer_get_name(pe, i);
      SendDlgItemMessage(
          hwnd, IDC_PRINTER, CB_ADDSTRING, 0, (LPARAM)printer_name);
    }
  ...
  CheckRadioButton(hwnd,
                   IDC_PROXYSOCKSVER5,
                   IDC_PROXYSOCKSVER4,
                   cfg.proxy_socks_version == 4 ? IDC_PROXYSOCKSVER4
                                                : IDC_PROXYSOCKSVER5);
  ...
  SendDlgItemMessage(
      ...
      IDC_BUGD_IGNORE1,
      CB_SETCURSEL,
      cfg.sshbug_ignore1 == BUG_ON ? 2 : cfg.sshbug_ignore1 == BUG_OFF ? 1 : 0,
      0);
  ...
  SendDlgItemMessage(hwnd,
                     IDC_BUGD_PLAINPW1,
                     CB_SETCURSEL,
                     cfg.sshbug_plainpw1 == BUG_ON
                         ? 2
                         : cfg.sshbug_plainpw1 == BUG_OFF ? 1 : 0,
                     0);
  ...
  SendDlgItemMessage(
      ...
      IDC_BUGD_RSA1,
      CB_SETCURSEL,
      cfg.sshbug_rsa1 == BUG_ON ? 2 : cfg.sshbug_rsa1 == BUG_OFF ? 1 : 0,
      0);
  ...
  SendDlgItemMessage(
      ...
      IDC_BUGD_HMAC2,
      CB_SETCURSEL,
      cfg.sshbug_hmac2 == BUG_ON ? 2 : cfg.sshbug_hmac2 == BUG_OFF ? 1 : 0,
      0);
  SendDlgItemMessage(hwnd, IDC_BUGD_DERIVEKEY2, CB_RESETCONTENT, 0, 0);
  SendDlgItemMessage(
      hwnd, IDC_BUGD_DERIVEKEY2, CB_ADDSTRING, 0, (LPARAM) "Auto");
  SendDlgItemMessage(
      hwnd, IDC_BUGD_DERIVEKEY2, CB_ADDSTRING, 0, (LPARAM) "Off");
  SendDlgItemMessage(hwnd, IDC_BUGD_DERIVEKEY2, CB_ADDSTRING, 0, (LPARAM) "On");
  SendDlgItemMessage(hwnd,
                     IDC_BUGD_DERIVEKEY2,
                     CB_SETCURSEL,
                     cfg.sshbug_derivekey2 == BUG_ON
                         ? 2
                         : cfg.sshbug_derivekey2 == BUG_OFF ? 1 : 0,
                     0);
  ...
  SendDlgItemMessage(
      ...
      IDC_BUGD_RSAPAD2,
      CB_SETCURSEL,
      cfg.sshbug_rsapad2 == BUG_ON ? 2 : cfg.sshbug_rsapad2 == BUG_OFF ? 1 : 0,
      0);
  ...
  SendDlgItemMessage(
      ...
      IDC_BUGD_DHGEX2,
      CB_SETCURSEL,
      cfg.sshbug_dhgex2 == BUG_ON ? 2 : cfg.sshbug_dhgex2 == BUG_OFF ? 1 : 0,
      0);
}
...
static void create_controls(HWND hwnd, int dlgtype, int panel)
{
  if (panel == sessionpanelstart) {
    ...
    bartitle(&cp, "Basic options for your PuTTY session", IDC_TITLE_SESSION);
    if (dlgtype == 0) {
      beginbox(&cp,
      ...
      endbox(&cp);
      beginbox(&cp, "Load, save or delete a stored session", IDC_BOX_SESSION2);
      sesssaver(&cp,
                "Sav&ed Sessions",
                IDC_SESSSTATIC,
                IDC_SESSEDIT,
  ...
  if (panel == loggingpanelstart) {
    ...
    struct ctlpos cp;
    ctlposinit(&cp, hwnd, 80, 3, 13);
    bartitle(&cp, "Options controlling session logging", IDC_TITLE_LOGGING);
  ...
  if (panel == terminalpanelstart) {
    ...
    ctlposinit(&cp, hwnd, 80, 3, 13);
    bartitle(
        &cp, "Options controlling the terminal emulation", IDC_TITLE_TERMINAL);
    ...
    multiedit(
        &cp, "An&swerback to ^E:", IDC_ANSWERBACK, IDC_ANSWEREDIT, 100, NULL);
    endbox(&cp);
  ...
  if (panel == featurespanelstart) {
    ...
    checkbox(&cp, "Disable remote-controlled terminal re&sizing", IDC_NORESIZE);
    checkbox(&cp,
  ...
  if (panel == bellpanelstart) {
    ...
    struct ctlpos cp;
    ctlposinit(&cp, hwnd, 80, 3, 13);
    bartitle(&cp, "Options controlling the terminal bell", IDC_TITLE_BELL);
    ...
    radioline(&cp,
              ...
              NULL);
    endbox(&cp);
    beginbox(&cp, "Control the bell overload behaviour", IDC_BOX_BELL2);
    checkbox(&cp, "Bell is temporarily &disabled when over-used", IDC_BELLOVL);
    staticedit(&cp,
               ...
               20);
    staticedit(
        &cp, "... in &this many seconds", IDC_BELLOVLTSTATIC, IDC_BELLOVLT, 20);
    statictext(&cp,
  ...
  if (panel == keyboardpanelstart) {
    ...
    ctlposinit(&cp, hwnd, 80, 3, 13);
    bartitle(
        &cp, "Options controlling the effects of keys", IDC_TITLE_KEYBOARD);
    ...
    radioline(&cp,
              ...
              NULL);
    endbox(&cp);
    beginbox(&cp, "Enable extra keyboard features:", IDC_BOX_KEYBOARD3);
    checkbox(&cp, "AltGr ac&ts as Compose key", IDC_COMPOSEKEY);
    checkbox(&cp, "Control-Alt is &different from AltGr", IDC_CTRLALTKEYS);
    endbox(&cp);
  }
  ...
  if (panel == windowpanelstart) {
    ...
    struct ctlpos cp;
    ctlposinit(&cp, hwnd, 80, 3, 13);
    bartitle(&cp, "Options controlling PuTTY's window", IDC_TITLE_WINDOW);
    ...
    radiobig(&cp,
             ...
             NULL);
    endbox(&cp);
    beginbox(&cp, "Control the scrollback in the window", IDC_BOX_WINDOW2);
    staticedit(&cp, "Lines of &scrollback", IDC_SAVESTATIC, IDC_SAVEEDIT, 50);
    checkbox(&cp, "&Display scrollbar", IDC_SCROLLBAR);
    checkbox(
        &cp, "D&isplay scrollbar in full screen mode", IDC_SCROLLBARFULLSCREEN);
    checkbox(&cp, "Reset scrollback on &keypress", IDC_SCROLLKEY);
    checkbox(&cp, "Reset scrollback on dis&play activity", IDC_SCROLLDISP);
    endbox(&cp);
  }
  ...
  if (panel == appearancepanelstart) {
    ...
    endbox(&cp);
    beginbox(
        &cp, "Set the font used in the terminal window", IDC_BOX_APPEARANCE2);
    staticbtn(&cp, "", IDC_FONTSTATIC, "Cha&nge...", IDC_CHOOSEFONT);
    endbox(&cp);
    beginbox(&cp, "Adjust the use of the window title", IDC_BOX_APPEARANCE3);
    multiedit(&cp, "Window &title:", IDC_WINTITLE, IDC_WINEDIT, 100, NULL);
    checkbox(&cp, "Avoid ever using &icon title", IDC_WINNAME);
    endbox(&cp);
    beginbox(&cp, "Adjust the use of the mouse pointer", IDC_BOX_APPEARANCE4);
    checkbox(&cp, "Hide mouse &pointer when typing in window", IDC_HIDEMOUSE);
    endbox(&cp);
    beginbox(&cp, "Adjust the window border", IDC_BOX_APPEARANCE5);
    checkbox(&cp, "&Sunken-edge border (slightly thicker)", IDC_SUNKENEDGE);
    staticedit(&cp,
  ...
  if (panel == behaviourpanelstart) {
    ...
    ctlposinit(&cp, hwnd, 80, 3, 13);
    bartitle(
        &cp, "Configure the behaviour of PuTTY's window", IDC_TITLE_WINDOW);
  ...
  if (panel == translationpanelstart) {
    ...
    radiobig(&cp,
             ...
             IDC_VTSTATIC,
             "Font has &XWindows encoding",
             IDC_VTXWINDOWS,
             "Use font in &both ANSI and OEM modes",
             IDC_VTOEMANSI,
             "Use font in O&EM mode only",
             IDC_VTOEMONLY,
             "&Poor man's line drawing ("
             "+"
             ", "
             "-"
             " and "
  ...
  if (panel == selectionpanelstart) {
    ...
    struct ctlpos cp;
    ctlposinit(&cp, hwnd, 80, 3, 13);
    bartitle(&cp, "Options controlling copy and paste", IDC_TITLE_SELECTION);
    beginbox(&cp, "Translation of pasted characters", IDC_BOX_SELECTION1);
    checkbox(
        &cp, "&Don't translate line drawing chars into +, - and |", IDC_RAWCNP);
    checkbox(
        &cp, "Paste to clipboard in RT&F as well as plain text", IDC_RTFPASTE);
    endbox(&cp);
    beginbox(
        &cp, "Control which mouse button does which thing", IDC_BOX_SELECTION2);
    radiobig(&cp,
             ...
             NULL);
    checkbox(
        &cp, "Shift overrides a&pplication's use of mouse", IDC_MOUSEOVERRIDE);
    radioline(&cp,
    ...
    endbox(&cp);
    beginbox(
        &cp, "Control the select-one-word-at-a-time mode", IDC_BOX_SELECTION3);
    charclass(&cp,
              "Charact&er classes:",
              IDC_CCSTATIC,
              IDC_CCLIST,
  ...
  if (panel == colourspanelstart) {
    ...
    struct ctlpos cp;
    ctlposinit(&cp, hwnd, 80, 3, 13);
    bartitle(&cp, "Options controlling use of colours", IDC_TITLE_COLOURS);
    beginbox(&cp, "General options for colour usage", IDC_BOX_COLOURS1);
    checkbox(&cp, "&Bolded text is a different colour", IDC_BOLDCOLOUR);
    checkbox(&cp, "Attempt to use &logical palettes", IDC_PALETTE);
    endbox(&cp);
    beginbox(
        &cp, "Adjust the precise colours PuTTY displays", IDC_BOX_COLOURS2);
    colouredit(&cp,
  ...
  if (panel == connectionpanelstart) {
    ...
    if (dlgtype == 0) {
      beginbox(&cp, "Data to send to the server", IDC_BOX_CONNECTION1);
      ...
      checkbox(
          &cp, "Keyboard sends telnet Backspace and Interrupt", IDC_TELNETKEY);
      checkbox(
          &cp, "Return key sends telnet New Line instead of ^M", IDC_TELNETRET);
      endbox(&cp);
    }
    beginbox(&cp,
    ...
    endbox(&cp);
    if (dlgtype == 0) {
      beginbox(&cp, "Low-level TCP connection options", IDC_BOX_CONNECTION3);
      checkbox(
          &cp, "Disable &Nagle's algorithm (TCP_NODELAY option)", IDC_NODELAY);
      endbox(&cp);
    }
  }
  ...
  if (panel == proxypanelstart) {
    ...
    ctlposinit(&cp, hwnd, 80, 3, 13);
    if (dlgtype == 0) {
      bartitle(&cp, "Options controlling proxy usage", IDC_TITLE_PROXY);
      ...
      multiedit(&cp,
                ...
                100,
                NULL);
      staticedit(&cp, "&Username", IDC_PROXYUSERSTATIC, IDC_PROXYUSEREDIT, 60);
      staticpassedit(
          &cp, "Pass&word", IDC_PROXYPASSSTATIC, IDC_PROXYPASSEDIT, 60);
      endbox(&cp);
  ...
  if (panel == rloginpanelstart) {
    ...
    if (dlgtype == 0) {
      ...
      staticedit(
          &cp, "Terminal-&speed string", IDC_R_TSSTATIC, IDC_R_TSEDIT, 50);
      staticedit(
          &cp, "&Local username:", IDC_RLLUSERSTATIC, IDC_RLLUSEREDIT, 50);
      endbox(&cp);
    }
  }
  ...
  if (panel == sshpanelstart) {
    ...
    if (dlgtype == 0) {
      ...
      multiedit(&cp, "&Remote command:", IDC_CMDSTATIC, IDC_CMDEDIT, 100, NULL);
      endbox(&cp);
      ...
      prefslist(&cipherlist,
                ...
                IDC_CIPHERDN);
      checkbox(
          &cp, "Enable non-standard use of single-&DES in SSH 2", IDC_SSH2DES);
      endbox(&cp);
    }
  }
  ...
  if (panel == sshauthpanelstart) {
    ...
    if (dlgtype == 0) {
      bartitle(
          &cp, "Options controlling SSH authentication", IDC_TITLE_SSHAUTH);
      beginbox(&cp, "Authentication methods", IDC_BOX_SSHAUTH1);
      checkbox(
          &cp, "Atte&mpt TIS or CryptoCard authentication (SSH1)", IDC_AUTHTIS);
      checkbox(&cp,
               "Attempt \"keyboard-&interactive\" authentication"
               " (SSH2)",
               IDC_AUTHKI);
      endbox(&cp);
      beginbox(&cp, "Authentication parameters", IDC_BOX_SSHAUTH2);
      checkbox(&cp, "Allow agent &forwarding", IDC_AGENTFWD);
      checkbox(
          &cp, "Allow attempted changes of &username in SSH2", IDC_CHANGEUSER);
      editbutton(&cp,
  ...
  if (panel == sshbugspanelstart) {
    ...
    ctlposinit(&cp, hwnd, 80, 3, 13);
    if (dlgtype == 0) {
      bartitle(&cp, "Workarounds for SSH server bugs", IDC_TITLE_SSHBUGS);
      beginbox(&cp, "Detection of known bugs in SSH servers", IDC_BOX_SSHBUGS1);
      staticddl(&cp,
  ...
  if (panel == tunnelspanelstart) {
    ...
    ctlposinit(&cp, hwnd, 80, 3, 13);
    if (dlgtype == 0) {
      bartitle(&cp, "Options controlling SSH tunnelling", IDC_TITLE_TUNNELS);
      ...
      checkbox(&cp,
               ...
               IDC_LPORT_ALL);
      checkbox(&cp, "Remote &ports do the same (SSH v2 only)", IDC_RPORT_ALL);
      staticbtn(
          &cp, "Forwarded ports:", IDC_PFWDSTATIC, "&Remove", IDC_PFWDREMOVE);
      fwdsetter(&cp,
                IDC_PFWDLIST,
                ...
                IDC_PFWDADD);
      bareradioline(
          &cp, 2, "&Local", IDC_PFWDLOCAL, "Re&mote", IDC_PFWDREMOTE, NULL);
      endbox(&cp);
    }
  }
}
...
{
  int n = SendDlgItemMessage(hwnd, IDC_SESSLIST, LB_GETCURSEL, 0, 0);
  int isdef;
  if (n == LB_ERR) {
    MessageBeep(0);
    return 0;
 ...
 */
static int GenericMainDlgProc(
    HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, int dlgtype)
{
  HWND hw, treeview;
    ...
    if (GetWindowLong(hwnd, GWL_USERDATA) == 1)
      ...
      case IDC_PROTRAW:
        if (HIWORD(wParam) == BN_CLICKED ||
          ...
          cfg.protocol =
              i ? PROT_SSH : j ? PROT_TELNET : k ? PROT_RLOGIN : PROT_RAW;
          /*
      ...
      case IDC_HOST:
        if (HIWORD(wParam) == EN_CHANGE)
          GetDlgItemText(hwnd, IDC_HOST, cfg.host, sizeof(cfg.host) - 1);
        break;
      case IDC_PORT:
        if (HIWORD(wParam) == EN_CHANGE) {
      ...
      case IDC_SESSEDIT:
        if (HIWORD(wParam) == EN_CHANGE) {
          SendDlgItemMessage(hwnd, IDC_SESSLIST, LB_SETCURSEL, (WPARAM)-1, 0);
          GetDlgItemText(
              hwnd, IDC_SESSEDIT, savedsession, sizeof(savedsession) - 1);
      ...
      case IDC_SESSSAVE:
        if (HIWORD(wParam) == BN_CLICKED ||
          ...
          get_sesslist(FALSE);
          get_sesslist(TRUE);
          SendDlgItemMessage(hwnd, IDC_SESSLIST, WM_SETREDRAW, FALSE, 0);
          SendDlgItemMessage(hwnd, IDC_SESSLIST, LB_RESETCONTENT, 0, 0);
          for (i = 0; i < nsessions; i++)
            SendDlgItemMessage(
                hwnd, IDC_SESSLIST, LB_ADDSTRING, 0, (LPARAM)(sessions[i]));
          SendDlgItemMessage(hwnd, IDC_SESSLIST, LB_SETCURSEL, (WPARAM)-1, 0);
          SendDlgItemMessage(hwnd, IDC_SESSLIST, WM_SETREDRAW, TRUE, 0);
          InvalidateRect(GetDlgItem(hwnd, IDC_SESSLIST), NULL, TRUE);
        }
        break;
      case IDC_SESSLIST:
      case IDC_SESSLOAD:
        ...
        if (LOWORD(wParam) == IDC_SESSLOAD && HIWORD(wParam) != BN_CLICKED &&
            HIWORD(wParam) != BN_DOUBLECLICKED)
          break;
        if (LOWORD(wParam) == IDC_SESSLIST && HIWORD(wParam) != LBN_DBLCLK)
          break;
        /* Load the session selected in SESSLIST. */
        if (load_selected_session(hwnd) && LOWORD(wParam) == IDC_SESSLIST) {
          /*
      ...
      case IDC_NODELAY:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
          cfg.tcp_nodelay = IsDlgButtonChecked(hwnd, IDC_NODELAY);
        break;
      case IDC_DEL008:
      case IDC_DEL127:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
      ...
      case IDC_NOMOUSEREP:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
          cfg.no_mouse_rep = IsDlgButtonChecked(hwnd, IDC_NOMOUSEREP);
        break;
      case IDC_NORESIZE:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
          cfg.no_remote_resize = IsDlgButtonChecked(hwnd, IDC_NORESIZE);
        break;
      case IDC_NOALTSCREEN:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
          cfg.no_alt_screen = IsDlgButtonChecked(hwnd, IDC_NOALTSCREEN);
        break;
      case IDC_NOWINTITLE:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
          cfg.no_remote_wintitle = IsDlgButtonChecked(hwnd, IDC_NOWINTITLE);
        break;
      case IDC_NODBACKSPACE:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
          cfg.no_dbackspace = IsDlgButtonChecked(hwnd, IDC_NODBACKSPACE);
        break;
      case IDC_NOCHARSET:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
          cfg.no_remote_charset = IsDlgButtonChecked(hwnd, IDC_NOCHARSET);
        break;
      case IDC_ALTF4:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
      ...
      case IDC_ANSWEREDIT:
        if (HIWORD(wParam) == EN_CHANGE)
          GetDlgItemText(
              hwnd, IDC_ANSWEREDIT, cfg.answerback, sizeof(cfg.answerback) - 1);
        break;
      case IDC_ALWAYSONTOP:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
      ...
      case IDC_TELNETKEY:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
          cfg.telnet_keyboard = IsDlgButtonChecked(hwnd, IDC_TELNETKEY);
        break;
      case IDC_TELNETRET:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
          cfg.telnet_newline = IsDlgButtonChecked(hwnd, IDC_TELNETRET);
        break;
      case IDC_WRAPMODE:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
      ...
      case IDC_CHOOSEFONT: {
        HDC hdc = GetDC(0);
        lf.lfHeight =
            -MulDiv(cfg.fontheight, GetDeviceCaps(hdc, LOGPIXELSY), 72);
        ReleaseDC(0, hdc);
      }
      ...
      case IDC_WINBEDIT:
        if (HIWORD(wParam) == EN_CHANGE)
          MyGetDlgItemInt(hwnd, IDC_WINBEDIT, &cfg.window_border);
      ...
      case IDC_RESIZEEITHER:
        if (HIWORD(wParam) == BN_CLICKED ||
            HIWORD(wParam) == BN_DOUBLECLICKED) {
          cfg.resize_action =
              IsDlgButtonChecked(hwnd, IDC_RESIZETERM)
                  ? RESIZE_TERM
                  : IsDlgButtonChecked(hwnd, IDC_RESIZEFONT)
                        ? RESIZE_FONT
                        : IsDlgButtonChecked(hwnd, IDC_RESIZEEITHER)
                              ? RESIZE_EITHER
                              : RESIZE_DISABLED;
        }
        break;
      case IDC_WINEDIT:
        if (HIWORD(wParam) == EN_CHANGE)
          GetDlgItemText(
              hwnd, IDC_WINEDIT, cfg.wintitle, sizeof(cfg.wintitle) - 1);
        break;
      case IDC_COEALWAYS:
      case IDC_COENEVER:
      case IDC_COENORMAL:
        if (HIWORD(wParam) == BN_CLICKED ||
            HIWORD(wParam) == BN_DOUBLECLICKED) {
          cfg.close_on_exit = IsDlgButtonChecked(hwnd, IDC_COEALWAYS)
                                  ? COE_ALWAYS
                                  : IsDlgButtonChecked(hwnd, IDC_COENEVER)
                                        ? COE_NEVER
                                        : COE_NORMAL;
        }
        break;
      ...
      case IDC_TTEDIT:
        if (HIWORD(wParam) == EN_CHANGE)
          GetDlgItemText(
              hwnd, IDC_TTEDIT, cfg.termtype, sizeof(cfg.termtype) - 1);
        break;
      ...
      case IDC_PROXYTYPETELNET:
        if (HIWORD(wParam) == BN_CLICKED ||
            HIWORD(wParam) == BN_DOUBLECLICKED) {
          cfg.proxy_type =
              IsDlgButtonChecked(hwnd, IDC_PROXYTYPEHTTP)
                  ? PROXY_HTTP
                  : IsDlgButtonChecked(hwnd, IDC_PROXYTYPESOCKS)
                        ? PROXY_SOCKS
                        : IsDlgButtonChecked(hwnd, IDC_PROXYTYPETELNET)
                              ? PROXY_TELNET
                              : PROXY_NONE;
        }
        break;
      ...
      case IDC_LGFEDIT:
        if (HIWORD(wParam) == EN_CHANGE)
          GetDlgItemText(
              hwnd, IDC_LGFEDIT, cfg.logfilename, sizeof(cfg.logfilename) - 1);
        break;
      case IDC_LGFBUTTON:
        memset(&of, 0, sizeof(of));
#ifdef OPENFILENAME_SIZE_VERSION_400
      ...
      case IDC_R_TSEDIT:
        if (HIWORD(wParam) == EN_CHANGE)
          GetDlgItemText(
              hwnd, LOWORD(wParam), cfg.termspeed, sizeof(cfg.termspeed) - 1);
        break;
      case IDC_LOGEDIT:
        if (HIWORD(wParam) == EN_CHANGE)
          GetDlgItemText(
              hwnd, IDC_LOGEDIT, cfg.username, sizeof(cfg.username) - 1);
        break;
      case IDC_RLLUSEREDIT:
        if (HIWORD(wParam) == EN_CHANGE)
          GetDlgItemText(hwnd,
      ...
      case IDC_TPASSIVE:
      case IDC_TACTIVE:
        cfg.passive_telnet = IsDlgButtonChecked(hwnd, IDC_TPASSIVE);
        break;
      case IDC_ENVADD:
        if (HIWORD(wParam) == BN_CLICKED ||
            HIWORD(wParam) == BN_DOUBLECLICKED) {
          ...
          *p++ = '\t';
          ...
          if ((p - cfg.environmt) + strlen(str) + 2 < sizeof(cfg.environmt)) {
            strcpy(p, str);
            p[strlen(str) + 1] = '\0';
            SendDlgItemMessage(hwnd, IDC_ENVLIST, LB_ADDSTRING, 0, (LPARAM)str);
      ...
      case IDC_ENVREMOVE:
        ...
        if (i == LB_ERR)
          ...
          SendDlgItemMessage(hwnd, IDC_ENVLIST, LB_DELETESTRING, i, 0);
          p = cfg.environmt;
          while (i > 0) {
            if (!*p)
              goto disaster;
      ...
      case IDC_SSH2DES:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
          cfg.ssh2_des_cbc = IsDlgButtonChecked(hwnd, IDC_SSH2DES);
        break;
      case IDC_AGENTFWD:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
      ...
      case IDC_CHANGEUSER:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
          cfg.change_username = IsDlgButtonChecked(hwnd, IDC_CHANGEUSER);
        break;
      case IDC_CIPHERLIST:
      case IDC_CIPHERUP:
      case IDC_CIPHERDN:
      ...
      case IDC_AUTHKI:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
          cfg.try_ki_auth = IsDlgButtonChecked(hwnd, IDC_AUTHKI);
        break;
      case IDC_PKEDIT:
        if (HIWORD(wParam) == EN_CHANGE)
          GetDlgItemText(
              hwnd, IDC_PKEDIT, cfg.keyfile, sizeof(cfg.keyfile) - 1);
        break;
      case IDC_CMDEDIT:
        if (HIWORD(wParam) == EN_CHANGE)
          GetDlgItemText(
              hwnd, IDC_CMDEDIT, cfg.remote_cmd, sizeof(cfg.remote_cmd) - 1);
        break;
      case IDC_PKBUTTON:
        memset(&of, 0, sizeof(of));
#ifdef OPENFILENAME_SIZE_VERSION_400
      ...
      case IDC_CCSET: {
        ...
        if (!ok)
          ...
          for (i = 0; i < 128; i++)
            if (SendDlgItemMessage(hwnd, IDC_CCLIST, LB_GETSEL, i, 0)) {
              char str[100];
              cfg.wordness[i] = n;
              SendDlgItemMessage(hwnd, IDC_CCLIST, LB_DELETESTRING, i, 0);
              sprintf(str,
                      "%d\t(0x%02X)\t%c\t%d",
                      i,
                      i,
                      ...
                      cfg.wordness[i]);
              SendDlgItemMessage(
                  hwnd, IDC_CCLIST, LB_INSERTSTRING, i, (LPARAM)str);
            }
        }
      } break;
      case IDC_BOLDCOLOUR:
        if (HIWORD(wParam) == BN_CLICKED ||
          ...
          SendDlgItemMessage(hwnd, IDC_COLOURLIST, WM_SETREDRAW, FALSE, 0);
          n = SendDlgItemMessage(hwnd, IDC_COLOURLIST, LB_GETCOUNT, 0, 0);
          ...
          SendDlgItemMessage(hwnd, IDC_COLOURLIST, WM_SETREDRAW, TRUE, 0);
          InvalidateRect(GetDlgItem(hwnd, IDC_COLOURLIST), NULL, TRUE);
        }
        break;
      case IDC_PALETTE:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
      ...
      case IDC_COLOURLIST:
        if (HIWORD(wParam) == LBN_DBLCLK || HIWORD(wParam) == LBN_SELCHANGE) {
          int i = SendDlgItemMessage(hwnd, IDC_COLOURLIST, LB_GETCURSEL, 0, 0);
          if (!cfg.bold_colour)
      ...
      case IDC_CHANGE:
        if (HIWORD(wParam) == BN_CLICKED ||
          ...
          if (ChooseColor(&cc)) {
            cfg.colours[i][0] = (unsigned char)(cc.rgbResult & 0xFF);
            cfg.colours[i][1] = (unsigned char)(cc.rgbResult >> 8) & 0xFF;
            cfg.colours[i][2] = (unsigned char)(cc.rgbResult >> 16) & 0xFF;
            SetDlgItemInt(hwnd, IDC_RVALUE, cfg.colours[i][0], FALSE);
            SetDlgItemInt(hwnd, IDC_GVALUE, cfg.colours[i][1], FALSE);
            SetDlgItemInt(hwnd, IDC_BVALUE, cfg.colours[i][2], FALSE);
          }
      ...
      case IDC_CODEPAGE:
        if (HIWORD(wParam) == CBN_SELCHANGE) {
          int index =
              SendDlgItemMessage(hwnd, IDC_CODEPAGE, CB_GETCURSEL, 0, 0);
          SendDlgItemMessage(hwnd,
                             IDC_CODEPAGE,
                             CB_GETLBTEXT,
                             index,
      ...
      case IDC_PRINTER:
        if (HIWORD(wParam) == CBN_SELCHANGE) {
          int index = SendDlgItemMessage(hwnd, IDC_PRINTER, CB_GETCURSEL, 0, 0);
          SendDlgItemMessage(
              hwnd, IDC_PRINTER, CB_GETLBTEXT, index, (LPARAM)cfg.printer);
        } else if (HIWORD(wParam) == CBN_EDITCHANGE) {
          GetDlgItemText(
              hwnd, IDC_PRINTER, cfg.printer, sizeof(cfg.printer) - 1);
        }
      ...
      case IDC_CAPSLOCKCYR:
        if (HIWORD(wParam) == BN_CLICKED ||
            HIWORD(wParam) == BN_DOUBLECLICKED) {
          cfg.xlat_capslockcyr = IsDlgButtonChecked(hwnd, IDC_CAPSLOCKCYR);
        }
        break;
      case IDC_VTXWINDOWS:
      case IDC_VTOEMANSI:
      ...
      case IDC_VTPOORMAN:
      case IDC_VTUNICODE:
        cfg.vtmode = (IsDlgButtonChecked(hwnd, IDC_VTXWINDOWS)
                          ? VT_XWINDOWS
                          : IsDlgButtonChecked(hwnd, IDC_VTOEMANSI)
                                ? VT_OEMANSI
                                : IsDlgButtonChecked(hwnd, IDC_VTOEMONLY)
                                      ? VT_OEMONLY
                                      : IsDlgButtonChecked(hwnd, IDC_VTUNICODE)
                                            ? VT_UNICODE
                                            : VT_POORMAN);
        break;
      case IDC_X11_FORWARD:
      ...
      case IDC_LPORT_ALL:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
          cfg.lport_acceptall = IsDlgButtonChecked(hwnd, IDC_LPORT_ALL);
        break;
      case IDC_RPORT_ALL:
        if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED)
          cfg.rport_acceptall = IsDlgButtonChecked(hwnd, IDC_RPORT_ALL);
        break;
      case IDC_X11_DISPLAY:
        if (HIWORD(wParam) == EN_CHANGE)
          GetDlgItemText(hwnd,
      ...
      case IDC_PFWDADD:
        if (HIWORD(wParam) == BN_CLICKED ||
          ...
          if (IsDlgButtonChecked(hwnd, IDC_PFWDLOCAL))
            ...
            str[0] = 'R';
          GetDlgItemText(hwnd, IDC_SPORTEDIT, str + 1, sizeof(str) - 2);
          if (!str[1]) {
            MessageBox(hwnd,
          ...
          p = str + strlen(str);
          *p++ = '\t';
          GetDlgItemText(hwnd, IDC_DPORTEDIT, p, sizeof(str) - 1 - (p - str));
          if (!*p || !strchr(p, ':')) {
            MessageBox(hwnd,
          ...
          if ((p - cfg.portfwd) + strlen(str) + 2 < sizeof(cfg.portfwd)) {
            strcpy(p, str);
            p[strlen(str) + 1] = '\0';
            SendDlgItemMessage(
                hwnd, IDC_PFWDLIST, LB_ADDSTRING, 0, (LPARAM)str);
      ...
      case IDC_PFWDREMOVE:
        if (HIWORD(wParam) != BN_CLICKED && HIWORD(wParam) != BN_DOUBLECLICKED)
          break;
        i = SendDlgItemMessage(hwnd, IDC_PFWDLIST, LB_GETCURSEL, 0, 0);
        if (i == LB_ERR)
          MessageBeep(0);
        else {
          char *p, *q;

          SendDlgItemMessage(hwnd, IDC_PFWDLIST, LB_DELETESTRING, i, 0);
          p = cfg.portfwd;
          while (i > 0) {
            if (!*p)
              goto disaster2;
      ...
      case IDC_BUGD_IGNORE1:
        if (HIWORD(wParam) == CBN_SELCHANGE) {
          int index =
              SendDlgItemMessage(hwnd, IDC_BUGD_IGNORE1, CB_GETCURSEL, 0, 0);
          cfg.sshbug_ignore1 =
              (index == 0 ? BUG_AUTO : index == 1 ? BUG_OFF : BUG_ON);
        }
        break;
      case IDC_BUGD_PLAINPW1:
        if (HIWORD(wParam) == CBN_SELCHANGE) {
          int index =
              SendDlgItemMessage(hwnd, IDC_BUGD_PLAINPW1, CB_GETCURSEL, 0, 0);
          cfg.sshbug_plainpw1 =
              (index == 0 ? BUG_AUTO : index == 1 ? BUG_OFF : BUG_ON);
        }
        break;
      case IDC_BUGD_RSA1:
        if (HIWORD(wParam) == CBN_SELCHANGE) {
          int index =
              SendDlgItemMessage(hwnd, IDC_BUGD_RSA1, CB_GETCURSEL, 0, 0);
          cfg.sshbug_rsa1 =
              (index == 0 ? BUG_AUTO : index == 1 ? BUG_OFF : BUG_ON);
        }
        break;
      case IDC_BUGD_HMAC2:
        if (HIWORD(wParam) == CBN_SELCHANGE) {
          int index =
              SendDlgItemMessage(hwnd, IDC_BUGD_HMAC2, CB_GETCURSEL, 0, 0);
          cfg.sshbug_hmac2 =
              (index == 0 ? BUG_AUTO : index == 1 ? BUG_OFF : BUG_ON);
        }
        break;
      case IDC_BUGD_DERIVEKEY2:
        if (HIWORD(wParam) == CBN_SELCHANGE) {
          int index =
              SendDlgItemMessage(hwnd, IDC_BUGD_DERIVEKEY2, CB_GETCURSEL, 0, 0);
          cfg.sshbug_derivekey2 =
              (index == 0 ? BUG_AUTO : index == 1 ? BUG_OFF : BUG_ON);
        }
        break;
      case IDC_BUGD_RSAPAD2:
        if (HIWORD(wParam) == CBN_SELCHANGE) {
          int index =
              SendDlgItemMessage(hwnd, IDC_BUGD_RSAPAD2, CB_GETCURSEL, 0, 0);
          cfg.sshbug_rsapad2 =
              (index == 0 ? BUG_AUTO : index == 1 ? BUG_OFF : BUG_ON);
        }
        break;
      case IDC_BUGD_DHGEX2:
        if (HIWORD(wParam) == CBN_SELCHANGE) {
          int index =
              SendDlgItemMessage(hwnd, IDC_BUGD_DHGEX2, CB_GETCURSEL, 0, 0);
          cfg.sshbug_dhgex2 =
              (index == 0 ? BUG_AUTO : index == 1 ? BUG_OFF : BUG_ON);
        }
        break;
...
static int CALLBACK MainDlgProc(HWND hwnd,
                                UINT msg,
                                WPARAM wParam,
                                LPARAM lParam)
{
...
static int CALLBACK ReconfDlgProc(HWND hwnd,
                                  UINT msg,
                                  WPARAM wParam,
                                  LPARAM lParam)
{
...
void defuse_showwindow(void)
{
   ...
   */
  {
    HWND hwnd;
    hwnd =
        CreateDialog(hinst, MAKEINTRESOURCE(IDD_ABOUTBOX), NULL, NullDlgProc);
...
void logevent(char *string)
{
  ...
  if (logbox) {
    int count;
    SendDlgItemMessage(
        logbox, IDN_LIST, LB_ADDSTRING, 0, (LPARAM)events[nevents]);
    ...
    char *host, int port, char *keytype, char *keystr, char *fingerprint)
{
  ...
  char message[160 +
               ...
               (sizeof(absentmsg) > sizeof(wrongmsg) ? sizeof(absentmsg)
                                                     : sizeof(wrongmsg))];
  ...
  if (ret == 2) { /* key was different */
    ...
    mbret = MessageBox(NULL, message, mbtitle, MB_ICONWARNING | MB_YESNOCANCEL);
    if (mbret == IDYES)
...
{
  static const char mbtitle[] = "PuTTY Security Alert";
  static const char msg[] = "The first %.35scipher supported by the server\n"
  ...
  sprintf(message,
          ...
          ciphername);
  mbret = MessageBox(NULL, message, mbtitle, MB_ICONWARNING | MB_YESNO);
  if (mbret == IDYES)
    return;
  else
    cleanup_exit(0);
...
int askappend(char *filename)
{
  ...
  mbret = MessageBox(NULL, message, mbtitle, MB_ICONQUESTION | MB_YESNOCANCEL);
  if (mbret == IDYES)
    return 2;
  else if (mbret == IDNO)
    return 1;
