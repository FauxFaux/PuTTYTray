...
const char *pfd_newconnect(
    Socket *s, char *hostname, int port, void *c, Conf *conf, int addressfamily)
{
  ...
  pr->s = *s =
      new_connection(addr, dummy_realhost, port, 0, 1, 0, 0, (Plug)pr, conf);
  sfree(dummy_realhost);
 ...
 */
const char *pfd_addforward(char *desthost,
                           int destport,
                           char *srcaddr,
                           int port,
                           void *backhandle,
