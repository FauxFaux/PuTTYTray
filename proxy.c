...
static void plug_proxy_closing(Plug p,
                               const char *error_msg,
                               int error_code,
                               int calling_back)
{
...
static char *dns_log_msg(const char *host,
                         int addressfamily,
                         const char *reason)
{
  return dupprintf("Looking up host \"%s\"%s for %s",
                   host,
...
Socket new_listener(const char *srcaddr,
                    int port,
                    Plug plug,
                    int local_host_only,
                    Conf *conf,
...
int proxy_http_negotiate(Proxy_Socket p, int change)
{
  if (p->state == PROXY_STATE_NEW) {
    ...
    if (username[0] || password[0]) {
      ...
      for (i = 0, j = strlen(buf2); i < len; i += 3, j += 4)
        base64_encode_atom(
            (unsigned char *)(buf + i), (len - i > 3 ? 3 : len - i), buf2 + j);
      strcpy(buf2 + j, "\r\n");
  ...
  if (change == PROXY_CHANGE_ACCEPTING) {
     ...
     * error message?
     */
    return plug_accepting(p->plug, p->accepting_constructor, p->accepting_ctx);
  }
...
int proxy_socks4_negotiate(Proxy_Socket p, int change)
{
  if (p->state == PROXY_CHANGE_NEW) {
    ...
    /* hostname */
    memcpy(command + 8 + strlen(username) + 1, hostname, namelen);
...
int proxy_telnet_negotiate(Proxy_Socket p, int change)
{
  if (p->state == PROXY_CHANGE_NEW) {
    ...
    formatted_cmd =
        format_telnet_command(p->remote_addr, p->remote_port, p->conf);

    {
      ...
      for (in = formatted_cmd, out = reescaped; *in; in++) {
        if (*in == '\n') {
          ...
          *out++ = '\\';
          *out++ = '\\';
        } else if ((unsigned)(((unsigned char)*in) - 0x20) < (0x7F - 0x20)) {
          *out++ = *in;
        } else {
