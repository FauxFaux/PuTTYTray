...
{
  if (o->option == TELOPT_NEW_ENVIRON || o->option == TELOPT_OLD_ENVIRON ||
  ...
  case TELOPT_NEW_ENVIRON:
    ...
    while (n < telnet->sb_len && telnet->sb_buf[n] == var) {
      ...
      sprintf(result,
              "%.*s=%.*s",
              varlen,
              telnet->sb_buf + varpos,
              vallen,
              telnet->sb_buf + valpos);
      if (telnet->shdata.nenvvars >= telnet->envvarsize) {
        telnet->envvarsize = telnet->shdata.nenvvars * 3 / 2 + 16;
        telnet->shdata.envvars =
            sresize(telnet->shdata.envvars, telnet->envvarsize, char *);
      }
