...
static DWORD WINAPI generate_rsa_key_thread(void *param)
{
  ...
  rsa_generate(
      params->key, params->aux, params->keysize, progress_update, &prog);
                                ...
                                LPARAM lParam)
{
  ...
  case WM_INITDIALOG:
    ...
    SetWindowLong(hwnd, GWL_USERDATA, (LONG)state);
    {
      ...
      beginbox(&cp, "Key", IDC_BOX_KEY, IDC_BOXT_KEY);
      cp2 = cp;
      ...
      staticedit(&cp, "Key fingerprint:", IDC_FPSTATIC, IDC_FINGERPRINT, 70);
      SendDlgItemMessage(hwnd, IDC_FINGERPRINT, EM_SETREADONLY, 1, 0);
      staticedit(&cp, "Key &comment:", IDC_COMMENTSTATIC, IDC_COMMENTEDIT, 70);
      staticpassedit(&cp,
                     "Key p&assphrase:",
                     IDC_PASSPHRASE1STATIC,
                     IDC_PASSPHRASE1EDIT,
      ...
      staticpassedit(&cp,
                     ...
                     70);
      endbox(&cp);
      beginbox(&cp, "Actions", IDC_BOX_ACTIONS, IDC_BOXT_ACTIONS);
      staticbtn(&cp,
      ...
      staticbtn(&cp,
                ...
                IDC_SAVE);
      endbox(&cp);
      beginbox(&cp, "Parameters", IDC_BOX_PARAMS, IDC_BOXT_PARAMS);
      staticedit(&cp,
        ...
        if (prompt_keyfile(hwnd, "Load private key:", filename, 0)) {
          ...
          do {
            if (needs_pass) {
              ...
              *passphrase = '\0';
            ret = loadrsakey(filename, &newkey, &newaux, passphrase);
          } while (ret == -1);
          if (comment)
            sfree(comment);
          if (ret == 0) {
