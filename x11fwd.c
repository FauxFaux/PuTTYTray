...
static int dummy_plug_accepting(Plug p, OSSocket sock)
{
  return 1;
}
...
struct X11Display *x11_setup_display(char *display,
                                     int authtype,
                                     const Config *cfg)
{
  ...
  if (!disp->unixdomain) {
    ...
    disp->addr = name_lookup(
        disp->hostname, disp->port, &disp->realhost, cfg, ADDRTYPE_UNSPEC);
  ...
  for (i = 0; i < disp->remoteauthdatalen; i++)
    sprintf(
        disp->remoteauthdatastring + i * 2, "%02x", disp->remoteauthdata[i]);
 ...
 */
extern const char *x11_init(Socket *s,
                            struct X11Display *disp,
                            void *c,
                            const char *peeraddr,
                            int peerport,
...
int x11_send(Socket s, char *data, int len)
{
  ...
  if (!pr->verified) {
     ...
     */
    {
      ...
      if (authstrlen) {
        sk_write(s, x11_authnames[pr->disp->localauthproto], authstrlen);
