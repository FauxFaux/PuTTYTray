...
void urlhack_launch_url(const char* app, const char *url, const int newbrowser )
{
	if (app) {
	...
	if( newbrowser ) 
		...
		ShellExecute(NULL,"open",u.c_str(),NULL,NULL,SW_SHOWNORMAL) ;	// Lance un nouvel onglet dans un browser déjà existant
}
