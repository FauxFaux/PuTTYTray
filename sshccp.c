...
#undef quarter
  ...
  ++ctx->state[12];
  /* Check for overflow, not done in one line so the 32 bits are chopped by the
   * type */
...
static void bigval_import_le(bigval *r, const void *vdata, int len)
{
  ...
  bigval_clear(r);
  for (i = 0; i < len; i++)
    r->w[i / BIGNUM_INT_BYTES] |= (BignumInt)data[i]
...
static void poly_op(void *handle,
                    unsigned char *blk,
                    int len,
                    unsigned long seq)
{
...
static void poly_generate(void *handle,
                          unsigned char *blk,
                          int len,
                          unsigned long seq)
{
...
static int poly_verify(void *handle,
                       unsigned char *blk,
                       int len,
                       unsigned long seq)
{
...
static void ccp_encrypt_length(void *vctx,
                               unsigned char *blk,
                               int len,
                               unsigned long seq)
{
...
static void ccp_decrypt_length(void *vctx,
                               unsigned char *blk,
                               int len,
                               unsigned long seq)
{
...
static const struct ssh2_cipher *const ccp_list[] = {&ssh2_chacha20_poly1305};

const struct ssh2_ciphers ssh2_ccp = {sizeof(ccp_list) / sizeof(*ccp_list),
                                      ccp_list};
