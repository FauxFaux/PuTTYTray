...
GType columns_get_type(void)
{
  ...
  if (!columns_type) {
    ...
    columns_type =
        g_type_register_static(GTK_TYPE_CONTAINER, "Columns", &columns_info, 0);
  }
...
static gint columns_focus(GtkContainer *container, GtkDirectionType dir)
{
  ...
  if (!GTK_WIDGET_DRAWABLE(cols) || !GTK_WIDGET_IS_SENSITIVE(cols))
    return FALSE;
...
static void columns_alloc_horiz(Columns *cols,
                                gint ourwidth,
  ...
  for (children = cols->children; children && (child = children->data);
    ...
    if (!child->widget) {
      ...
      for (i = 0; i < ncols; i++) {
        ...
        colxpos[i + 1] =
            (((ourwidth - 2 * border) + cols->spacing) * percent / 100);
      }
      continue;
    }
...
static void columns_alloc_vert(Columns *cols,
                               gint ourheight,
