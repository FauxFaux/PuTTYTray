...
struct gui_data {
  ...
  GtkWidget *menu, *specialsmenu, *specialsitem1, *specialsitem2, *restartitem;
...
static void update_mouseptr(struct gui_data *inst)
{
  ...
  case BUSY_NOT:
    if (!inst->mouseptr_visible) {
      ...
      gdk_window_set_cursor(gtk_widget_get_window(inst->area), inst->rawcursor);
    } else {
      gdk_window_set_cursor(gtk_widget_get_window(inst->area),
                            inst->textcursor);
    }
  ...
  case BUSY_CPU:
    ...
    gdk_window_set_cursor(gtk_widget_get_window(inst->area), inst->waitcursor);
    break;
  default:
    assert(0);
  }
...
gint configure_area(GtkWidget *widget, GdkEventConfigure *event, gpointer data)
{
...

  {
    ...
    inst->pixmap =
        gdk_pixmap_new(gtk_widget_get_window(widget), backing_w, backing_h, -1);
#endif
    ...
    inst->surface =
        cairo_image_surface_create(CAIRO_FORMAT_RGB24, backing_w, backing_h);
#endif
  }
...
{
  cairo_get_matrix(dctx->uctx.u.cairo.cr, &dctx->uctx.u.cairo.origmatrix);
...
static gint draw_area(GtkWidget *widget, cairo_t *cr, gpointer data)
{
  ...
  if (inst->surface) {
    ...
    cairo_rectangle(
        cr, dirtyrect.x, dirtyrect.y, dirtyrect.width, dirtyrect.height);
    cairo_fill(cr);
  }
...
#define KEY_PRESSED(k) (inst->keystate[(k) / 32] & (1 << ((k) % 32)))
...
gint key_event(GtkWidget *widget, GdkEventKey *event, gpointer data)
{
...
#ifdef KEY_EVENT_DIAGNOSTICS
  {
    ...
    type_string = (event->type == GDK_KEY_PRESS
                       ? dupstr("PRESS")
...

    {
      ...
      if (val || !state_string[0]) {
        char *old = state_string;
        state_string =
            dupprintf("%s%s%d", state_string, state_string[0] ? "|" : "", val);
        sfree(old);
      }
    }
    ...
    string_string = dupstr("");
    {
      ...
      for (i = 0; event->string[i]; i++) {
        ...
        string_string = dupprintf("%s%s%02x",
                                  string_string,
                                  string_string[0] ? " " : "",
  ...
  if (event->type == GDK_KEY_RELEASE) {
    if ((event->keyval == GDK_KEY_Meta_L || event->keyval == GDK_KEY_Meta_R ||
  ...
  if (event->type == GDK_KEY_PRESS) {
    ...
    if (event->keyval == GDK_KEY_greater && (event->state & GDK_CONTROL_MASK)) {
#ifdef KEY_EVENT_DIAGNOSTICS
    ...
    if (event->keyval == GDK_KEY_less && (event->state & GDK_CONTROL_MASK)) {
#ifdef KEY_EVENT_DIAGNOSTICS
      debug((" - Ctrl-<: increase font size\n"));
#endif
      change_font_size(inst, -1);
    ...
    if (event->keyval == GDK_KEY_Page_Up && (event->state & GDK_CONTROL_MASK)) {
#ifdef KEY_EVENT_DIAGNOSTICS
    ...
    if (app_keypad_mode &&
        (event->keyval == GDK_KEY_Num_Lock ||
         ...
         event->keyval == GDK_KEY_KP_Add || event->keyval == GDK_KEY_KP_Enter ||
         event->keyval == GDK_KEY_KP_0 || event->keyval == GDK_KEY_KP_Insert ||
         event->keyval == GDK_KEY_KP_1 || event->keyval == GDK_KEY_KP_End ||
         event->keyval == GDK_KEY_KP_2 || event->keyval == GDK_KEY_KP_Down ||
         event->keyval == GDK_KEY_KP_3 ||
         event->keyval == GDK_KEY_KP_Page_Down ||
         event->keyval == GDK_KEY_KP_4 || event->keyval == GDK_KEY_KP_Left ||
         event->keyval == GDK_KEY_KP_5 || event->keyval == GDK_KEY_KP_Begin ||
         event->keyval == GDK_KEY_KP_6 || event->keyval == GDK_KEY_KP_Right ||
         event->keyval == GDK_KEY_KP_7 || event->keyval == GDK_KEY_KP_Home ||
         event->keyval == GDK_KEY_KP_8 || event->keyval == GDK_KEY_KP_Up ||
         event->keyval == GDK_KEY_KP_9 || event->keyval == GDK_KEY_KP_Page_Up ||
         event->keyval == GDK_KEY_KP_Decimal ||
         event->keyval == GDK_KEY_KP_Delete)) {
      /* app keypad; do nothing */
    } else if (nethack_mode && (event->keyval == GDK_KEY_KP_1 ||
    ...
    output_charset = CS_UTF8;
    {
      ...
      wlen = mb_to_wc(DEFAULT_CODEPAGE,
                      0,
                      event->string,
                      strlen(event->string),
                      widedata,
...
#ifdef KEY_EVENT_DIAGNOSTICS
      {
        ...
        for (i = 0; i < wlen; i++) {
          char *old = string_string;
          string_string = dupprintf("%s%s%04x",
                                    string_string,
                                    string_string[0] ? " " : "",
                                    (unsigned)widedata[i]);
          sfree(old);
      ...
      wp = widedata;
      ulen = charset_from_unicode(
          &wp, &wlen, output + 1, lenof(output) - 2, CS_UTF8, NULL, NULL, 0);
    ...
    if (!output[1] && (ucsval = keysym_to_unicode(event->keyval)) >= 0) {
      ucsoutput[0] = '\033';
      ucsoutput[1] = ucsval;
#ifdef KEY_EVENT_DIAGNOSTICS
    ...
    if (event->keyval == GDK_KEY_Break && (event->state & GDK_CONTROL_MASK)) {
#ifdef KEY_EVENT_DIAGNOSTICS
    ...
    if (event->keyval == GDK_KEY_BackSpace &&
      ...
      output[1] =
          conf_get_int(inst->conf, CONF_bksp_is_delete) ? '\x7F' : '\x08';
#ifdef KEY_EVENT_DIAGNOSTICS
    ...
    if (event->keyval == GDK_KEY_BackSpace && (event->state & GDK_SHIFT_MASK)) {
      output[1] =
          conf_get_int(inst->conf, CONF_bksp_is_delete) ? '\x08' : '\x7F';
#ifdef KEY_EVENT_DIAGNOSTICS
     ...
     */
    {
      ...
      if ((inst->term->vt52_mode || funky_type == FUNKY_VT100P) && code >= 11 &&
          code <= 24) {
        int offt = 0;
        if (code > 15)
          offt++;
...
#endif
          end = 1 + sprintf(output + 1, "\x1B%c", code + 'P' - 11 - offt);
        } else {
#ifdef KEY_EVENT_DIAGNOSTICS
          debug((" - VT100+ mode function key"));
#endif
          end = 1 + sprintf(output + 1, "\x1BO%c", code + 'P' - 11 - offt);
        }
        use_ucsoutput = FALSE;
        goto done;
      }
     ...
     */
    {
      ...
      if (xkey) {
        end = 1 + format_arrow_key(output + 1,
                                   inst->term,
                                   xkey,
                                   event->state & GDK_CONTROL_MASK);
#ifdef KEY_EVENT_DIAGNOSTICS
  ...
  if (end - start > 0) {
    if (special) {
      ...
      debug((" - final output, special, generic encoding = [%s]\n",
             charset_to_localenc(output_charset),
             string_string));
      sfree(string_string);
#endif
      /*
      ...
      if (!use_ucsoutput) {
        ...
        if (inst->ldisc)
          lpage_send(
              inst->ldisc, output_charset, output + start, end - start, 1);
      } else {
#ifdef KEY_EVENT_DIAGNOSTICS
        char *string_string = dupstr("");
        int i;
...
gboolean scroll_internal(
    struct gui_data *inst, gdouble delta, guint state, gdouble ex, gdouble ey)
{
  ...
  raw_mouse_mode = send_raw_mouse &&
                   !(shift && conf_get_int(inst->conf, CONF_mouse_override));
  ...
  if (!raw_mouse_mode) {
    ...
    int scroll_events = (int)(inst->cumulative_scroll / SCROLL_INCREMENT_LINES);
    if (scroll_events) {
      int button;
...
gboolean button_internal(struct gui_data *inst,
                         guint32 timestamp,
                         GdkEventType type,
                         guint ebutton,
                         guint state,
                         ...
                         gdouble ey)
{
  ...
  if (ebutton == 3 && ctrl) {
    gtk_menu_popup(
        GTK_MENU(inst->menu), NULL, NULL, NULL, NULL, ebutton, timestamp);
    return TRUE;
  }
  ...
  case GDK_3BUTTON_PRESS:
    ...
    break;
  default:
    return FALSE; /* don't know this event type */
  }
...
#if GTK_CHECK_VERSION(2, 0, 0)
static void compute_whole_window_size(
    struct gui_data *inst, int wchars, int hchars, int *wpix, int *hpix);
#endif
...
#if GTK_CHECK_VERSION(2, 0, 0)
  gtk_window_resize(
      GTK_WINDOW(inst->window), area_x + offset_x, area_y + offset_y);
#else
...
static void clipboard_provide_data(GtkClipboard *clipboard,
                                   ...
                                   gpointer data)
{
  struct clipboard_data_instance *cdi = (struct clipboard_data_instance *)data;
  struct gui_data *inst = cdi->inst;

  if (inst->current_cdi == cdi) {
    gtk_selection_data_set_text(
        selection_data, cdi->pasteout_data_utf8, cdi->pasteout_data_utf8_len);
  }
}
    ...
    void *frontend, wchar_t *data, int *attr, int len, int must_deselect)
{
  ...
  cdi->pasteout_data_utf8 = snewn(len * 6, char);
  {
    ...
    int tmplen = len;
    cdi->pasteout_data_utf8_len = charset_from_unicode(&tmp,
                                                       &tmplen,
                                                       cdi->pasteout_data_utf8,
                                                       len * 6,
                                                       CS_UTF8,
                                                       NULL,
                                                       NULL,
   ...
   */
  {
    ...
    gtk_clipboard_set_with_data(inst->clipboard,
                                targettable,
                                n_targets,
                                    ...
                                    gpointer data)
{
  ...
  inst->pastein_data_len =
      mb_to_wc(CS_UTF8, 0, text, length, inst->pastein_data, length);
    ...
    void *frontend, wchar_t *data, int *attr, int len, int must_deselect)
{
  ...
  if (!inst->direct_to_font) {
    ...
    if (inst->pasteout_data_utf8_len == 0) {
      ...
      inst->pasteout_data_utf8 = sresize(
          inst->pasteout_data_utf8, inst->pasteout_data_utf8_len + 1, char);
    ...
    if (Xutf8TextListToTextProperty(disp, list, 1, XCompoundTextStyle, &tp) ==
        0) {
  ...
  inst->pasteout_data_len = wc_to_mb(inst->ucsdata.line_codepage,
                                     0,
                                     data,
                                     len,
                                     inst->pasteout_data,
  ...
  if (gtk_selection_owner_set(
          inst->area, GDK_SELECTION_PRIMARY, inst->input_event_time)) {
#if GTK_CHECK_VERSION(2, 0, 0)
    gtk_selection_clear_targets(inst->area, GDK_SELECTION_PRIMARY);
#endif
    gtk_selection_add_target(
        inst->area, GDK_SELECTION_PRIMARY, GDK_SELECTION_TYPE_STRING, 1);
    if (inst->pasteout_data_ctext)
      gtk_selection_add_target(
          inst->area, GDK_SELECTION_PRIMARY, compound_text_atom, 1);
    if (inst->pasteout_data_utf8)
      gtk_selection_add_target(
          inst->area, GDK_SELECTION_PRIMARY, utf8_string_atom, 1);
  }
...
void request_paste(void *frontend)
{
  ...
  if (!inst->direct_to_font) {
     ...
     */
    gtk_selection_convert(inst->area,
                               ...
                               gpointer data)
{
   ...
   */
  if (seldata_length > 0 && seldata_type != GDK_SELECTION_TYPE_STRING &&
      seldata_type != compound_text_atom && seldata_type != utf8_string_atom)
    return;
  ...
  if (seldata_length <= 0) {
    ...
    if (seldata_type == compound_text_atom) {
      ...
      charset =
          (seldata_type == utf8_string_atom ? CS_UTF8
                                            : inst->ucsdata.line_codepage);
    }
  }
  ...
  inst->pastein_data_len = mb_to_wc(
...
void init_clipboard(struct gui_data *inst)
{
  ...
  g_signal_connect(G_OBJECT(inst->area),
                   "selection_received",
                   G_CALLBACK(selection_received),
                   inst);
  g_signal_connect(
      G_OBJECT(inst->area), "selection_get", G_CALLBACK(selection_get), inst);
  g_signal_connect(G_OBJECT(inst->area),
...
static void draw_rectangle(
    struct draw_ctx *dctx, int filled, int x, int y, int w, int h)
{
#ifdef DRAW_TEXT_GDK
  if (dctx->uctx.type == DRAWTYPE_GDK) {
    gdk_draw_rectangle(
        dctx->uctx.u.gdk.target, dctx->uctx.u.gdk.gc, filled, x, y, w, h);
  }
#endif
#ifdef DRAW_TEXT_CAIRO
  if (dctx->uctx.type == DRAWTYPE_CAIRO) {
    ...
    if (filled) {
      ...
      cairo_rectangle(dctx->uctx.u.cairo.cr, x + 0.5, y + 0.5, w, h);
...
#ifdef DRAW_TEXT_GDK
  if (dctx->uctx.type == DRAWTYPE_GDK) {
    gdk_draw_line(dctx->uctx.u.gdk.target, dctx->uctx.u.gdk.gc, x0, y0, x1, y1);
  }
#endif
#ifdef DRAW_TEXT_CAIRO
  if (dctx->uctx.type == DRAWTYPE_CAIRO) {
...
static void draw_stretch_before(struct draw_ctx *dctx,
                                int x,
                                int y,
                                int w,
                                int wdouble,
...
static void draw_stretch_after(struct draw_ctx *dctx,
                               int x,
                               int y,
                               int w,
                               int wdouble,
...
#ifdef DRAW_TEXT_CAIRO
  if (dctx->uctx.type == DRAWTYPE_CAIRO) {
    cairo_set_matrix(dctx->uctx.u.cairo.cr, &dctx->uctx.u.cairo.origmatrix);
  }
#endif
}
                      ...
                      int lattr)
{
  ...
  int monochrome = gdk_visual_get_depth(gtk_widget_get_visual(inst->area)) == 1;
  ...
  if (attr & ATTR_UNDER) {
    ...
    draw_line(dctx,
              ...
              (x + len) * widefactor * inst->font_width - 1 +
                  inst->window_border,
              y * inst->font_height + uheight + inst->window_border);
  }
             ...
             int lattr)
{
  ...
  draw_update(dctx,
              x * inst->font_width + inst->window_border,
              y * inst->font_height + inst->window_border,
              len * widefactor * inst->font_width,
              inst->font_height);
}
...
char *setup_fonts_ucs(struct gui_data *inst)
{
  ...
  fonts[0] = multifont_create(
      inst->area, fs->name, FALSE, FALSE, shadowboldoffset, shadowbold);
  if (!fonts[0]) {
  ...
  if (shadowbold || !fs->name[0]) {
    ...
    fonts[1] = multifont_create(
        inst->area, fs->name, FALSE, TRUE, shadowboldoffset, shadowbold);
    if (!fonts[1]) {
      if (fonts[0])
  ...
  if (fs->name[0]) {
    fonts[2] = multifont_create(
        inst->area, fs->name, TRUE, FALSE, shadowboldoffset, shadowbold);
    if (!fonts[2]) {
      for (i = 0; i < 2; i++)
        if (fonts[i])
  ...
  if (shadowbold || !fs->name[0]) {
    ...
    fonts[3] = multifont_create(
        inst->area, fs->name, TRUE, TRUE, shadowboldoffset, shadowbold);
    if (!fonts[3]) {
      for (i = 0; i < 3; i++)
        if (fonts[i])
          unifont_destroy(fonts[i]);
...
static void compute_geom_hints(struct gui_data *inst, GdkGeometry *geom)
{
   ...
   */
  {
    ...
    ctx->area = inst->area;
    ctx->menubar = NULL;
    gtk_container_foreach(GTK_CONTAINER(inst->window), find_app_menu_bar, ctx);
    ...
    if (ctx->menubar) {
      ...
      min_menu_width -= ((min_menu_width - geom->base_width % geom->width_inc) %
                         geom->width_inc);
...
#endif
  gtk_window_set_geometry_hints(GTK_WINDOW(inst->window), NULL, &geom, flags);
}
    ...
    struct gui_data *inst, int wchars, int hchars, int *wpix, int *hpix)
{
  ...
  compute_geom_hints(inst, &geom);
  if (wpix)
    *wpix = geom.base_width + wchars * geom.width_inc;
  if (hpix)
    *hpix = geom.base_height + hchars * geom.height_inc;
}
#endif
...
{
  struct gui_data *inst = (struct gui_data *)data;
  int code = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(item), "user-data"));
...
void change_settings_menuitem(GtkMenuItem *item, gpointer data)
{
  ...
  if (do_config_box(title,
    ...
    if (conf_get_int(oldconf, CONF_scrollbar_on_left) !=
      ...
      gtk_box_reorder_child(inst->hbox,
                            inst->sbar,
                            conf_get_int(newconf, CONF_scrollbar_on_left) ? 0
                                                                          : 1);
    }
    ...
    if (strcmp(conf_get_fontspec(oldconf, CONF_font)->name,
      ...
      if (errmsg) {
        char *msgboxtext = dupprintf(
            "Could not change fonts in terminal window: %s\n", errmsg);
        messagebox(inst->window,
                   "Font setup error",
                   msgboxtext,
...
static void change_font_size(struct gui_data *inst, int increment)
{
  ...
  for (i = 0; i < lenof(newfonts); i++) {
    if (newfonts[i]) {
      oldfonts[i] = fontspec_copy(conf_get_fontspec(inst->conf, conf_keys[i]));
  ...
  set_geom_hints(inst);
  request_resize(inst,
                 conf_get_int(inst->conf, CONF_width),
...
static void update_savedsess_menu(GtkMenuItem *menuitem, gpointer data)
{
  ...
  for (i = 1; i < sesslist.nsessions; i++) {
    ...
    g_object_set_data(
        G_OBJECT(menuitem), "user-data", dupstr(sesslist.sessions[i]));
  ...
  if (sesslist.nsessions <= 1) {
    GtkWidget *menuitem = gtk_menu_item_new_with_label("(No sessions)");
                     ...
                     int n_icon)
{
  ...
  iconpm = gdk_pixmap_create_from_xpm_d(
      gtk_widget_get_window(window), &iconmask, NULL, (gchar **)icon[0]);
...
void update_specials_menu(void *frontend)
{
  ...
  gtk_container_foreach(
      GTK_CONTAINER(inst->specialsmenu), (GtkCallback)gtk_widget_destroy, NULL);
  if (specials) {
    int i;
    ...
    for (i = 0; nesting > 0; i++) {
      ...
      default:
        menuitem = gtk_menu_item_new_with_label(specials[i].name);
        g_object_set_data(
            G_OBJECT(menuitem), "user-data", GINT_TO_POINTER(specials[i].code));
        g_signal_connect(
            G_OBJECT(menuitem), "activate", G_CALLBACK(special_menuitem), inst);
        break;
      }
      if (menuitem) {
...
static void start_backend(struct gui_data *inst)
{
  ...
  error = inst->back->init((void *)inst,
                           &inst->backhandle,
                           inst->conf,
  ...
  inst->ldisc =
      ldisc_create(inst->conf, inst->term, inst->back, inst->backhandle, inst);
...
struct gui_data *new_session_window(Conf *conf, const char *geometry_string)
{
  ...
  if (geometry_string) {
    ...
    if (flags & (XValue | YValue)) {
      ...
      inst->gotpos = TRUE;
      inst->gravity =
          ((flags & XNegative ? 1 : 0) | (flags & YNegative ? 2 : 0));
    }
  }
#endif
  ...
  inst->window = make_gtk_toplevel_window(inst);
  {
    ...
    if (*winclass)
      gtk_window_set_wmclass(GTK_WINDOW(inst->window), winclass, winclass);
  }
  ...
  if (inst->gotpos) {
    ...
    compute_whole_window_size(inst, inst->width, inst->height, &wp, &hp);
    if (inst->gravity & 1)
      x += (gdk_screen_width() - wp);
    if (inst->gravity & 2)
      y += (gdk_screen_height() - hp);
  ...
  g_signal_connect(
      G_OBJECT(inst->window), "destroy", G_CALLBACK(destroy), inst);
  g_signal_connect(
      G_OBJECT(inst->window), "delete_event", G_CALLBACK(delete_window), inst);
  g_signal_connect(
      G_OBJECT(inst->window), "key_press_event", G_CALLBACK(key_event), inst);
  g_signal_connect(
      G_OBJECT(inst->window), "key_release_event", G_CALLBACK(key_event), inst);
  g_signal_connect(
      G_OBJECT(inst->window), "focus_in_event", G_CALLBACK(focus_event), inst);
  g_signal_connect(
      G_OBJECT(inst->window), "focus_out_event", G_CALLBACK(focus_event), inst);
  g_signal_connect(G_OBJECT(inst->area),
                   "configure_event",
                   G_CALLBACK(configure_area),
                   inst);
#if GTK_CHECK_VERSION(3, 0, 0)
  g_signal_connect(G_OBJECT(inst->area), "draw", G_CALLBACK(draw_area), inst);
#else
  g_signal_connect(
      G_OBJECT(inst->area), "expose_event", G_CALLBACK(expose_area), inst);
#endif
  g_signal_connect(G_OBJECT(inst->area),
                   "button_press_event",
...
#if GTK_CHECK_VERSION(2, 0, 0)
  g_signal_connect(
      G_OBJECT(inst->area), "scroll_event", G_CALLBACK(scroll_event), inst);
#endif
  g_signal_connect(G_OBJECT(inst->area),
                   "motion_notify_event",
                   ...
                   inst);
#if GTK_CHECK_VERSION(2, 0, 0)
  g_signal_connect(G_OBJECT(inst->imc),
                   "commit",
                   G_CALLBACK(input_method_commit_event),
                   inst);
#endif
  if (conf_get_int(inst->conf, CONF_scrollbar))
    g_signal_connect(G_OBJECT(inst->sbar_adjust),
                     "value_changed",
                     G_CALLBACK(scrollbar_moved),
                     inst);
...
#define MKMENUITEM(title, func)                                                \
  do {                                                                         \
    menuitem = gtk_menu_item_new_with_label(title);                            \
    gtk_container_add(GTK_CONTAINER(inst->menu), menuitem);                    \
    gtk_widget_show(menuitem);                                                 \
    g_signal_connect(G_OBJECT(menuitem), "activate", G_CALLBACK(func), inst);  \
  } while (0)

#define MKSUBMENU(title)                                                       \
  do {                                                                         \
    menuitem = gtk_menu_item_new_with_label(title);                            \
    gtk_container_add(GTK_CONTAINER(inst->menu), menuitem);                    \
    gtk_widget_show(menuitem);                                                 \
