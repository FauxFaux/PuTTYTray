...
static gint timer_trigger(gpointer data)
{
  ...
  if (run_timers(now, &next)) {
    ...
    timer_id = gtk_timeout_add(
        ticks > 0 ? ticks : 1, timer_trigger, LONG_TO_GPOINTER(next));
  }
  ...
  for (i = 0; i < NALLCOLOURS; i++) {
    if (!success[i])
      g_error("%s: couldn't allocate colour %d (#%02x%02x%02x)\n",
              appname,
              i,
              inst->cfg.colours[i][0],
              inst->cfg.colours[i][1],
                      ...
                      int lattr)
{
  ...
  gdk_gc_set_foreground(gc, &inst->cols[nfg]);
  {
    ...
    for (combining = 0; combining < ncombining; combining++) {
      int mblen = wc_to_mb(inst->fonts[fontid]->real_charset,
                           0,
                           text + combining,
                           len,
                           gcs,
...
#define SECOND_PASS_ONLY                                                       \
  ...
  while (--argc > 0) {
    ...
    ret = cmdline_process_param(
        p, (argc > 1 ? argv[1] : NULL), do_everything ? 1 : -1, cfg);
...
void setup_fonts_ucs(struct gui_data *inst)
{
  ...
  inst->fonts[0] = unifont_create(inst->area,
                                  inst->cfg.font.name,
                                  FALSE,
                                  FALSE,
  ...
  if (inst->cfg.shadowbold || !inst->cfg.boldfont.name[0]) {
    ...
    inst->fonts[1] = unifont_create(inst->area,
  ...
  if (inst->cfg.widefont.name[0]) {
    inst->fonts[2] = unifont_create(inst->area,
  ...
  if (do_config_box(title,
    ...
    if (oldcfg.scrollbar_on_left != cfg2.scrollbar_on_left) {
      gtk_box_reorder_child(
          inst->hbox, inst->sbar, cfg2.scrollbar_on_left ? 0 : 1);
    }
    ...
    if (strcmp(oldcfg.font.name, cfg2.font.name) ||
        ...
        oldcfg.vtmode != cfg2.vtmode || oldcfg.shadowbold != cfg2.shadowbold) {
    ...
    if (oldcfg.width != cfg2.width || oldcfg.height != cfg2.height ||
      ...
      if (oldcfg.savelines != cfg2.savelines)
        term_size(
            inst->term, inst->term->rows, inst->term->cols, cfg2.savelines);
    }
...
int pt_main(int argc, char **argv)
{
  ...
  if (inst->cfg.winclass[0])
    gtk_window_set_wmclass(
        GTK_WINDOW(inst->window), inst->cfg.winclass, inst->cfg.winclass);
