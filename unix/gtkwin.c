...
gint configure_area(GtkWidget *widget, GdkEventConfigure *event, gpointer data)
{
...

  {
    ...
    inst->surface =
        cairo_image_surface_create(CAIRO_FORMAT_ARGB32, backing_w, backing_h);
#endif
  }
...
#else
    gtk_menu_popup(GTK_MENU(inst->menu),
                   NULL,
                   NULL,
                   NULL,
                   NULL,
...
struct gui_data *new_session_window(Conf *conf, const char *geometry_string)
{
  ...
  if (inst->gotpos) {
    ...
    if (inst->gravity & 1)
      x += (monitor_geometry.width - wp);
    if (inst->gravity & 2)
      y += (monitor_geometry.height - hp);
