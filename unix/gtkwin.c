...
void draw_backing_rect(struct gui_data *inst)
{
  ...
  gdk_draw_rectangle(inst->pixmap,
                     gc,
                     1,
                     0,
                     0,
                     inst->width * inst->font_width + 2 * inst->window_border,
                     inst->height * inst->font_height +
                         2 * inst->window_border);
  gdk_gc_unref(gc);
}
...
gint configure_area(GtkWidget *widget, GdkEventConfigure *event, gpointer data)
{
  ...
  inst->pixmap =
      gdk_pixmap_new(widget->window,
                     ...
                     (h * inst->font_height + 2 * inst->window_border),
                     -1);
...
gint key_event(GtkWidget *widget, GdkEventKey *event, gpointer data)
{
  ...
  if (event->type == GDK_KEY_PRESS) {
...
#ifdef KEY_DEBUGGING
        printf("Adding digit %d to keycode %d", digit, inst->alt_keycode);
#endif
     ...
     */
    if (app_keypad_mode &&
        (event->keyval == GDK_Num_Lock || event->keyval == GDK_KP_Divide ||
         event->keyval == GDK_KP_Multiply || event->keyval == GDK_KP_Subtract ||
         event->keyval == GDK_KP_Add || event->keyval == GDK_KP_Enter ||
         event->keyval == GDK_KP_0 || event->keyval == GDK_KP_Insert ||
         event->keyval == GDK_KP_1 || event->keyval == GDK_KP_End ||
         event->keyval == GDK_KP_2 || event->keyval == GDK_KP_Down ||
         event->keyval == GDK_KP_3 || event->keyval == GDK_KP_Page_Down ||
         event->keyval == GDK_KP_4 || event->keyval == GDK_KP_Left ||
         event->keyval == GDK_KP_5 || event->keyval == GDK_KP_Begin ||
         event->keyval == GDK_KP_6 || event->keyval == GDK_KP_Right ||
         event->keyval == GDK_KP_7 || event->keyval == GDK_KP_Home ||
         event->keyval == GDK_KP_8 || event->keyval == GDK_KP_Up ||
         event->keyval == GDK_KP_9 || event->keyval == GDK_KP_Page_Up ||
         event->keyval == GDK_KP_Decimal || event->keyval == GDK_KP_Delete)) {
    ...
    if (event->keyval == GDK_BackSpace && !(event->state & GDK_SHIFT_MASK)) {
      output[1] =
...
static gint timer_trigger(gpointer data)
{
  ...
  if (run_timers(now, &next) && !timer_id) {
    ...
    if (now - then > next - then)
      ...
      ticks = next - now;
    timer_id = gtk_timeout_add(ticks, timer_trigger, LONG_TO_GPOINTER(next));
  }
...
void request_resize(void *frontend, int w, int h)
{
  ...
  gdk_window_resize(inst->window->window, area_x + offset_x, area_y + offset_y);
#endif
}
...
void write_clip(Terminal *terminal,
                void *frontend,
                wchar_t *data,
                int *attr,
                int len,
                int must_deselect)
{
  ...
  if (!inst->direct_to_font) {
    ...
    if (Xutf8TextListToTextProperty(
            GDK_DISPLAY(), list, 1, XCompoundTextStyle, &tp) == 0) {
                        ...
                        gpointer data)
{
   ...
   */
  if (seldata->length > 0 && seldata->type != GDK_SELECTION_TYPE_STRING &&
      seldata->type != compound_text_atom && seldata->type != utf8_string_atom)
    return;
  ...
  if (seldata->length <= 0) {
    ...
    if (seldata->type == compound_text_atom) {
      ...
      charset =
          (seldata->type == utf8_string_atom ? CS_UTF8
  ...
  if ((lattr & LATTR_MODE) != LATTR_NORM) {
    ...
    for (i = 0; i < len * widefactor * inst->font_width; i++) {
      gdk_draw_pixmap(inst->pixmap,
                      ...
                      y * inst->font_height + inst->window_border,
                      len * widefactor * inst->font_width - i,
                      inst->font_height);
    }
    len *= 2;
             ...
             int lattr)
{
  ...
  gdk_draw_pixmap(inst->area->window,
                  gc,
                  inst->pixmap,
                  x * inst->font_width + inst->window_border,
                  y * inst->font_height + inst->window_border,
...
#define SECOND_PASS_ONLY                                                       \
  ...
  while (--argc > 0) {
    ...
    ret = cmdline_process_param(
        p, (argc > 1 ? argv[1] : NULL), do_everything ? 1 : -1, conf);
...
void special_menuitem(GtkMenuItem *item, gpointer data)
{
  struct gui_data *inst = (struct gui_data *)data;
  int code =
      GPOINTER_TO_INT(gtk_object_get_data(GTK_OBJECT(item), "user-data"));
  ...
  if (specials) {
    ...
    for (i = 0; nesting > 0; i++) {
      ...
      default:
        ...
        gtk_signal_connect(GTK_OBJECT(menuitem),
                           "activate",
                           GTK_SIGNAL_FUNC(special_menuitem),
                           inst);
        break;
      }
      if (menuitem) {
...
int pt_main(int argc, char **argv)
{
  ...
  if (argc > 1 && !strncmp(argv[1], "---", 3)) {
    ...
    if ((!allow_launch || !conf_launchable(inst->conf)) && !cfgbox(inst->conf))
  ...
  gtk_signal_connect(
      GTK_OBJECT(inst->window), "destroy", GTK_SIGNAL_FUNC(destroy), inst);
  gtk_signal_connect(GTK_OBJECT(inst->window),
                     "delete_event",
  ...
  gtk_signal_connect(GTK_OBJECT(inst->window),
                     "focus_in_event",
                     GTK_SIGNAL_FUNC(focus_event),
                     inst);
  gtk_signal_connect(GTK_OBJECT(inst->window),
                     "focus_out_event",
                     GTK_SIGNAL_FUNC(focus_event),
                     inst);
  gtk_signal_connect(GTK_OBJECT(inst->area),
                     "configure_event",
...
#endif
  if (conf_get_int(inst->conf, CONF_scrollbar))
    gtk_signal_connect(GTK_OBJECT(inst->sbar_adjust),
                       "value_changed",
...
#define MKMENUITEM(title, func)                                                \
  do {                                                                         \
    ...
    gtk_widget_show(menuitem);                                                 \
    gtk_signal_connect(                                                        \
        GTK_OBJECT(menuitem), "activate", GTK_SIGNAL_FUNC(func), inst);        \
  } while (0)
