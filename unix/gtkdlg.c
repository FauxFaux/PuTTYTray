...
struct uctrl {
  ...
  GtkWidget *toplevel;
  GtkWidget **buttons;
  int nbuttons;      /* for radio buttons */
 ...
 * Forward references.
 */
static gboolean widget_focus(GtkWidget *widget,
                             GdkEventFocus *event,
                             gpointer data);
static void shortcut_add(
    struct Shortcuts *scs, GtkWidget *labelw, int chr, int action, void *ptr);
static void shortcut_highlight(GtkWidget *label, int chr);
#if !GTK_CHECK_VERSION(2, 0, 0)
static gboolean listitem_single_key(GtkWidget *item,
                                    GdkEventKey *event,
                                    gpointer data);
static gboolean listitem_multi_key(GtkWidget *item,
                                   GdkEventKey *event,
                                   gpointer data);
static gboolean listitem_button_press(GtkWidget *item,
                                      GdkEventButton *event,
                                      gpointer data);
static gboolean listitem_button_release(GtkWidget *item,
                                        GdkEventButton *event,
                                        gpointer data);
#endif
#if !GTK_CHECK_VERSION(2, 4, 0)
                           ...
                           int id)
{
  ...
  if (uc->menu) {
    ...
    g_object_set_data(G_OBJECT(menuitem), "user-data", GINT_TO_POINTER(id));
    g_signal_connect(
        G_OBJECT(menuitem), "activate", G_CALLBACK(menuitem_activate), dp);
    goto done;
  }
  if (uc->list && uc->entry) {
    /*
    ...
    g_object_set_data(G_OBJECT(listitem), "user-data", GINT_TO_POINTER(id));
    goto done;
  }
#endif
#if !GTK_CHECK_VERSION(2, 0, 0)
  if (uc->list) {
    ...
    assert(ncols <= (uc->ctrl->listbox.ncols ? uc->ctrl->listbox.ncols : 1));
    ...
    if (ctrl->listbox.multisel) {
      g_signal_connect(G_OBJECT(listitem),
                       "key_press_event",
                       G_CALLBACK(listitem_multi_key),
                       uc->adj);
    } else {
      g_signal_connect(G_OBJECT(listitem),
                       "key_press_event",
                       G_CALLBACK(listitem_single_key),
                       uc->adj);
    }
    g_signal_connect(
        G_OBJECT(listitem), "focus_in_event", G_CALLBACK(widget_focus), dp);
    g_signal_connect(G_OBJECT(listitem),
                     "button_press_event",
                     G_CALLBACK(listitem_button_press),
                     dp);
    g_signal_connect(G_OBJECT(listitem),
                     "button_release_event",
                     G_CALLBACK(listitem_button_release),
                     dp);
    g_object_set_data(G_OBJECT(listitem), "user-data", GINT_TO_POINTER(id));
    goto done;
  }
...
int dlg_listbox_getid(union control *ctrl, void *dlg, int index)
{
  ...
  if (uc->menu || uc->list) {
    ...
    children =
        gtk_container_children(GTK_CONTAINER(uc->menu ? uc->menu : uc->list));
...
void dlg_listbox_select(union control *ctrl, void *dlg, int index)
{
  ...
  if (uc->list) {
    ...
    if (nitems > 0) {
      ...
      g_list_free(items);
      newtop =
          uc->adj->lower + (uc->adj->upper - uc->adj->lower) * index / nitems;
      newbot = uc->adj->lower +
  ...
  if (uc->treeview) {
    ...
    gtk_tree_view_scroll_to_cell(
        GTK_TREE_VIEW(uc->treeview), path, NULL, FALSE, 0.0, 0.0);
    gtk_tree_path_free(path);
    return;
  }
#endif
...
void dlg_error_msg(void *dlg, const char *msg)
{
  ...
  g_signal_connect(
      G_OBJECT(ok), "clicked", G_CALLBACK(errmsg_button_clicked), window);
  g_signal_connect(
      G_OBJECT(window), "destroy", G_CALLBACK(window_destroy), NULL);
...
#if GTK_CHECK_VERSION(3, 0, 0)
  GtkWidget *coloursel =
      gtk_color_chooser_dialog_new("Select a colour", GTK_WINDOW(dp->window));
  gtk_color_chooser_set_use_alpha(GTK_COLOR_CHOOSER(coloursel), FALSE);
#else
  GtkWidget *okbutton, *cancelbutton;
  GtkWidget *coloursel = gtk_color_selection_dialog_new("Select a colour");
  GtkColorSelectionDialog *ccs = GTK_COLOR_SELECTION_DIALOG(coloursel);
  GtkColorSelection *cs =
      GTK_COLOR_SELECTION(gtk_color_selection_dialog_get_color_selection(ccs));
...
#if GTK_CHECK_VERSION(3, 0, 0)
  g_signal_connect(G_OBJECT(coloursel),
                   "response",
                   G_CALLBACK(colourchoose_response),
                   (gpointer)dp);
#else
...
#endif
  g_object_set_data(G_OBJECT(okbutton), "user-data", (gpointer)coloursel);
  g_object_set_data(G_OBJECT(cancelbutton), "user-data", (gpointer)coloursel);
  g_signal_connect(
      G_OBJECT(okbutton), "clicked", G_CALLBACK(coloursel_ok), (gpointer)dp);
  g_signal_connect(G_OBJECT(cancelbutton),
                   "clicked",
                   G_CALLBACK(coloursel_cancel),
                   (gpointer)dp);
  g_signal_connect_swapped(G_OBJECT(okbutton),
                           "clicked",
...
static gboolean editbox_key(GtkWidget *widget,
                            GdkEventKey *event,
                            gpointer data)
{
  /*
  ...
  if (event->keyval == GDK_KEY_Return && parent != NULL) {
    ...
    g_signal_emit_by_name(
        G_OBJECT(parent), "key_press_event", event, &return_val);
    return return_val;
  }
  return FALSE;
}
...
static gboolean editbox_lostfocus(GtkWidget *ed,
                                  GdkEventFocus *event,
                                  gpointer data)
{
  struct dlgparam *dp = (struct dlgparam *)data;
...
static gboolean listitem_key(GtkWidget *item,
                             GdkEventKey *event,
                             gpointer data,
                             int multiple)
{
  ...
  if (event->keyval == GDK_Up || event->keyval == GDK_KP_Up ||
    ...
    if (!multiple && GTK_WIDGET_STATE(item) != GTK_STATE_SELECTED) {
      ...
      int step =
          (event->keyval == GDK_Page_Down ||
           event->keyval == GDK_KP_Page_Down || event->keyval == GDK_Page_Up ||
      ...
      if (children && children->data) {
        if (!multiple)
          gtk_list_select_child(GTK_LIST(list), GTK_WIDGET(children->data));
...
static void draglist_move(struct dlgparam *dp, struct uctrl *uc, int direction)
{
  ...
  if ((index < 0) || (index == 0 && direction < 0) ||
...
static void listbox_doubleclick(GtkTreeView *treeview,
                                GtkTreePath *path,
...
static void listbox_selchange(GtkTreeSelection *treeselection, gpointer data)
{
  struct dlgparam *dp = (struct dlgparam *)data;
...
{
  struct draglist_valchange_ctx *ctx = (struct draglist_valchange_ctx *)data;

  ctx->uc->ctrl->generic.handler(
      ctx->uc->ctrl, ctx->dp, ctx->dp->data, EVENT_VALCHANGE);
                            ...
                            gpointer data)
{
  ...
  if (uc) {
     ...
     */
    struct draglist_valchange_ctx *ctx = snew(struct draglist_valchange_ctx);
    ctx->uc = uc;
    ctx->dp = dp;
...
static void filechoose_response(GtkDialog *dialog, gint response, gpointer data)
{
  /* struct dlgparam *dp = (struct dlgparam *)data; */
...
static void fontsel_ok(GtkButton *button, gpointer data)
{
  ...
  unifontsel *fontsel =
      (unifontsel *)g_object_get_data(G_OBJECT(button), "user-data");
...
static void filefont_clicked(GtkButton *button, gpointer data)
{
  ...
  if (uc->ctrl->generic.type == CTRL_FILESELECT) {
    ...
    GtkWidget *filechoose = gtk_file_chooser_dialog_new(
        uc->ctrl->fileselect.title,
        GTK_WINDOW(dp->window),
        (uc->ctrl->fileselect.for_writing ? GTK_FILE_CHOOSER_ACTION_SAVE
                                          : GTK_FILE_CHOOSER_ACTION_OPEN),
        STANDARD_CANCEL_LABEL,
        GTK_RESPONSE_CANCEL,
        STANDARD_OPEN_LABEL,
        GTK_RESPONSE_ACCEPT,
        (const gchar *)NULL);
    ...
    g_signal_connect(G_OBJECT(filechoose),
                     "response",
                     G_CALLBACK(filechoose_response),
                     (gpointer)dp);
    gtk_widget_show(filechoose);
#else
    GtkWidget *filesel = gtk_file_selection_new(uc->ctrl->fileselect.title);
    gtk_window_set_modal(GTK_WINDOW(filesel), TRUE);
    g_object_set_data(G_OBJECT(GTK_FILE_SELECTION(filesel)->ok_button),
                      "user-data",
                      (gpointer)filesel);
    g_object_set_data(G_OBJECT(filesel), "user-data", (gpointer)uc);
    g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(filesel)->ok_button),
                     "clicked",
                     G_CALLBACK(filesel_ok),
                     (gpointer)dp);
  ...
  if (uc->ctrl->generic.type == CTRL_FONTSELECT) {
    ...
    GtkWidget *fontsel = gtk_font_selection_dialog_new("Select a font");
    ...
    g_object_set_data(G_OBJECT(GTK_FONT_SELECTION_DIALOG(fontsel)->ok_button),
                      "user-data",
                      (gpointer)fontsel);
    ...
    g_object_set_data(
        G_OBJECT(fontsel->ok_button), "user-data", (gpointer)fontsel);
    fontsel->user_data = uc;
    g_signal_connect(G_OBJECT(fontsel->ok_button),
                     "clicked",
                     G_CALLBACK(fontsel_ok),
                     (gpointer)dp);
    g_signal_connect_swapped(G_OBJECT(fontsel->ok_button),
                             "clicked",
                             G_CALLBACK(unifontsel_destroy),
                             (gpointer)fontsel);
    g_signal_connect_swapped(G_OBJECT(fontsel->cancel_button),
                             "clicked",
                             G_CALLBACK(unifontsel_destroy),
                             (gpointer)fontsel);
...
#if !GTK_CHECK_VERSION(3, 0, 0)
static void label_sizealloc(GtkWidget *widget,
                            GtkAllocation *alloc,
                            gpointer data)
{
  struct dlgparam *dp = (struct dlgparam *)data;
 ...
 */
GtkWidget *layout_ctrls(struct dlgparam *dp,
  ...
  for (i = 0; i < s->ncontrols; i++) {
    ...
    case CTRL_COLUMNS: {
      ...
      columns_set_cols(
          cols,
          ctrl->columns.ncols,
          (ctrl->columns.percentages ? ctrl->columns.percentages : simplecols));
    }
    ...
    case CTRL_BUTTON:
      ...
      g_signal_connect(G_OBJECT(w), "clicked", G_CALLBACK(button_clicked), dp);
      g_signal_connect(
          G_OBJECT(w), "focus_in_event", G_CALLBACK(widget_focus), dp);
      shortcut_add(scs,
    ...
    case CTRL_CHECKBOX:
      ...
      g_signal_connect(G_OBJECT(w), "toggled", G_CALLBACK(button_toggled), dp);
      g_signal_connect(
          G_OBJECT(w), "focus_in_event", G_CALLBACK(widget_focus), dp);
      shortcut_add(scs,
                   gtk_bin_get_child(GTK_BIN(w)),
    ...
    case CTRL_RADIO:
       ...
       */
      {
        ...
        if (ctrl->generic.label) {
          ...
          gtk_widget_show(label);
          shortcut_add(scs, label, ctrl->radio.shortcut, SHORTCUT_UCTRL, uc);
          uc->label = label;
        }
        percentages = g_new(gint, ctrl->radio.ncolumns);
        for (i = 0; i < ctrl->radio.ncolumns; i++) {
          percentages[i] = ((100 * (i + 1) / ctrl->radio.ncolumns) -
                            100 * i / ctrl->radio.ncolumns);
        }
        columns_set_cols(COLUMNS(w), ctrl->radio.ncolumns, percentages);
        g_free(percentages);
        group = NULL;
        ...
        for (i = 0; i < ctrl->radio.nbuttons; i++) {
          ...
          b = (gtk_radio_button_new_with_label(group, ctrl->radio.buttons[i]));
          uc->buttons[i] = b;
          group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(b));
          colstart = i % ctrl->radio.ncolumns;
          columns_add(COLUMNS(w),
                      b,
                      colstart,
                      (i == ctrl->radio.nbuttons - 1
                           ? ctrl->radio.ncolumns - colstart
                           : 1));
          columns_force_left_align(COLUMNS(w), b);
          gtk_widget_show(b);
          g_signal_connect(
              G_OBJECT(b), "toggled", G_CALLBACK(button_toggled), dp);
          g_signal_connect(
              G_OBJECT(b), "focus_in_event", G_CALLBACK(widget_focus), dp);
          if (ctrl->radio.shortcuts) {
            shortcut_add(scs,
                         gtk_bin_get_child(GTK_BIN(b)),
    ...
    case CTRL_EDITBOX: {
      ...
      if (ctrl->editbox.has_list) {
         ...
         * GTK 2 combo box.
         */
        uc->listmodel = gtk_list_store_new(2, G_TYPE_INT, G_TYPE_STRING);
        w = gtk_combo_box_new_with_model_and_entry(
            GTK_TREE_MODEL(uc->listmodel));
        g_object_set(G_OBJECT(w), "entry-text-column", 1, (const char *)NULL);
      ...
      uc->entrysig = g_signal_connect(
          G_OBJECT(signalobject), "changed", G_CALLBACK(editbox_changed), dp);
      g_signal_connect(G_OBJECT(signalobject),
                       "key_press_event",
                       G_CALLBACK(editbox_key),
                       dp);
      g_signal_connect(G_OBJECT(signalobject),
                       "focus_in_event",
                       G_CALLBACK(widget_focus),
                       dp);
      g_signal_connect(G_OBJECT(signalobject),
                       "focus_out_event",
                       G_CALLBACK(editbox_lostfocus),
                       dp);
      g_signal_connect(G_OBJECT(signalobject),
                       "focus_out_event",
                       G_CALLBACK(editbox_lostfocus),
      ...
      if (ctrl->generic.label) {
        ...
        shortcut_add(
            scs, label, ctrl->editbox.shortcut, SHORTCUT_FOCUS, uc->entry);
        ...
        if (ctrl->editbox.percentwidth == 100) {
          ...
          columns_force_same_height(COLUMNS(container), label, w);
        }
        gtk_widget_show(label);
        gtk_widget_show(w);
    ...
    case CTRL_FONTSELECT: {
      ...
      const char *browsebtn =
          (ctrl->generic.type == CTRL_FILESELECT ? "Browse..." : "Change...");
      ...
      g_signal_connect(
          G_OBJECT(uc->entry), "key_press_event", G_CALLBACK(editbox_key), dp);
      uc->entrysig = g_signal_connect(
          G_OBJECT(uc->entry), "changed", G_CALLBACK(editbox_changed), dp);
      g_signal_connect(
          G_OBJECT(uc->entry), "focus_in_event", G_CALLBACK(widget_focus), dp);
      g_signal_connect(
          G_OBJECT(uc->button), "focus_in_event", G_CALLBACK(widget_focus), dp);
      g_signal_connect(
          G_OBJECT(ww), "clicked", G_CALLBACK(filefont_clicked), dp);
    } break;
    case CTRL_LISTBOX:
      ...
      if (ctrl->listbox.height == 0) {
        ...
        g_object_set_data(
            G_OBJECT(uc->menu), "user-data", (gpointer)uc->optmenu);
        g_signal_connect(G_OBJECT(uc->optmenu),
                         "focus_in_event",
                         G_CALLBACK(widget_focus),
                         dp);
#else
         ...
         */
        w = gtk_combo_box_new_with_model(GTK_TREE_MODEL(uc->listmodel));
        uc->combo = w;
        ...
        gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(w), cr, "text", 1, NULL);
         ...
         */
        g_signal_connect(
            G_OBJECT(w), "changed", G_CALLBACK(droplist_selchange), dp);
        ...
        if (ctrl->listbox.multisel == 2) {
          ...
          gtk_list_set_selection_mode(GTK_LIST(uc->list), GTK_SELECTION_SINGLE);
        }
        w = gtk_scrolled_window_new(NULL, NULL);
        gtk_scrolled_window_add_with_viewport(GTK_SCROLLED_WINDOW(w), uc->list);
        gtk_scrolled_window_set_policy(
            GTK_SCROLLED_WINDOW(w), GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
        uc->adj = gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(w));

        gtk_widget_show(uc->list);
        g_signal_connect(G_OBJECT(uc->list),
                         "selection-changed",
                         G_CALLBACK(list_selchange),
                         dp);
        g_signal_connect(
            G_OBJECT(uc->list), "focus_in_event", G_CALLBACK(widget_focus), dp);
         ...
         */
        {
          ...
          gtk_widget_set_size_request(
              w, 10, 2 * edge + (ctrl->listbox.height * get_listitemheight(w)));
        }
        ...
        if (ctrl->listbox.draglist) {
          ...
          g_signal_connect(
              G_OBJECT(button), "clicked", G_CALLBACK(draglist_up), dp);
          g_signal_connect(
              G_OBJECT(button), "focus_in_event", G_CALLBACK(widget_focus), dp);
          button = gtk_button_new_with_label("Down");
          columns_add(COLUMNS(cols), button, 1, 1);
          gtk_widget_show(button);
          g_signal_connect(
              G_OBJECT(button), "clicked", G_CALLBACK(draglist_down), dp);
          g_signal_connect(
              G_OBJECT(button), "focus_in_event", G_CALLBACK(widget_focus), dp);
         ...
         */
        w = gtk_tree_view_new_with_model(GTK_TREE_MODEL(uc->listmodel));
        g_object_set_data(G_OBJECT(uc->listmodel), "user-data", (gpointer)w);
        ...
        uc->treeview = w;
        g_signal_connect(
            G_OBJECT(w), "row-activated", G_CALLBACK(listbox_doubleclick), dp);
        g_signal_connect(
            G_OBJECT(w), "focus_in_event", G_CALLBACK(widget_focus), dp);
        g_signal_connect(
            G_OBJECT(sel), "changed", G_CALLBACK(listbox_selchange), dp);

        if (ctrl->listbox.draglist) {
          gtk_tree_view_set_reorderable(GTK_TREE_VIEW(w), TRUE);
          g_signal_connect(G_OBJECT(uc->listmodel),
                           "row-inserted",
                           G_CALLBACK(listbox_reorder),
                           dp);
        }
...

        {
          ...
          gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scroll),
                                         ...
                                         GTK_POLICY_ALWAYS);
          gtk_widget_set_size_request(
              scroll, -1, ctrl->listbox.height * get_listitemheight(w));
      ...
      if (ctrl->generic.label) {
        ...
        shortcut_add(scs, label, ctrl->listbox.shortcut, SHORTCUT_UCTRL, uc);
    ...
    case CTRL_TEXT:
      ...
      uc->textsig = g_signal_connect(
          G_OBJECT(w), "size-allocate", G_CALLBACK(label_sizealloc), dp);
#else
      /*
...
gint tree_focus(GtkContainer *container,
...
int win_key_press(GtkWidget *widget, GdkEventKey *event, gpointer data)
{
  ...
  if ((event->state & GDK_MOD1_MASK) && (unsigned char)event->string[0] > 0 &&
        ...
        if (schr == sc->uc->ctrl->radio.shortcut) {
          ...
          for (i = 0; i < sc->uc->ctrl->radio.nbuttons; i++)
            if (schr == sc->uc->ctrl->radio.shortcuts[i]) {
              gtk_widget_grab_focus(sc->uc->buttons[i]);
              g_signal_emit_by_name(G_OBJECT(sc->uc->buttons[i]), "clicked");
            }
        }
        break;
      case CTRL_LISTBOX:
        ...
        if (sc->uc->list) {
           ...
           * do the Right Thing.
           */
          gtk_container_focus(GTK_CONTAINER(sc->uc->list), GTK_DIR_TAB_FORWARD);
          break;
        }
#else
        if (sc->uc->treeview) {
                  ...
                  int protcfginfo)
{
...
#if GTK_CHECK_VERSION(2, 0, 0)
  treestore = gtk_tree_store_new(TREESTORE_NUM, G_TYPE_STRING, G_TYPE_INT);
...
#endif
  g_signal_connect(
      G_OBJECT(tree), "focus_in_event", G_CALLBACK(widget_focus), &dp);
  ...
  for (index = 0; index < ctrlbox->nctrlsets; index++) {
    ...
    if (!*s->pathname) {
      ...
      if (j != INT_MAX) { /* add to treeview, start new panel */
        ...
        gtk_notebook_append_page(GTK_NOTEBOOK(panels), panelvbox, NULL);
        if (first) {
          gint page_num;

          page_num = gtk_notebook_page_num(GTK_NOTEBOOK(panels), panelvbox);
          gtk_notebook_set_current_page(GTK_NOTEBOOK(panels), page_num);
        }

        if (nselparams >= selparamsize) {
          selparamsize += 16;
          selparams = sresize(selparams, selparamsize, struct selparam);
        }
        ...
        if (j > 0)
           ...
           */
          gtk_tree_store_append(treestore, &treeiter, &treeiterlevels[j - 1]);
        else
        ...
        if (j > 0) {
          selparams[nselparams].treepath = gtk_tree_model_get_path(
          ...
          gtk_tree_view_expand_row(
              GTK_TREE_VIEW(tree), selparams[nselparams].treepath, FALSE);
        } else {
        ...
        if (j > 0) {
          if (!treelevels[j - 1]) {
            ...
            gtk_tree_item_set_subtree(treeitemlevels[j - 1],
        ...
        g_signal_connect(G_OBJECT(treeitem),
                         "key_press_event",
                         G_CALLBACK(tree_key_press),
                         &dp);
        g_signal_connect(G_OBJECT(treeitem),
                         "focus_in_event",
                         G_CALLBACK(widget_focus),
   ...
   */
  g_signal_connect(G_OBJECT(tree), "map", G_CALLBACK(treeview_map_event), &dp);
#endif /* GTK 2 vs 3 */
#endif /* GTK 2+ vs 1 */

#if GTK_CHECK_VERSION(2, 0, 0)
  g_signal_connect(G_OBJECT(treeselection),
                   "changed",
                   G_CALLBACK(treeselection_changed),
                   selparams);
#else
  dp.ntreeitems = nselparams;
  dp.treeitems = snewn(dp.ntreeitems, GtkWidget *);
  for (index = 0; index < nselparams; index++) {
    g_signal_connect(G_OBJECT(selparams[index].treeitem),
                     "select",
  ...
  {
    /* in gtkwin.c */
    extern void set_window_icon(
        GtkWidget * window, const char *const *const *icon, int n_icon);
...
#endif
  gtk_scrolled_window_set_policy(
      GTK_SCROLLED_WINDOW(treescroll), GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
  ...
  g_signal_connect(
      G_OBJECT(window), "destroy", G_CALLBACK(window_destroy), NULL);
  g_signal_connect(
      G_OBJECT(window), "key_press_event", G_CALLBACK(win_key_press), &dp);
...
static void messagebox_handler(union control *ctrl,
                               void *dlg,
                               void *data,
                               int event)
{
               ...
               ...)
{
  ...
  for (index = 0; index < ncols; index++)
    c->columns.percentages[index] =
        (index + 1) * 100 / ncols - index * 100 / ncols;
  va_start(ap, selectable);
  index = 0;
  while (1) {
    ...
    value = va_arg(ap, int);
    c = ctrl_pushbutton(
        s0, title, shortcut, HELPCTX(no_help), messagebox_handler, I(value));
  ...
  if (parentwin) {
    ...
    gtk_window_set_transient_for(GTK_WINDOW(window), GTK_WINDOW(parentwin));
  } else
    gtk_window_set_position(GTK_WINDOW(window), GTK_WIN_POS_CENTER);
...
static void about_key_press(GtkWidget *widget,
                            GdkEventKey *event,
                            gpointer data)
{
...
void about_box(void *window)
{
  ...
  g_signal_connect(
      G_OBJECT(w), "clicked", G_CALLBACK(about_close_clicked), NULL);
  gtk_widget_show(w);

  w = gtk_button_new_with_label("View Licence");
  gtk_widget_set_can_default(w, TRUE);
  gtk_box_pack_end(action_area, w, FALSE, FALSE, 0);
  g_signal_connect(G_OBJECT(w), "clicked", G_CALLBACK(licence_clicked), NULL);
  gtk_widget_show(w);
  ...
  g_signal_connect(
      G_OBJECT(aboutbox), "key_press_event", G_CALLBACK(about_key_press), NULL);

  set_transient_window_pos(GTK_WIDGET(window), aboutbox);
  gtk_window_set_transient_for(GTK_WINDOW(aboutbox), GTK_WINDOW(window));
...
static void eventlog_ok_handler(union control *ctrl,
                                void *dlg,
                                void *data,
                                int event)
{
...
static void eventlog_list_handler(union control *ctrl,
                                  void *dlg,
                                  void *data,
                                  int event)
{
  ...
  if (event == EVENT_REFRESH) {
    ...
    if (gtk_selection_owner_set(
            es->window, GDK_SELECTION_PRIMARY, GDK_CURRENT_TIME)) {
      extern GdkAtom compound_text_atom;

      gtk_selection_add_target(
          es->window, GDK_SELECTION_PRIMARY, GDK_SELECTION_TYPE_STRING, 1);
      gtk_selection_add_target(
          es->window, GDK_SELECTION_PRIMARY, compound_text_atom, 1);
    }
  }
}

void eventlog_selection_get(GtkWidget *widget,
...
gint eventlog_selection_clear(GtkWidget *widget,
                              GdkEventSelection *seldata,
                              gpointer data)
{
  struct eventlog_stuff *es = (struct eventlog_stuff *)data;
  struct uctrl *uc;
...
void showeventlog(void *estuff, void *parentwin)
{
  ...
  c = ctrl_pushbutton(
      s0, "Close", 'c', HELPCTX(no_help), eventlog_ok_handler, P(NULL));
  c->button.column = 1;
  c->button.isdefault = TRUE;

  s1 = ctrl_getset(es->eventbox, "x", "", "");
  es->listctrl = c = ctrl_listbox(
      s1, NULL, NO_SHORTCUT, HELPCTX(no_help), eventlog_list_handler, P(es));
  c->listbox.height = 10;
  ...
  gtk_widget_set_size_request(
      w1,
      20 + string_width("LINE OF TEXT GIVING WIDTH OF EVENT LOG IS "
  ...
  g_signal_connect(
      G_OBJECT(window), "destroy", G_CALLBACK(eventlog_destroy), es);
  g_signal_connect(
      G_OBJECT(window), "key_press_event", G_CALLBACK(win_key_press), &es->dp);
  g_signal_connect(G_OBJECT(window),
                   "selection_get",
                   G_CALLBACK(eventlog_selection_get),
                   es);
  g_signal_connect(G_OBJECT(window),
                   "selection_clear_event",
                   G_CALLBACK(eventlog_selection_clear),
                   es);
}
