                       ...
                       int address_family)
{
  ...
  *realhost = '\0';
  ...
  if ((a = inet_addr(host)) == (unsigned long)(in_addr_t)(-1)) {
    ...
    if (ret->superfamily == UNRESOLVED) {
      ret->error =
          (h_errno == HOST_NOT_FOUND || h_errno == NO_DATA ||
                   h_errno == NO_ADDRESS
               ? "Host does not exist"
               : h_errno == TRY_AGAIN ? "Temporary name service failure"
...
#ifndef NO_IPV6
    if (getnameinfo(addr->ais->ai_addr,
                    addr->ais->ai_addrlen,
                    buf,
                    buflen,
                    NULL,
...
static int try_connect(Actual_Socket sock)
{
  ...
  if (sock->oobinline) {
    int b = TRUE;
    if (setsockopt(s, SOL_SOCKET, SO_OOBINLINE, (void *)&b, sizeof(b)) < 0) {
      err = errno;
      close(s);
      goto ret;
    }
  ...
  if (sock->nodelay) {
    int b = TRUE;
    if (setsockopt(s, IPPROTO_TCP, TCP_NODELAY, (void *)&b, sizeof(b)) < 0) {
      err = errno;
      close(s);
      goto ret;
    }
  ...
  if (sock->keepalive) {
    int b = TRUE;
    if (setsockopt(s, SOL_SOCKET, SO_KEEPALIVE, (void *)&b, sizeof(b)) < 0) {
      err = errno;
      close(s);
      goto ret;
    }
                      ...
                      int orig_address_family)
{
  ...
  if (address_family == AF_INET6 && orig_address_family == ADDRTYPE_UNSPEC) {
    ...
    other = (Actual_Socket)sk_newlistener(
        srcaddr, port, plug, local_host_only, ADDRTYPE_IPV4);
...
static void net_select_result(int fd, int event)
{
  ...
  case 4: /* exceptional */
    if (!s->oobinline) {
      ...
      if (ret <= 0) {
        plug_closing(s->plug,
                     ret == 0 ? "Internal networking trouble" : strerror(errno),
                     errno,
                     0);
      } else {
        /*
  ...
  case 1: /* readable; also acceptance */
    if (s->listener) {
      ...
      if ((!s->addr || s->addr->superfamily != UNIX) && s->localhost_only &&
    ...
    if (!s->connected) {
       ...
       */
      {
        ...
        if (getsockopt(s->s, SOL_SOCKET, SO_ERROR, &err, &errlen) < 0) {
          errmsg = dupprintf("getsockopt(SO_ERROR): %s", strerror(errno));
...
static char *sk_tcp_peer_info(Socket sock)
{
  ...
  if (addr.storage.ss_family == AF_INET) {
    return dupprintf(
        "%s:%d", inet_ntoa(addr.sin.sin_addr), (int)ntohs(addr.sin.sin_port));
#ifndef NO_IPV6
...
char *get_hostname(void)
{
  ...
  do {
    ...
    if ((gethostname(hostname, len) < 0) && (errno != ENAMETOOLONG)) {
      sfree(hostname);
      hostname = NULL;
      break;
    }
...
SockAddr platform_get_x11_unix_address(const char *sockpath, int displaynum)
{
  ...
  if (sockpath) {
    ...
    n = snprintf(
        ret->hostname, sizeof ret->hostname, "%s%d", X11_UNIX_PATH, displaynum);
  }
