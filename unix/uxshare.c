...
static char *make_dir_and_check_ours(const char *dirname)
{
  ...
  if (st.st_uid != getuid())
    return dupprintf(
        "%s: directory owned by uid %d, not by us", dirname, st.st_uid);
  if ((st.st_mode & 077) != 0)
...
static char *make_dirname(const char *pi_name, char **logtext)
{
   ...
   */
  {
    ...
    if (saltfd < 0) {
      ...
      if (errno != ENOENT) {
        *logtext = dupprintf("%s: open: %s", saltname, strerror(errno));
        sfree(saltname);
        sfree(parentdirname);
        return NULL;
      }
      ...
      for (i = 0;; i++) {
        tmpname =
            dupprintf("%s/%s.tmp.%d.%d", parentdirname, SALT_FILENAME, pid, i);
        saltfd = open(tmpname, O_WRONLY | O_EXCL | O_CREAT, 0400);
        if (saltfd >= 0)
          break;
        if (errno != EEXIST) {
          *logtext = dupprintf("%s: open: %s", tmpname, strerror(errno));
          sfree(tmpname);
          sfree(saltname);
          sfree(parentdirname);
          return NULL;
      ...
      if (ret < 0) {
        close(saltfd);
        *logtext = dupprintf("%s: write: %s", tmpname, strerror(errno));
        sfree(tmpname);
        sfree(saltname);
        sfree(parentdirname);
        return NULL;
      ...
      if (close(saltfd) < 0) {
        *logtext = dupprintf("%s: close: %s", tmpname, strerror(errno));
        sfree(tmpname);
        sfree(saltname);
        sfree(parentdirname);
        return NULL;
      ...
      if (link(tmpname, saltname) < 0 && errno != EEXIST) {
        *logtext = dupprintf("%s: link: %s", saltname, strerror(errno));
        sfree(tmpname);
        sfree(saltname);
        sfree(parentdirname);
        return NULL;
       ...
       */
      if (unlink(tmpname) < 0) {
        *logtext = dupprintf("%s: unlink: %s", tmpname, strerror(errno));
        sfree(tmpname);
        sfree(saltname);
        sfree(parentdirname);
        return NULL;
    ...
    for (i = 0; i < SALT_SIZE; i++) {
      ...
      if (ret <= 0) {
        close(saltfd);
        *logtext = dupprintf("%s: read: %s",
                             saltname,
                             ret == 0 ? "unexpected EOF" : strerror(errno));
        sfree(saltname);
        sfree(parentdirname);
        return NULL;
      }
                       ...
                       int can_downstream)
{
  ...
  if (flock(lockfd, LOCK_EX) < 0) {
    *logtext = dupprintf("%s: flock(LOCK_EX): %s", lockname, strerror(errno));
    sfree(dirname);
    sfree(lockname);
    close(lockfd);
    return SHARE_NONE;
  ...
  *logtext = NULL;
  ...
  if (can_downstream) {
    retsock = new_connection(
        unix_sock_addr(sockname), "", 0, 0, 1, 0, 0, downplug, conf);
