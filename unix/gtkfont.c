                              ...
                              int cellwidth)
{
  ...
  if (xfont->sixteen_bit) {
    ...
    maxchars = 0;
    for (i = 0; i < len; i++)
      if ((unsigned char)string[i] <= 0x7F || (unsigned char)string[i] >= 0xC0)
        maxchars++;
    wcs = snewn(maxchars + 1, wchar_t);
    nchars = charset_to_unicode(
        (char **)&string, &len, wcs, maxchars, CS_UTF8, NULL, NULL, 0);
                                ...
                                int cellwidth)
{
  ...
  while (len > 0) {
     ...
     */
    clen = 1;
    while (clen < len && (unsigned char)string[clen] >= 0x80 &&
