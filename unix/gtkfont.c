...
struct unifont_vtable {
  ...
  void (*draw_text)(GdkDrawable *target,
                    GdkGC *gc,
                    unifont *font,
                    int x,
                    int y,
...
static void x11font_draw_text(GdkDrawable *target,
                              GdkGC *gc,
                              unifont *font,
                              int x,
                              int y,
...
static void x11_alloc_subfont(struct x11font *xfont, int sfid)
{
  Display *disp = GDK_DISPLAY();
  char *derived_name =
      x11_guess_derived_font_name(xfont->fonts[0], sfid & 1, !!(sfid & 2));
...
static int x11font_has_glyph(unifont *font, wchar_t glyph)
{
  ...
  if (xfont->sixteen_bit) {
    ...
    return x11_font_has_glyph(xfont->fonts[0], 0, (unsigned char)sbstring[0]);
  }
}
...
static void x11font_really_draw_text_16(GdkDrawable *target,
                                        XFontStruct *xfs,
                                        GC gc,
                                        int x,
                                        int y,
                                        ...
                                        int cellwidth)
{
  ...
  while (nsteps-- > 0) {
    ...
    XDrawString16(disp, GDK_DRAWABLE_XID(target), gc, X, y, string, step);
    if (shadowoffset)
      XDrawString16(disp,
...
static void x11font_really_draw_text(GdkDrawable *target,
                                     XFontStruct *xfs,
                                     GC gc,
                                     int x,
                                     int y,
                                     ...
                                     int cellwidth)
{
  ...
  while (nsteps-- > 0) {
    ...
    XDrawString(disp, GDK_DRAWABLE_XID(target), gc, X, y, string, step);
    if (shadowoffset)
      XDrawString(disp,
                  GDK_DRAWABLE_XID(target),
                  gc,
                              ...
                              int cellwidth)
{
  ...
  if (xfont->sixteen_bit) {
    ...
    x11font_really_draw_text_16(target,
                                xfont->fonts[sfid],
                                gc,
                                x,
                                y,
    ...
    int sblen = wc_to_mb(xfont->real_charset,
                         ...
                         NULL,
                         NULL);
    x11font_really_draw_text(target,
                             xfont->fonts[sfid],
                             gc,
                             x,
                             y,
  ...
  for (i = 0; i < nnames; i++) {
    if (fontnames[i][0] == '-') {
      ...
      *p++ = '\0';
       ...
       */
      style = p;
      p += sprintf(p, "%s", components[2][0] ? components[2] : "regular");
      ...
      p += sprintf(p,
                   ...
                   weightkey,
                   (int)strlen(components[2]),
                   components[2],
                   slantkey,
                   (int)strlen(components[3]),
                   components[3],
                   setwidthkey,
                   (int)strlen(components[4]),
                   components[4],
                   (int)strlen(components[10]),
                   components[10],
                   (int)strlen(components[5]),
                   components[5]);
...
static void pangofont_draw_text(GdkDrawable *target,
                                GdkGC *gc,
                                unifont *font,
                                int x,
                                int y,
    ...
    if (is_rtl(string[0]) ||
      ...
      while (clen < utflen) {
        ...
        while (clen < utflen && (unsigned char)utfptr[clen] >= 0x80 &&
          ...
          clen++;
        n++;
        if (pangofont_char_width(layout,
                                 pfont,
                                 string[n - 1],
                                 utfptr + oldclen,
...
void unifont_draw_text(GdkDrawable *target,
                       GdkGC *gc,
                       unifont *font,
                       int x,
                       int y,
...
{
  font->vt->draw_text(
      target, gc, font, x, y, string, len, wide, bold, cellwidth);
}
...
static void multifont_draw_text(GdkDrawable *target,
                                GdkGC *gc,
                                unifont *font,
                                int x,
                                int y,
                                ...
                                int cellwidth)
{
  ...
  while (len > 0) {
     ...
     * Now display it.
     */
    unifont_draw_text(target,
                      gc,
                      ok ? mfont->main : mfont->fallback,
                      x,
                      y,
                      string,
                      i,
...
static void unifontsel_set_filter_buttons(unifontsel_internal *fs)
{
  ...
  for (i = 0; i < lenof(fs->filter_buttons); i++) {
    int flagbit = GPOINTER_TO_INT(
        gtk_object_get_data(GTK_OBJECT(fs->filter_buttons[i]), "user-data"));
...
static void unifontsel_draw_preview_text(unifontsel_internal *fs)
{
  ...
  if (fs->preview_pixmap) {
    ...
    gdk_draw_rectangle(
        fs->preview_pixmap, gc, 1, 0, 0, fs->preview_width, fs->preview_height);
    ...
    if (font) {
       ...
       */
      info->fontclass->draw_text(fs->preview_pixmap,
                                 gc,
                                 font,
                                 0,
                                 font->ascent,
...
static void unifontsel_button_toggled(GtkToggleButton *tb, gpointer data)
{
  ...
  int newflags;
  int flagbit =
      GPOINTER_TO_INT(gtk_object_get_data(GTK_OBJECT(tb), "user-data"));
...
unifontsel *unifontsel_new(const char *wintitle)
{
  ...
  w = table;
#endif
  gtk_box_pack_start(GTK_BOX(GTK_DIALOG(fs->u.window)->vbox), w, TRUE, TRUE, 0);
  ...
  model = gtk_list_store_new(
      4, G_TYPE_STRING, G_TYPE_INT, G_TYPE_INT, G_TYPE_BOOLEAN);
  w = gtk_tree_view_new_with_model(GTK_TREE_MODEL(model));
  gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(w), FALSE);
  gtk_label_set_mnemonic_widget(GTK_LABEL(label), w);
  gtk_widget_show(w);
  ...
  gtk_signal_connect(GTK_OBJECT(fs->preview_area),
                     "expose_event",
                     GTK_SIGNAL_FUNC(unifontsel_expose_area),
                     fs);
  gtk_signal_connect(GTK_OBJECT(fs->preview_area),
                     "configure_event",
                     GTK_SIGNAL_FUNC(unifontsel_configure_area),
                     fs);
  ...
  gtk_object_set_data(
      GTK_OBJECT(w), "user-data", GINT_TO_POINTER(FONTFLAG_CLIENTSIDE));
  gtk_signal_connect(
      GTK_OBJECT(w), "toggled", GTK_SIGNAL_FUNC(unifontsel_button_toggled), fs);
  gtk_widget_show(w);
  ...
  gtk_object_set_data(
      GTK_OBJECT(w), "user-data", GINT_TO_POINTER(FONTFLAG_SERVERSIDE));
  gtk_signal_connect(
      GTK_OBJECT(w), "toggled", GTK_SIGNAL_FUNC(unifontsel_button_toggled), fs);
  gtk_widget_show(w);
  fs->filter_buttons[i++] = w;
  ...
  gtk_object_set_data(
      GTK_OBJECT(w), "user-data", GINT_TO_POINTER(FONTFLAG_SERVERALIAS));
  gtk_signal_connect(
      GTK_OBJECT(w), "toggled", GTK_SIGNAL_FUNC(unifontsel_button_toggled), fs);
  gtk_widget_show(w);
  fs->filter_buttons[i++] = w;
  ...
  gtk_object_set_data(
      GTK_OBJECT(w), "user-data", GINT_TO_POINTER(FONTFLAG_NONMONOSPACED));
  gtk_signal_connect(
      GTK_OBJECT(w), "toggled", GTK_SIGNAL_FUNC(unifontsel_button_toggled), fs);
  gtk_widget_show(w);
  fs->filter_buttons[i++] = w;
