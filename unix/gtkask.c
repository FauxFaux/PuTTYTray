...
static gint key_event(GtkWidget *widget, GdkEventKey *event, gpointer data)
{
  ...
  if (event->keyval == GDK_KEY_Return && event->type == GDK_KEY_PRESS) {
    gtk_main_quit();
  } else if (event->keyval == GDK_KEY_Escape && event->type == GDK_KEY_PRESS) {
    ...
    if (event->type == GDK_KEY_PRESS) {
      if (!strcmp(event->string, "\x15")) {
        ...
        while (ctx->passlen > 0) {
          ...
          prior = (ctx->passlen == 0 ? ' ' : ctx->passphrase[ctx->passlen - 1]);
...
#if GTK_CHECK_VERSION(2, 0, 0)
static void input_method_commit_event(GtkIMContext *imc,
                                      gchar *str,
                                      gpointer data)
{
  struct askpass_ctx *ctx = (struct askpass_ctx *)data;
...
static void askpass_redraw_cairo(cairo_t *cr, struct drawing_area_ctx *ctx)
{
  cairo_set_source_rgb(
      cr, 1 - ctx->current, 1 - ctx->current, 1 - ctx->current);
  cairo_paint(cr);
}
#else
...
#else
static gint expose_area(GtkWidget *widget, GdkEventExpose *event, gpointer data)
{
  struct drawing_area_ctx *ctx = (struct drawing_area_ctx *)data;
...
static int try_grab_keyboard(struct askpass_ctx *ctx)
{
  ...
  seat = gdk_display_get_default_seat(gtk_widget_get_display(ctx->dialog));
  if (!seat)
    return FALSE;

  ctx->seat = seat;
  ret = gdk_seat_grab(seat,
  ...
  dm = gdk_display_get_device_manager(gtk_widget_get_display(ctx->dialog));
  if (!dm)
    return FALSE;
   ...
   */
  ret = gdk_keyboard_grab(
      gtk_widget_get_window(ctx->dialog), FALSE, GDK_CURRENT_TIME);
#endif
                                     ...
                                     const char *prompt_text)
{
  ...
  our_dialog_add_to_content_area(
      GTK_WINDOW(ctx->dialog), ctx->promptlabel, TRUE, TRUE, 0);
#if GTK_CHECK_VERSION(2, 0, 0)
...
#ifndef DRAW_DEFAULT_CAIRO
  {
    ...
    gdk_colormap_alloc_colors(ctx->colmap, ctx->cols, 2, FALSE, TRUE, success);
  ...
  for (i = 0; i < N_DRAWING_AREAS; i++) {
    ...
    gtk_box_pack_end(action_area, ctx->drawingareas[i].area, TRUE, TRUE, 5);
...
#if GTK_CHECK_VERSION(2, 0, 0)
  g_signal_connect(
      G_OBJECT(ctx->imc), "commit", G_CALLBACK(input_method_commit_event), ctx);
#endif
  g_signal_connect(G_OBJECT(ctx->promptlabel),
                   "key_press_event",
