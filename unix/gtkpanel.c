...
static void panels_forall(GtkContainer *container,
...
static void panels_map(GtkWidget *widget)
{
  ...
  for (children = panels->children; children && (child = children->data);
...
static void panels_draw(GtkWidget *widget, GdkRectangle *area)
{
  ...
  if (GTK_WIDGET_DRAWABLE(widget)) {
    ...
    for (children = panels->children; children && (child = children->data);
         children = children->next) {
      if (child && GTK_WIDGET_DRAWABLE(child) &&
...
static gint panels_expose(GtkWidget *widget, GdkEventExpose *event)
{
  ...
  if (GTK_WIDGET_DRAWABLE(widget)) {
    ...
    for (children = panels->children; children && (child = children->data);
         children = children->next) {
      if (child && GTK_WIDGET_DRAWABLE(child) && GTK_WIDGET_NO_WINDOW(child) &&
