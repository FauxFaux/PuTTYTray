...
static void gss_init(struct ssh_gss_library *lib,
                     void *dlhandle,
                     int id,
                     const char *msg)
{
...
struct ssh_gss_liblist *ssh_gss_setup(const Config *cfg)
{
  ...
  if ((gsslib = dlopen("libgssapi.so.2", RTLD_LAZY)) != NULL)
    gss_init(&list->libraries[list->nlibraries++],
             gsslib,
             0,
  ...
  if ((gsslib = dlopen("libgssapi_krb5.so.2", RTLD_LAZY)) != NULL)
    gss_init(&list->libraries[list->nlibraries++],
             gsslib,
             1,
             "Using GSSAPI from libgssapi_krb5.so.2");
