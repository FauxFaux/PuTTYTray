 ...
 */
Bignum crt_modpow(
    Bignum base, Bignum exp, Bignum mod, Bignum p, Bignum q, Bignum iqmp)
{
...
static Bignum rsa_privkey_op(Bignum input, struct RSAKey *key)
{
   ...
   */
  random_encrypted = crt_modpow(
      random, key->exponent, key->modulus, key->p, key->q, key->iqmp);
  input_blinded = modmul(input, random_encrypted, key->modulus);
  ret_blinded = crt_modpow(input_blinded,
...
static unsigned char *rsa2_sign(void *key,
                                const char *data,
                                int datalen,
                                int *siglen)
{
...
static void oaep_mask(
    const struct ssh_hash *h, void *seed, int seedlen, void *vdata, int datalen)
{
...
static const struct ssh_kex *const rsa_kex_list[] = {&ssh_rsa_kex_sha256,
                                                     &ssh_rsa_kex_sha1};

const struct ssh_kexes ssh_rsa_kex = {
    sizeof(rsa_kex_list) / sizeof(*rsa_kex_list), rsa_kex_list};
