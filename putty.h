...
#define LATTR_WRAPPED2                                                         \
  0x00000020UL /* with WRAPPED: CJK wide character                             \
...
typedef enum
{
  ...
  MBT_LEFT,
  MBT_MIDDLE,
  MBT_RIGHT, /* `raw' button designations */
  MBT_SELECT,
  MBT_EXTEND,
  MBT_PASTE, /* `cooked' button designations */
  MBT_WHEEL_UP,
  MBT_WHEEL_DOWN /* mouse wheel */
} Mouse_Button;
...
#define CONFIG_OPTIONS(X)                                                      \
  ...
  X(STR,                                                                       \
    NONE,                                                                      \
    remote_cmd2) /* fallback if remote_cmd fails; never loaded or saved */     \
  X(INT, NONE, nopty)                                                          \
  ...
  X(INT,                                                                       \
    NONE,                                                                      \
    lport_acceptall) /* accept conns from hosts other than localhost */        \
  X(INT,                                                                       \
    NONE,                                                                      \
    rport_acceptall) /* same for remote forwarded ports (SSH-2 only) */        \
  /*                                                                           \
...
{
  CONFIG_OPTIONS(CONF_ENUM_DEF) N_CONFIG_OPTIONS
};
#undef CONF_ENUM_DEF
...
void conf_set_str_str(Conf *conf, int key, const char *subkey, const char *val);
...
size_t conf_deserialise(Conf *conf,
                        void *data,
                        size_t maxsize); /*returns size used*/
...
void conf_checkbox_handler(union control *ctrl,
                           ...
                           void *data,
                           int event);
void conf_editbox_handler(union control *ctrl,
                          void *dlg,
                          void *data,
                          int event);
void conf_filesel_handler(union control *ctrl,
                          void *dlg,
                          void *data,
                          int event);
void conf_fontsel_handler(union control *ctrl,
                          void *dlg,
                          void *data,
                          int event);
...
#define IS_SURROGATE(wch)                                                      \
  (((wch) >= HIGH_SURROGATE_START) && ((wch) <= LOW_SURROGATE_END))
