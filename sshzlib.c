...
#define CHARAT(k)                                                              \
  ((k) < 0 ? st->data[(st->winpos + k) & (WINSIZE - 1)] : data[k])
                          ...
                          int compress)
{
  ...
  while (len > 0) {
    ...
    if (compress && len >= HASHCHARS) {
       ...
       */
      nmatch = 0;
      for (off = st->hashtab[hash].first; off != INVALID;
           off = st->win[off].next) {
...
int zlib_compress_block(void *handle,
                        unsigned char *block,
                        int len,
 ...
 */
static struct zlib_table *zlib_mkonetab(int *codes,
                                        ...
                                        int bits)
{
  ...
  for (code = 0; code <= tab->mask; code++) {
    ...
    tab->table[code].nexttable = zlib_mkonetab(
        codes, lengths, nsyms, pfx | (code << pfxbits), pfxbits + bits, nbits);
  }
...
static int zlib_huflookup(unsigned long *bitsp,
                          int *nbitsp,
                          struct zlib_table *tab)
{
...
int zlib_decompress_block(void *handle,
                          unsigned char *block,
                          int len,
                          unsigned char **outblock,
                          int *outlen)
{
  ...
  while (len > 0 || dctx->nbits > 0) {
    ...
    case START:
       ...
       */
      if ((header & 0x0F00) != 0x0800 || (header & 0xF000) > 0x7000 ||
    ...
    case TREES_LEN:
      ...
      if (code < 16)
        ...
        dctx->lenrep =
            (code == 16 && dctx->lenptr > 0 ? dctx->lengths[dctx->lenptr - 1]
                                            : 0);
    ...
    case GOTDISTSYM:
      ...
      while (dctx->len--)
        zlib_emit_char(dctx,
                       dctx->window[(dctx->winpos - dist) & (WINSIZE - 1)]);
      break;
    case UNCOMP_LEN:
      /*
...
const struct ssh_compress ssh_zlib = {"zlib",
                                      zlib_compress_init,
